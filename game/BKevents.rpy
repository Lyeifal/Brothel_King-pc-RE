    ...
    if hasattr(store, 'game'):
        ...
        if game.version != config.version:
            menu:
                ...
                _("Yes, and deactivate future warnings for this game/this version"):                    $ game.version = config.version # quick and dirty
...
label chapter(chapter = None, silent=False): ## Shows the chapter intro with text1 as chapter number and text2 as chapter title. Keep game logic out of this label (use advance_to_chapter instead)
    ...
    python:
        if game.chapter == 0:
            ...
            text2 = "A Dance of Fire and Steel"
...
## 3. Brothel relocation
    ...
    if debug_mode != "quick" and chapter in (1, 6) and not silent:
        ...
        menu:
            ...
            _("A tavern"):                $ brothel.add_room("tavern", forced=True)
    ...
    if start:
        if not story_mode or (chapter > 1 and debug_mode == "quick"):
            ...
            menu:
                ...
                _("Farah, the corrupt guard captain"):                    $ story_flags["c1_path"] = "evil"
        ...
        if len(MC.girls) >= brothel.bedrooms:
            ...
            label found_runaway_girl_no_room:
                ...
                menu:
                    ...
                    _("Ask them to come back tomorrow"):                        $ come_back = 1
        ...
        menu:
            ...
            _("Persuade her to come back."):
                ...
                if (dice(100) + MC.charisma * 5 + girl.love - girl.fear) >= 67:
                    ...
                    if not brothel.can_have(girl.job):
                        $ notify("%s couldn't go back to being a %s, so she was set to rest." % (girl.fullname, girl.job), pic=girl.portrait)
                        $ girl.set_job(None)
...
#    show screen dark_filter
    ...
    menu:
        ...
        _("Train her"):            if girl.MC_interact_counters["train"] >= 1:
                "[girl.fullname] has already trained today. She cannot train more than once per day."
            else:
                hide screen overlay
...
label set_up_extractor(resource): # returns True if extractor has been set-up or action is cancelled
    ...
    if MC.get_items(name=ext, strict=True) and not auto_extractors[resource]: # When the player has an extractor in inventory and the location has none
        menu:
            ...
            _("Yes, set up a resource [ext] in this location"):                play sound resource_dict[resource].sound
    ...
    if auto_extractors[resource + " ON"]: # When an extractor is ON
        menu:
            ...
            _("Turn it OFF"):                play sound s_fiz
        ...
        menu:
            ...
            _("Turn it ON"):                play sound resource_dict[resource].sound
...
label challenge(name, diff, score=False, raw=False, bonus=0, opponent_bonus=0, _sound=True, score_limit=0, forced=False, bonus_text="", opponent_bonus_text=""):
    ...
    if (result and not score) or (result >= score_limit and score) or forced:
        if score:
            $ notify("Challenge result: %s" % plus_text(result, decimals=False))
                ...
                if MC.has_gold(price):
                    ...
                    if result == True:
                        ...
                        if d == 1:
                            ...
                            menu:
                                _("Sure"):                                    call hide_everything() from _call_hide_everything_46
    ...
    menu:
        ...
        "Chat":
            label goldie_chat_menu():
                ...
                menu:
                    _("How are things?"):                        you "How's business?"
                    ...
                    "Can I come in?":
                        ...
                        menu:
                            ...
                            _("Strip for me"):                                play sound s_mmmh
...
## Step 3: Determine success of the show (depends on girl stats, min level and x perf)
    ...
    python:
        cust_score = 0
        intro_title = "%s's introduction" % girl.fullname
        ...
        if debug_mode:
            intro_title += " (score: %i)" % cust_score
...
## Step 4: Show performance - test skills depending on act type ##
    ...
    python:
        ...
        # 4.4 (Naked) End the performance
            cum_desc = "After giving the customers time to look at %s from every angle, Gizel tells the expectant crowd that the show is now over." % g_name
            ...
            if debug_mode: # Debugging only
                perf_title += " (score: %i)" % cust_score_perf
        ...
        # 4.4 (Other sex acts) End the performance
            ...
            if chosen_act in ("sex", "group"):
                if girl.pop_virginity("farm"):
                    cum_desc += "The customers gasp as they notice %s was a virgin. They go wild realizing she just lost her virginity to a mindless minion in a filthy farm exhibit (%s)." % (g_name, event_color['good'] % "large bonus tip") # M6. Finish
                    cum_bonus += 1.0
            ...
            if cust_score_perf > 0:
                ...
                cum_pic = perf_pic
                cum_desc += "After a while, realizing that %s is not going to carry the show much further, Gizel signals the end of the show. The customers leave, disappointed that it was cut short before %s could cum." % (g_name, partner)
...
## Step 6: Results - Girl stats, preferences, mood and minion xp ##
    ...
    python:
        ...
        for mn in chosen_minions:
            mn.xp += girl.rank*2
            if mn.level_up():
                notify("MINION LEVEL UP (level " + str(mn.level) + ")", col="special")
...
label send_to_farm_menu():
    ...
    if girl not in farm.girls:
        if not silent:
            ...
            if can_beg and girl.farm_beg_test():
                ...
                menu:
                    ...
                    _("Ignore her"):                        you "I'm the one in charge here. Get yourself ready. Gizel will pick you up shortly."
                ...
                if girl.is_(["very sub", "lewd"]) or girl.get_stat("libido") >= 40*girl.rank:
                    ...
                    menu:
                        ...
                        _("Force her"):                            you "You are going, and that's final!"
                            $ forced = True
        ...
        if can_follow:
            menu:
                _("Follow her to the farm"):                    $ selected_destination = "farm"
                    $ sent_success = True
                    jump teleport
...
label farm_discovered_weakness(girl):
    ...
    if girl in farm.girls:
        ...
        menu:
            _("Yes, do that"):                $ prog.installation = farm_installations[farm_installations_dict[girl.weakness]] # farm_installations_dict[girl.weakness]
                $ prog.update()
        ...
        python:
            ...
            if context.startswith("rebel"):
                if context.endswith("runaway"):
                    ...
                    text1 = "%s fought Gizel at the Farm and got hurt." % girl.fullname
                    _col = "a little bad"

                elif context.endswith("subdued"):
                    text1 = "%s fought Gizel at the Farm and got subdued." % girl.fullname
                    _col = "a little bad"

            elif context == "refused":
                text1 = "%s refused training (visit Farm to change training mode)." % girl.fullname
                _col = "a little bad"

            elif context == "resisted":
                text1 = "%s resisted training (visit Farm to change training mode)." % girl.fullname
                _col = "a little bad"
    ...
    if girl in farm.girls:
        menu:
            ...
            "Do not warn me about training problems anymore (switch to notifications)":
                ...
                if renpy.call_screen("yes_no", "Are you sure you want to replace Gizel events with notifications?", "Get rid of that bitch", "On second thought..."):
...
label farm_change_training_mode(girl):
    ...
    menu:
        _("Change current training mode ([prog.mode])"):            menu:
                "Change current training mode ([prog.mode])"

                _("Gentle"):                    $ prog.mode = "gentle"
        ...
        "Use her weakness for [girl.weakness]s ([not_avoid])" if farm.knows["weakness"][girl]:
            menu:
                ...
                _("Use it"):                    $ prog.avoid_weakness = False
    ...
    if _girl in farm.girls:
        if prog.duration > 1 and prog.fixed_duration:
            ...
            if context == "normal":
                show screen dark_filter(False)
                menu:
                    _("Follow her to the brothel"):                        hide screen dark_filter
                        jump girls
        ...
        if result == "back":
            ...
            menu:
                ...
                "None of my girls has the skills for this.":
                    ...
                    menu:
                        _("Let me think about it."):                            jobgirl "All right. I'll be back tonight. But it's your last chance! You can't keep important customers waiting."
                            $ add_event("first_contract_return", date=calendar.time, type="night")
    ...
    label broken_menu:
        ...
        menu:
            ...
            "Let her go (free girl)" if girl.free:
                if renpy.call_screen("yes_no", "Are you sure you want to let her go?"):
...
label asylum_return(girl, score):
    ...
    if score > 60: # Restored some sanity
        ...
        menu:
            _("Keep her at the asylum for another month (cost: {image=img_gold_24} [asylum_cost])"):                play sound s_gold
                $ MC.gold -= asylum_cost
...
label asylum_no_room(girl):
    ...
    menu:
        _("Let her stay one more day (50 gold)"):            $ price = 50
            $ duration = 1
...
label acquire_girl(girl, price=0, context="generic", can_follow=True):
    ...
    if price and not MC.has_gold(price):
        ...
        elif price == 0 or renpy.call_screen("yes_no", "Do you really want to buy [girl.fullname] for [price] gold?"):
            if len(MC.girls) < brothel.bedrooms:
                    $ result = True
