# TODO: Translation updated at 2026-06-03 23:34


translate chinese_simplified city_rape_28dc46b8:

    # "Making your way through the [loc], you suddenly hear something."
    "当你穿过[loc]时，你突然听到一些声音。"

# game/BKcity_events.rpy:24
translate chinese_simplified city_rape_795cacf6:

    # "Listening intently, you hear muffled screams coming from a dark corner. You rush towards the source..."
    "你聚精会神地听着，顺着声音走到了一个黑暗的角落，里面传来低沉的尖叫声。你冲向源头...."

# game/BKcity_events.rpy:33
translate chinese_simplified city_rape_69107b8c:

    # ev_girl1 "Mmmmph! Aaaah!"
    ev_girl1 "唔！啊！"

# game/BKcity_events.rpy:35
translate chinese_simplified city_rape_6081d6b6:

    # "A poor girl is being raped mercilessly by a disgusting monster."
    "一个可怜的女孩正在被一个可怕的怪物凌辱。"

# game/BKcity_events.rpy:37
translate chinese_simplified city_rape_956cf43e:

    # ev_girl1 "Aaah, aah, noooo... It's ravaging me! Help! Help me!!!"
    ev_girl1 "啊，啊，不....不要插进来！救命！谁来救救我！！！"

# game/BKcity_events.rpy:39
translate chinese_simplified city_rape_df1322d0:

    # "It seems the evil beast wants to impregnate this poor lass with his cursed semen."
    "看来这头邪恶的怪物想让这个可怜的姑娘为它产下子嗣。"

# game/BKcity_events.rpy:48
translate chinese_simplified city_rape_fae09fc1:

    # "You ready your weapon."
    "你拿出了武器。"

# game/BKcity_events.rpy:50
translate chinese_simplified city_rape_837365c4:

    # you "Back away, you filthy beast!"
    you "滚开，你这肮脏的畜生！"

# game/BKcity_events.rpy:54
translate chinese_simplified city_rape_fea121be:

    # "The monster turns to face you and roars threateningly."
    "怪物转过身来对着你怒吼咆哮。"

# game/BKcity_events.rpy:64
translate chinese_simplified city_rape_a343a597:

    # "Charging at the monster with your sword drawn, you slice off a couple of tentacles that try to stop you."
    "你拔剑冲向怪物，砍下几根袭来的触须。"

# game/BKcity_events.rpy:80
translate chinese_simplified city_rape_8abaafca:

    # "The monster roars in pain, rearing its ugly head towards you. You meet it with all your strength and a metal-clad gauntlet."
    "怪物痛苦地咆哮着，丑陋恶心的头颅径直朝你撞过来。你凝聚全身力量，摆出架势用金属护腕格挡它。"

# game/BKcity_events.rpy:89
translate chinese_simplified city_rape_c9b371e3:

    # "You punch the monster square in what you think is his stupid face, although you're not quite sure."
    "你用拳头向那张愚蠢的脸猛击，尽管你不知道这有没有效。"

# game/BKcity_events.rpy:94
translate chinese_simplified city_rape_28fb9dab:

    # "The monster is stunned and falls flat on the ground in a cloud of dust. The girl screams."
    "怪物被你一拳击昏，在一片尘土中倒在地上。那女孩还是一直在尖叫。"

# game/BKcity_events.rpy:98
translate chinese_simplified city_rape_61cbe017:

    # you "Don't worry, I've got this under control."
    you "别担心，我已经控制住场面了。"

# game/BKcity_events.rpy:100
translate chinese_simplified city_rape_8c1203b7:

    # ev_girl1 "Thanks, you saved me... *sob* I will tell everyone what you did for me..."
    ev_girl1 "谢谢你救了我...我会告诉大家你的英勇事迹的！"

# game/BKcity_events.rpy:102
translate chinese_simplified city_rape_07c5e6ea:

    # you "You be on your way. Don't stay here."
    you "赶紧离开。这里不安全。"

# game/BKcity_events.rpy:104
translate chinese_simplified city_rape_fb02adf5:

    # you "Now, this knocked out monster... What should I do with it?"
    you "现在，怪物倒下了....我该怎么处置它？"

# game/BKcity_events.rpy:117
translate chinese_simplified city_rape_3b16f6a2:

    # "Moving with lighting speed, you aim for weak spots on the creature's bloated body."
    "你以闪电般的速度快速移动，抓住机会瞄准了这个怪物的身上的弱点。"

# game/BKcity_events.rpy:129
translate chinese_simplified city_rape_004738f9:

    # "In a matter of seconds, the monster falls into the dirt, lifeless."
    "几秒钟后，怪物倒在泥土中，奄奄一息。"

# game/BKcity_events.rpy:131
translate chinese_simplified city_rape_bac50fb6:

    # ev_girl1 "Thank, you saved me... *sob* I will tell everyone what you did for me..."
    ev_girl1 "谢谢你救了我...我会告诉大家你的英勇事迹的！"

# game/BKcity_events.rpy:137
translate chinese_simplified city_rape_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:147
translate chinese_simplified city_rape_e9965350:

    # "The beast braces for your attack and parries your blow."
    "怪物识破了你的攻击，侧身闪过。"

# game/BKcity_events.rpy:159
translate chinese_simplified city_rape_9fb8dc99:

    # "The beast hits you to the side, bruising your ribs through your armor and sending you crashing into a wall."
    "随后怪物抓住了破绽，沉重的一击隔着防具令你的肋骨隐隐作痛，你被击飞到墙上。"

# game/BKcity_events.rpy:163
translate chinese_simplified city_rape_abcf6f33:

    # "Before you can get back to your feet, the beast grabs its unfortunate victim and runs off into the night."
    "在你爬起来之前，怪物已经抓住了那个不幸的受害者，逃进了夜色之中。"

# game/BKcity_events.rpy:169
translate chinese_simplified city_rape_036f648a:

    # you "Damn you..."
    you "该死的..."

# game/BKcity_events.rpy:171
translate chinese_simplified city_rape_9e384a5b:

    # "You are wounded and lose your remaining actions for the day."
    "你受伤了，失去了今天剩余的行动力。"

# game/BKcity_events.rpy:179
translate chinese_simplified city_rape_b0a90d75:

    # "You lift your staff high in the air."
    "你将法杖高高举起。"

# game/BKcity_events.rpy:183
translate chinese_simplified city_rape_9ac4bedd:

    # you "Banish spell? Do I look like I have time for this nonsense?"
    you "驱逐咒语？我看起来像是有时间和你废话吗？"

# game/BKcity_events.rpy:185
translate chinese_simplified city_rape_6b5419d7:

    # you "Fry, you piece of shit!"
    you "燃烧吧，混账东西！"

# game/BKcity_events.rpy:189
translate chinese_simplified city_rape_800e1264:

    # you "Thou shall not pass...{w=1.0} your genes!"
    you "你无法从我这里通过....{w=1.0}恶魔！"

# game/BKcity_events.rpy:192
translate chinese_simplified city_rape_c3acfded:

    # you "Back to the underworld with you, foul demon!"
    you "滚回地狱去吧，邪恶的恶魔！"

# game/BKcity_events.rpy:196
translate chinese_simplified city_rape_44dd620e:

    # "The monster hears you as you start your incantation, and turns to face you."
    "当你开始念咒时，怪物听到了你的声音，转身面对着你。"

# game/BKcity_events.rpy:209
translate chinese_simplified city_rape_a160ff8c:

    # "The monster is hit squarely by a powerful bolt of lightning, sending it crashing into the alley's wall."
    "怪物被一道强烈的闪电击中，身体蜷缩成一团以保护自己。"

# game/BKcity_events.rpy:213
translate chinese_simplified city_rape_8bfd5117:

    # you "So, you're still alive?"
    you "嗯？这样都没死？"

# game/BKcity_events.rpy:215
translate chinese_simplified city_rape_4d07f4ed:

    # "The monster curls up and whines as you approach menacingly, staff in hand. It is defeated."
    "当你用法杖指着它时，它发出哀嚎，表示屈服于你。"

# game/BKcity_events.rpy:221
translate chinese_simplified city_rape_a2fbc6f3:

    # you "Shazam!"
    you "沙赞（咒语）！"

# game/BKcity_events.rpy:227
translate chinese_simplified city_rape_cde27e5c:

    # "The monster freezes in its track, and just stands there hazily looking at you."
    "怪物被定身在它前进的方向上，只是迷迷糊糊地站在那里看着你。"

# game/BKcity_events.rpy:229
translate chinese_simplified city_rape_463a3e6f:

    # ev_girl1 "Wh... What happenned?"
    ev_girl1 "什....发生了什么？"

# game/BKcity_events.rpy:231
translate chinese_simplified city_rape_3b0ecb8b:

    # you "I cast a hypnotic spell... Don't worry, child, it is unable to harm you now."
    you "我对它施展了催眠咒....别担心，女士，现在它伤害不了你了。"

# game/BKcity_events.rpy:233
translate chinese_simplified city_rape_f234f69e:

    # ev_girl1 "Oh, you saved me... *sob* I will tell everyone what you did for me... *sob*"
    ev_girl1 "你救了我...我会告诉大家你的英勇事迹的！*抽泣*"

# game/BKcity_events.rpy:235
translate chinese_simplified city_rape_c1f8088e:

    # you "It is best for you to leave now, while I dispose of this creature."
    you "你最好现在就离开，我来处理这个家伙。"

# game/BKcity_events.rpy:249
translate chinese_simplified city_rape_c02efa77:

    # "A great ball of lightning falls from the sky, frying the monster where he stands."
    "一个巨大的闪电球从天而降，把怪物炸飞了。"

# game/BKcity_events.rpy:253
translate chinese_simplified city_rape_cd195c41:

    # "The girl screams in terror as her hair curls and stands out on end. Luckily, she is unharmed."
    "女孩惊恐地尖叫着，她的头发卷了起来，慢慢地站起身来。幸运的是，她没有受伤。"

# game/BKcity_events.rpy:255
translate chinese_simplified city_rape_60b7ab26:

    # you "What a dumb bitch... <sigh>"
    you "都叫你离远点了，笨蛋...<叹气>"

# game/BKcity_events.rpy:262
translate chinese_simplified city_rape_a2fbc6f3_1:

    # you "Shazam!"
    you "沙赞（咒语）！"

# game/BKcity_events.rpy:268
translate chinese_simplified city_rape_660b4123:

    # "The beast whines and roars in vain as the fabric of space and time distorts and it is pulled back to its original dimension. Within a few seconds, it vanishes into thin air."
    "空间和时间的结构发生扭曲，这头怪物徒劳地哀鸣和咆哮，它被送回到原来的维度。几秒钟后，它消失在稀薄的空气中。"

# game/BKcity_events.rpy:271
translate chinese_simplified city_rape_f234f69e_1:

    # ev_girl1 "Oh, you saved me... *sob* I will tell everyone what you did for me... *sob*"
    ev_girl1 "你救了我...我会告诉大家你的英勇事迹的！*抽泣*"

# game/BKcity_events.rpy:277
translate chinese_simplified city_rape_a9baabcd_1:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:283
translate chinese_simplified city_rape_fa1760f7:

    # you "Now take that, you..."
    you "拿着这个..."

# game/BKcity_events.rpy:289
translate chinese_simplified city_rape_e29b3f0b:

    # you "Uh oh..."
    you "哦不...."

# game/BKcity_events.rpy:293
translate chinese_simplified city_rape_49436318:

    # "The creature rushes towards you and viciously slams you off your feet. Your staff lands several feet away from you."
    "那家伙向你冲过来，把你撞倒在地。你的法杖掉在离你几米远的地方。"

# game/BKcity_events.rpy:297
translate chinese_simplified city_rape_1bb2b3de:

    # "As you try frantically to reach for your staff, the beast seizes its victim and disappears into the night, uninterested in a fight."
    "当你试图疯狂地去取回你的法杖时，野兽已经抓住了她，消失在夜色中，它对战斗毫无兴趣。"

# game/BKcity_events.rpy:303
translate chinese_simplified city_rape_9e384a5b_1:

    # "You are wounded and lose your remaining actions for the day."
    "你受伤了，失去了今天剩余的行动力。"

# game/BKcity_events.rpy:311
translate chinese_simplified city_rape_7bc3e35c:

    # "The beast has now noticed you, and glares at you with murderous eyes..."
    "怪物注意到了你，它用凶狠的眼睛瞪着你...."

# game/BKcity_events.rpy:313
translate chinese_simplified city_rape_612d9431:

    # you "Well, discretion is the better part of valor..."
    you "嗯，见义勇为不可取，还是先管好自己吧...."

# game/BKcity_events.rpy:317
translate chinese_simplified city_rape_8c0db1f9:

    # "You hightail it while you still can, ignoring the screams of the poor girl you leave behind."
    "趁你还能逃跑的时候赶快溜吧，你无视了身后那个可怜女孩的尖叫。"

# game/BKcity_events.rpy:325
translate chinese_simplified city_rape_a9b4658c:

    # "As the beast roars, you almost feel as if it is mocking you."
    "当怪物咆哮时，你觉得它是在嘲笑你。"

# game/BKcity_events.rpy:337
translate chinese_simplified city_monster_menu_6c123776:

    # you "Now, this knocked out monster... What should I do with it?" nointeract
    you "现在，这只昏迷的怪物……我该怎么处置它呢？" nointeract

# game/BKcity_events.rpy:341
translate chinese_simplified city_monster_menu_88539a28:

    # you "How would you like to get room and board, pal, and fuck some girls all day?"
    you "要不要来我的农场，包吃包住，可能还有女人玩？"

# game/BKcity_events.rpy:346
translate chinese_simplified city_monster_menu_7fa4cb7f:

    # "[msg]"
    "[msg]"

# game/BKcity_events.rpy:358
translate chinese_simplified city_monster_menu_c964ce72:

    # "After sending the girl off, you drag the submissive monster to the farm. This consumes all your remaining actions."
    "送走女孩后，你把屈服的怪物送到农场。你消耗了所有的行动力。"

# game/BKcity_events.rpy:361
translate chinese_simplified city_monster_menu_f2a54552:

    # you "I know someone who can take care of it, making me some money in the process..."
    you "我知道谁能好好处理你，我还能赚点外快...."

# game/BKcity_events.rpy:371
translate chinese_simplified city_monster_menu_5763083e:

    # "You sell [mn.name] the monster to Willow for [price] gold."
    "你把[mn.name]怪物卖给了怪物猎人薇洛，赚到了[price]枚金币。"

# game/BKcity_events.rpy:373
translate chinese_simplified city_monster_menu_6971486e:

    # willow "Thanks!"
    willow "这家伙品相很不错，辛苦了！"

# game/BKcity_events.rpy:384
translate chinese_simplified city_monster_menu_72d8c12b:

    # you "Your days of happily roaming around and randomly banging innocent girls are over, monster. Prepare to die."
    you "随心所欲地游荡，欺负无辜女孩的日子结束了，怪物。准备受死吧。"

# game/BKcity_events.rpy:403
translate chinese_simplified city_monster_menu_a1280571:

    # "You can't bring yourself to kill off a wounded opponent."
    "你不能残忍地杀死一个没有反抗能力的生物。"

# game/BKcity_events.rpy:405
translate chinese_simplified city_monster_menu_6a46223a:

    # you "Go, fly, little monster. But don't you dare ever attack an innocent girl again."
    you "走吧，丑八怪。再有下次我可不会这么轻易放过你了。"

# game/BKcity_events.rpy:413
translate chinese_simplified city_monster_menu_ac479bb5:

    # "You're not sure if the monster understood you, but it retreats into the night swiftly to nurse its wounds."
    "你不确定怪物是否听懂了你的意思，但它一眨眼就失去了踪影。"

# game/BKcity_events.rpy:415
translate chinese_simplified city_monster_menu_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:428
translate chinese_simplified city_impress_daff885e:

    # "Walking through the [loc], you run into a party of women adventurers."
    "穿过[loc]时，你遇到了一群女冒险者。"

# game/BKcity_events.rpy:446
translate chinese_simplified city_impress_2ac65c34:

    # ev_girl1 "Aw... After so long scouting muddy swamps and dirty dungeons, it's good to be back ..."
    ev_girl1 "啊...我们在泥泞的沼泽和肮脏的地牢里待了好几天，还是人类文明好啊..."

# game/BKcity_events.rpy:450
translate chinese_simplified city_impress_3baa2d51:

    # ev_girl2 "But look at those dumb peasants... I wish we could find a real man..."
    ev_girl2 "但是看看那些愚蠢又下流的农夫....真希望我们能找到一个真正的男人..."

# game/BKcity_events.rpy:452
translate chinese_simplified city_impress_4c5a4d70:

    # ev_girl3 "Patience, sister... Hey look, someone is coming."
    ev_girl3 "心急吃不了热豆腐,姐姐....看，有人来了。"

# game/BKcity_events.rpy:456
translate chinese_simplified city_impress_06040ba6:

    # ev_girl2 "Hey, you! Are you man enough to party with us?"
    ev_girl2 "嘿,小哥哥！有兴趣和姐姐们一起玩点刺激的游戏吗？如果你想，证明给我们看。"

# game/BKcity_events.rpy:467
translate chinese_simplified city_impress_d84bfeb9:

    # you "Of course I am! Watch and enjoy the show, ladies!"
    you "当然有兴趣！美丽的女士们，请让我证明给你们看！"

# game/BKcity_events.rpy:478
translate chinese_simplified city_impress_a762f9b4:

    # "You take off your shirt and start doing a few push ups and abs flexes. You decide to impress them with a backflip."
    "你脱下衬衫，快速地做了几组俯卧撑，你决定用一个后空翻给他们留下深刻的印象。"

# game/BKcity_events.rpy:492
translate chinese_simplified city_impress_9db62c4b:

    # you "Ooooh... My back... <holding back tears>"
    you "唔....我的背...<强忍眼泪>"

# game/BKcity_events.rpy:496
translate chinese_simplified city_impress_9bb6561b:

    # ev_girl2 "Well, that was an unexpected move! I suppose it deserves an artistic note... I bet Katya could show you how it's done..."
    ev_girl2 "嗯，你的表演确实令人吃惊！这也是表演的一部分吗....我敢打赌卡蒂亚知道这是怎么做的...."

# game/BKcity_events.rpy:498
translate chinese_simplified city_impress_53ccd857:

    # ev_girl3 "I don't like to waste my time on losers."
    ev_girl3 "我不喜欢把时间浪费在小丑身上。"

# game/BKcity_events.rpy:500
translate chinese_simplified city_impress_1cd7b566:

    # ev_girl4 "None of us does... This is embarrassing. Let's move on, sisters!"
    ev_girl4 "看来没有人对你感兴趣....真是尴尬。让我们继续去寻找好男人吧，姐妹们！"

# game/BKcity_events.rpy:509
translate chinese_simplified city_impress_b1056eb7:

    # "Taking off your shirt, you perform a series of moves and flips, showing off your toned body and muscles."
    "脱下衬衫，你拔出佩剑耍了几套剑招，步步生风，剑气凌然。"

# game/BKcity_events.rpy:513
translate chinese_simplified city_impress_0fff7e2b:

    # ev_girl3 "So, what do you think?"
    ev_girl3 "姐妹们，你们觉得他怎么样？"

# game/BKcity_events.rpy:515
translate chinese_simplified city_impress_cec4c3ed:

    # ev_girl2 "I don't know... He's good, but I think I could take him in a fight."
    ev_girl2 "我不知道...他很潇洒，但我想我的技巧和他不相上下。"

# game/BKcity_events.rpy:517
translate chinese_simplified city_impress_fba1e449:

    # ev_girl1 "I like him. I can't wait anymore sisters... You! Come here."
    ev_girl1 "他是我的菜。我忍不住了，姐妹们....今天谁也别和我抢！"

# game/BKcity_events.rpy:519
translate chinese_simplified city_impress_b9f03b8c:

    # you "Yes?"
    you "什么事?"

# game/BKcity_events.rpy:521
translate chinese_simplified city_impress_fb1754e7:

    # ev_girl1 "I have waited for this moment... You, strong guy! Let's fuck!"
    ev_girl1 "我等这一刻好久了....勇士！尽情地蹂躏我吧！"

# game/BKcity_events.rpy:530
translate chinese_simplified city_impress_6ef50743:

    # "Ripping off your shirt, you easily lift one of the girls on your shoulder."
    "你扯下衬衫，轻松地把其中一个女孩扛在肩上。"

# game/BKcity_events.rpy:534
translate chinese_simplified city_impress_53ef784c:

    # ev_girl1 "Wow! He's strong!"
    ev_girl1 "哇！好强壮的肱二头肌*抚摸*线条感也很棒！"

# game/BKcity_events.rpy:536
translate chinese_simplified city_impress_b20d0adf:

    # ev_girl3 "And he's got a nice body..."
    ev_girl3 "他的身材也很健美..."

# game/BKcity_events.rpy:538
translate chinese_simplified city_impress_0bb32398:

    # you "I could easily carry the both of you, you know."
    you "把你们两个一起举起来不过是小菜一碟。"

# game/BKcity_events.rpy:540
translate chinese_simplified city_impress_5863f924:

    # ev_girl4 "You could take care of two of us?"
    ev_girl4 "你能同时应付我们两个人吗？"

# game/BKcity_events.rpy:542
translate chinese_simplified city_impress_48b7d138:

    # ev_girl3 "Let's see about that..."
    ev_girl3 "让我们拭目以待...."

# game/BKcity_events.rpy:550
translate chinese_simplified city_impress_4a22364e:

    # "Stripping to your underwear, you demonstrate a series of spectacular martial arts moves."
    "脱下最后的布料，你在她们面前自豪的释放你的阳刚之气和雄性荷尔蒙。你胯下的巨龙不怒自威。"

# game/BKcity_events.rpy:554
translate chinese_simplified city_impress_6c5af145:

    # "The girls watch you with rapture, their eyes glistening with lust."
    "她们的眼睛完全离不开你的巨龙，眼睛里闪烁着欲望的光芒。"

# game/BKcity_events.rpy:556
translate chinese_simplified city_impress_0e53cc21:

    # ev_girl2 "I want to fuck that one so bad..."
    ev_girl2 "他好大，这么大的东西要是插进来..."

# game/BKcity_events.rpy:558
translate chinese_simplified city_impress_68760cfe:

    # ev_girl3 "Oh, me too!"
    ev_girl3 "哦,我会爽到昏过去的！*用手抚摸胸部*"

# game/BKcity_events.rpy:560
translate chinese_simplified city_impress_1de6010c:

    # ev_girl4 "Hey, what about me?"
    ev_girl4 "我能整根吞下去吗？*咽口水*"

# game/BKcity_events.rpy:566
translate chinese_simplified city_impress_1825b68f:

    # "You simply strip to your underwear, and show the girls your perfect muscular body. Their jaws drop in awe."
    "你只需要脱掉内衣，向女孩们展示你完美的肌肉。他们敬畏得张口结舌。"

# game/BKcity_events.rpy:570
translate chinese_simplified city_impress_f75b081d:

    # ev_girl3 "Girls, are we in heaven?"
    ev_girl3 "姐妹们，我们是在天堂吗？"

# game/BKcity_events.rpy:572
translate chinese_simplified city_impress_4d7f2852:

    # ev_girl2 "Look at those pecs..."
    ev_girl2 "看看那头巨龙..."

# game/BKcity_events.rpy:574
translate chinese_simplified city_impress_5ee28dbf:

    # ev_girl4 "And looks like he's got a big... too..."
    ev_girl4 "看起来他有个大...大鸡...."

# game/BKcity_events.rpy:576
translate chinese_simplified city_impress_90323092:

    # ev_girl1 "Oh, I want him!"
    ev_girl1 "噢，我这辈子非他不可了！"

# game/BKcity_events.rpy:578
translate chinese_simplified city_impress_7fe30539:

    # ev_girl2 "I want him too!"
    ev_girl2 "他是我的！"

# game/BKcity_events.rpy:580
translate chinese_simplified city_impress_4c958196:

    # ev_girl4 "Sisters! Calm down... Look how much man there is, we can all share!"
    ev_girl4 "姐妹们！冷静下来....好男人只有一个，我们要学会分享！"

# game/BKcity_events.rpy:582
translate chinese_simplified city_impress_834c2638:

    # ev_girl3 "Oh, what a wonderful idea!"
    ev_girl3 "哦，好主意！"

# game/BKcity_events.rpy:591
translate chinese_simplified city_impress_29c5be5e:

    # you "Well, hello, ladies! I'm always one to party..."
    you "嘿,女士们！有没有兴趣一起参加一场狂欢派对..."

# game/BKcity_events.rpy:593
translate chinese_simplified city_impress_dadddaff:

    # "You start chatting up the girls."
    "你开始和她们搭讪。"

# game/BKcity_events.rpy:601
translate chinese_simplified city_impress_7fef2142:

    # you "So, erm... The weather is quite fine today, don't you think? Even though, yesterday..."
    you "所以,嗯....你不觉得今天天气很好吗？虽然,昨天...."

# game/BKcity_events.rpy:603
translate chinese_simplified city_impress_515d2231:

    # ev_girl3 "Aw... This man is a bore, don't you think?"
    ev_girl3 "啊...不说说话的话你可以闭嘴，你不觉得吗？"

# game/BKcity_events.rpy:607
translate chinese_simplified city_impress_c6722fb7:

    # ev_girl2 "Totally! Get out of here, maggot! We have better things to do than waste our time with you."
    ev_girl2 "完全同意！滚开，小丑！我们的时间很宝贵，不能浪费时间在你身上。"

# game/BKcity_events.rpy:609
translate chinese_simplified city_impress_e0cc606b:

    # ev_girl4 "She's a little harsh, friend, but she's right. We do have to move on... Sorry!"
    ev_girl4 "她有点苛刻，先生，但她是对的。我们得走了....对不起！"

# game/BKcity_events.rpy:615
translate chinese_simplified city_impress_33533913:

    # you "...and then I said: 'I had the cactus in my pants all along!'"
    you "...然后我和她们说:“我的裤子里有棵仙人掌！”"

# game/BKcity_events.rpy:619
translate chinese_simplified city_impress_16eddf55:

    # ev_girl4 "Good one!"
    ev_girl4 "这故事真有趣！"

# game/BKcity_events.rpy:621
translate chinese_simplified city_impress_aecc46b4:

    # ev_girl1 "Can you tell us one more?"
    ev_girl1 "你能再讲一个吗？"

# game/BKcity_events.rpy:623
translate chinese_simplified city_impress_0c26feb9:

    # ev_girl2 "Aw, enough jokes, girls! I have an itch that funny guy here could scratch..."
    ev_girl2 "故事时间到此为止了，姐妹们！我的心有点痒痒的，他很风趣...."

# game/BKcity_events.rpy:625
translate chinese_simplified city_impress_8bac7a4b:

    # ev_girl3 "Ooh, lucky him! *wink*"
    ev_girl3 "哦,没想到你好这口！*眨眼*"

# game/BKcity_events.rpy:631
translate chinese_simplified city_impress_c14379a4:

    # you "Wow, you girls are all so pretty I can't decide which one I'd rather bring home..."
    you "哇，你们都这么漂亮，我都不知道该和谁一起..."

# game/BKcity_events.rpy:635
translate chinese_simplified city_impress_59621a05:

    # ev_girl1 "Oh, you're such a tease!"
    ev_girl1 "哦，你真会讨女孩欢心！"

# game/BKcity_events.rpy:637
translate chinese_simplified city_impress_a09aed63:

    # ev_girl2 "If you can't choose one, why don't you take two of us?"
    ev_girl2 "如果选一个不行，为什么不选我们两个呢？"

# game/BKcity_events.rpy:639
translate chinese_simplified city_impress_01b0316a:

    # ev_girl3 "Sounds like a plan..."
    ev_girl3 "这主意听起来不错..."

# game/BKcity_events.rpy:645
translate chinese_simplified city_impress_b7e3043f:

    # you "...and that's how I brought peace to the kingdom, and ended up in the princess's bed..."
    you "...我就是这样给王国带来了和平，最后躺在公主的床上...."

# game/BKcity_events.rpy:649
translate chinese_simplified city_impress_7299ea59:

    # ev_girl4 "She's a lucky one... I'd like you in my bed as well!"
    ev_girl4 "她很幸运....我也想让你睡在我的床上！"

# game/BKcity_events.rpy:651
translate chinese_simplified city_impress_e4125816:

    # ev_girl3 "So would I!"
    ev_girl3 "我也会！"

# game/BKcity_events.rpy:653
translate chinese_simplified city_impress_629bd96b:

    # ev_girl2 "Hey, what about me?"
    ev_girl2 "我能整根吞下去吗？*咽口水*"

# game/BKcity_events.rpy:655
translate chinese_simplified city_impress_7bbe0445:

    # ev_girl3 "I go first!"
    ev_girl3 "我开动了！"

# game/BKcity_events.rpy:659
translate chinese_simplified city_impress_97f21646:

    # ev_girl2 "No, I do!!!"
    ev_girl2 "不,我先来！！！"

# game/BKcity_events.rpy:665
translate chinese_simplified city_impress_c615e5c3:

    # you "...that's how I ended up with the three ninja sisters. It was a fun night!"
    you "...我就是这样和三个忍者姐妹滚到了床上。真是一个有趣的夜晚！"

# game/BKcity_events.rpy:669
translate chinese_simplified city_impress_f3b3bb54:

    # ev_girl1 "Ah, you're so cool... Talking about sex with you is making me all wet..."
    ev_girl1 "啊，你真迷人....光是和你说话就让我那里湿透了..."

# game/BKcity_events.rpy:671
translate chinese_simplified city_impress_4bca3277:

    # ev_girl2 "Me too... I want some!"
    ev_girl2 "我也是....快把你的宝贝给人家嘛！"

# game/BKcity_events.rpy:675
translate chinese_simplified city_impress_dcadeb26:

    # ev_girl4 "Three sisters? You're a horny devil... Think you could take on the four of us?"
    ev_girl4 "三个忍者姐妹吗？你真是个色鬼...不过你能同时应付我们姐妹四个人吗？"

# game/BKcity_events.rpy:677
translate chinese_simplified city_impress_6f0fb496:

    # ev_girl3 "Yes! Fuck the four of us! I bet we could show the ninja sisters a thing or two..."
    ev_girl3 "是的！我们四个！我打赌我们要比忍者姐妹强得多..."

# game/BKcity_events.rpy:685
translate chinese_simplified city_impress_034035c7:

    # you "Sorry ladies, I have a schedule to keep."
    you "对不起，女士们，我还有安排。"

# game/BKcity_events.rpy:687
translate chinese_simplified city_impress_be00aa42:

    # ev_girl2 "Hey! Who do you think you are!"
    ev_girl2 "嘿！你撩了我们半天想一走了之？"

# game/BKcity_events.rpy:689
translate chinese_simplified city_impress_18b34c32:

    # ev_girl3 "Calm down, sister... Plenty of fish in the sea. Let's look elsewhere."
    ev_girl3 "冷静下来,姐姐....天涯何处无芳草。别吊死在一棵树上。"

# game/BKcity_events.rpy:701
translate chinese_simplified city_impress_c55b17d5:

    # "That could have gone better."
    "本来可以和平解决的。"

# game/BKcity_events.rpy:721
translate chinese_simplified city_impress_a8fab31f:

    # "Before you have a chance to express agreement or disagreement, the woman shoves her clothing to the side and proceeds to milk you for all you're worth, right here in the [loc]."
    "在你还没来得及表示同意或不同意之前，这个女人就把衣服扔到一边，然后开始榨干你所有的钱财，就在这里[loc]。"

# game/BKcity_events.rpy:725
translate chinese_simplified city_impress_58e8061b:

    # ev_girl1 "Oh, yes!!! Cum! Give me more cum! The smell of cock is driving me crazy..."
    ev_girl1 "哦,腥臭的肉棒真是最美味的食物！！给我更多的精液！鸡巴的味道快把我逼疯了...."

# game/BKcity_events.rpy:727
translate chinese_simplified city_impress_9b39d4b4:

    # ev_girl2 "That's our girl..."
    ev_girl2 "我不认识她....*把头转向一边*"

# game/BKcity_events.rpy:729
translate chinese_simplified city_impress_30727017:

    # ev_girl4 "She's such a cum hungry whore, isn't she?"
    ev_girl4 "她简直就是个饥渴的妓女，不是吗？"

# game/BKcity_events.rpy:731
translate chinese_simplified city_impress_a46b4173:

    # ev_girl3 "Way to go, sister..."
    ev_girl3 "做的不错，妹妹...."

# game/BKcity_events.rpy:737
translate chinese_simplified city_impress_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:755
translate chinese_simplified city_impress_93d2c2ea:

    # "Before you know it, a couple of girls are running their hands and mouths all over your body."
    "不知不觉中，女孩们丰韵的肉体紧贴在你的身上，你的头被不知道谁的屁股盖住，两只手被胸部包裹，手指被舌头舔舐，肉棒正插在其中一人的小穴里。"

# game/BKcity_events.rpy:759
translate chinese_simplified city_impress_8086c3f7:

    # "They play with and each other and hungrily wait for you to cum all over them."
    "没有肉棒的人互相抚慰，饥渴地等着你的肉棒插入她们的小穴。"

# game/BKcity_events.rpy:761
translate chinese_simplified city_impress_f2241653:

    # ev_girl1 "I love it!"
    ev_girl1 "我要升天了！干死我吧！"

# game/BKcity_events.rpy:763
translate chinese_simplified city_impress_4f0b1f1d:

    # ev_girl3 "Oh, it's sooo good... Aaaaah!!!"
    ev_girl3 "哦，干死我，惩罚淫荡的我吧！！！"

# game/BKcity_events.rpy:771
translate chinese_simplified city_impress_a9baabcd_1:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:787
translate chinese_simplified city_impress_b485c9ae:

    # "Three of the girls surround you and hungrily grab your manhood."
    "三个女孩围在你身边，被你的雄性荷尔蒙迷得神魂颠倒。"

# game/BKcity_events.rpy:789
translate chinese_simplified city_impress_45fcdc80:

    # ev_girl4 "Master, allow us to suck your dick dry..."
    ev_girl4 "好老公，让我来清理你的鸡巴...."

# game/BKcity_events.rpy:793
translate chinese_simplified city_impress_eac51347:

    # ev_girl3 "Please, spare some cum for me! I love it..."
    ev_girl3 "好哥哥也给我留点牛奶嘛！我还一口都没喝上..."

# game/BKcity_events.rpy:795
translate chinese_simplified city_impress_6b277130:

    # ev_girl2 "You have to give us all a good fucking..."
    ev_girl2 "你可不能厚此薄彼哦，亲爱的..."

# game/BKcity_events.rpy:803
translate chinese_simplified city_impress_55d3ab7e:

    # "Today is a good day..."
    "今天真是幸运日...."

# game/BKcity_events.rpy:809
translate chinese_simplified city_impress_a9baabcd_2:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:825
translate chinese_simplified city_impress_1e1a8a90:

    # "All the girls bend to your will, letting you fuck them all for hours on end."
    "所有的女孩都被你的肉棒征服，你和她们干得天昏地暗。"

# game/BKcity_events.rpy:829
translate chinese_simplified city_impress_8b4ef274:

    # ev_girl2 "Oh, master, it's sooo big... Fuck me harder! Ruin my pussy!"
    ev_girl2 "哦，好老公，你太大了....更用力的操我！我的里面要被你玩坏了！"

# game/BKcity_events.rpy:833
translate chinese_simplified city_impress_dda7fded:

    # ev_girl1 "No, it's my turn! I want your cum!"
    ev_girl1 "不，该到我了，好哥哥！射在我嘴里！"

# game/BKcity_events.rpy:837
translate chinese_simplified city_impress_01b725c3:

    # ev_girl4 "Master, I feel empty inside... Why don't you stick it in my ass?"
    ev_girl4 "亲爱的，我的那里好空虚寂寞....为什么不插在我的屁股上？"

# game/BKcity_events.rpy:841
translate chinese_simplified city_impress_1831fa09:

    # ev_girl3 "Oh, please, let me drink your milk..."
    ev_girl3 "哦，求你了，我的那里好涨，你能帮我止止痒吗...."

# game/BKcity_events.rpy:853
translate chinese_simplified city_impress_e9410bdc:

    # "You have a hard time leaving..."
    "你很难离开...."

# game/BKcity_events.rpy:859
translate chinese_simplified city_impress_efb088bc:

    # "You have earned a lot of prestige. You are exhausted and can no longer act today."
    "你赢得了很多声望。你累坏了，你消耗了所有的行动力。"

# game/BKcity_events.rpy:868
translate chinese_simplified city_impress_7648ac44:

    # "You have received a prestige boost from your [spent_AP] spent AP."
    "你在[spent_AP]中消耗的行动力使你获得了声望。"

# game/BKcity_events.rpy:871
translate chinese_simplified city_impress_c749dbaa:

    # ev_girl1 "Hey, buddy, thanks for the ride... Here, we found this on our last adventure."
    ev_girl1 "嘿，伙计，感谢你为我们排解寂寞...这是我们上次探险时发现的。"

# game/BKcity_events.rpy:873
translate chinese_simplified city_impress_16e83859:

    # ev_girl1 "You can have it. This will be a nice souvenir..."
    ev_girl1 "收下它。看到它就会想起我们...."

# game/BKcity_events.rpy:889
translate chinese_simplified city_slave_98472443:

    # "Wandering around the [loc], you hear a familiar voice."
    "在[loc]附近闲逛时，你听到一个熟悉的声音。"

# game/BKcity_events.rpy:895
translate chinese_simplified city_slave_41f096b7:

    # slavegirl1 "Master [MC.name]! It's good to see you!"
    slavegirl1 "[MC.name]先生！没想到能在这里遇见你！"

# game/BKcity_events.rpy:897
translate chinese_simplified city_slave_d5c1bdf2:

    # you "Hey, you're the trainer from the slave market! What are you doing at the [loc]?"
    you "嘿，你就是奴隶市场上的那个训练师！你在[loc]干什么呢？"

# game/BKcity_events.rpy:899
translate chinese_simplified city_slave_ec6a122a:

    # slavegirl1 "Well, it's about this slave..."
    slavegirl1 "我也不想走这么远...都怪这个不听话的奴隶。"

# game/BKcity_events.rpy:913
translate chinese_simplified city_slave_d25a17fd:

    # slavegirl1 "My master wants her broken and trained by tomorrow, but she stubbornly refuses to do her duties."
    slavegirl1 "老板想在明天之前把她训练成一个合格的奴隶，但是她固执地拒绝履行她的职责。"

# game/BKcity_events.rpy:917
translate chinese_simplified city_slave_b704bef2:

    # slavegirl1 "I have tried everything and I'm at the end of my wits..."
    slavegirl1 "我什么手段都试过了，现在已经无计可施了。"

# game/BKcity_events.rpy:921
translate chinese_simplified city_slave_3bb89407:

    # slavegirl1 "But wait, you're an experienced slave trainer! Maybe you could try some of your tricks on her?"
    slavegirl1 "等等，你手下有许多奴隶！你一定有独家秘诀，帮人家管教一下这个不听话的小猫咪如何？"

# game/BKcity_events.rpy:923
translate chinese_simplified city_slave_e93e26ab:

    # slavegirl1 "Oh, please, Master, I beg of you!"
    slavegirl1 "噢，求你了，帅哥，求你了！*把你的手抱在胸口磨蹭*"

# game/BKcity_events.rpy:934
translate chinese_simplified city_slave_3fc58e28:

    # you "Let me see if I can talk some sense into her."
    you "那就让我来看看这只猫咪有多不听话吧。"

# game/BKcity_events.rpy:936
translate chinese_simplified city_slave_03b23db1:

    # slavegirl1 "Oh, thank you!"
    slavegirl1 "噢,谢谢你！"

# game/BKcity_events.rpy:938
translate chinese_simplified city_slave_40fc8fdc:

    # "You approach the bound slave. She gives you an indifferent look."
    "你靠近被捆起来的奴隶。她鄙视地看了你一眼。"

# game/BKcity_events.rpy:940
translate chinese_simplified city_slave_e4bdc8ff:

    # slave "What do you want?"
    slave "这次又想刷什么花招？"

# game/BKcity_events.rpy:942
translate chinese_simplified city_slave_d170ee15:

    # you "Your behavior is not gonna help, you know."
    you "你知道吗，你的反抗都是徒劳的。"

# game/BKcity_events.rpy:951
translate chinese_simplified city_slave_86661eb6:

    # "You give her a long lecture on the duties of a proper slave."
    "你给她讲了一大段关于奴隶的地位和职责的话。"

# game/BKcity_events.rpy:953
translate chinese_simplified city_slave_f24b61f7:

    # slave "Go to hell."
    slave "下地狱去吧，你们这些卑鄙的家伙。"

# game/BKcity_events.rpy:959
translate chinese_simplified city_slave_d4353c3c:

    # "You gently explain that being a slave is not the end of the world."
    "你温和地解释说，当奴隶并不是世界末日。"

# game/BKcity_events.rpy:961
translate chinese_simplified city_slave_ae26c544:

    # you "You belong to your rightful owner. There is nothing you can do about it. If you accept it, it will be easier for you."
    you "你属于你合法的主人。你没办法拒绝这个现实。如果你接受它，你会更加轻松一些。"

# game/BKcity_events.rpy:963
translate chinese_simplified city_slave_3f38a025:

    # "She sobs."
    "她开始抽泣。"

# game/BKcity_events.rpy:965
translate chinese_simplified city_slave_4780975b:

    # slave "But I don't want to serve the needs of this dirty old man... *sob*"
    slave "但我不想做那个老男人让我做的事...*抽泣*"

# game/BKcity_events.rpy:967
translate chinese_simplified city_slave_0fff381f:

    # you "I'm sorry, but it's not your call."
    you "我很遗憾，但这是你的义务。"

# game/BKcity_events.rpy:969
translate chinese_simplified city_slave_99bff055:

    # slave "..."
    slave "..."

# game/BKcity_events.rpy:973
translate chinese_simplified city_slave_a6260ef4:

    # slave "I understand. The choice is not mine, anyway. I have to find a way to live with this."
    slave "我明白了。无论如何，我的想法都不重要。我得想办法接受这一切。"

# game/BKcity_events.rpy:975
translate chinese_simplified city_slave_8b40cafb:

    # you "This is the right attitude."
    you "很高兴你能想明白。"

# game/BKcity_events.rpy:981
translate chinese_simplified city_slave_d6149901:

    # "Sitting down next to her, you tell her about your life as a brothel owner. You tell her all about your girls, their lives, where they came from, and how they cope with their situation now."
    "坐在她旁边，你告诉她你作为青楼老板的生活。你告诉她关于你的姑娘们的一切，她们的生活，她们从哪里来，她们现在如何处理她们的关系。"

# game/BKcity_events.rpy:983
translate chinese_simplified city_slave_a4ad0d8a:

    # you "At first, everyone is afraid, but with time this work can grow on you. Being a slave doesn't mean giving up on life - it's an entirely new life. With the right mindset, it can be exciting."
    you "起初，每个人都很害怕，但随着时间的推移，你会越来越喜欢这份工作。当奴隶并不意味着放弃生活——这是一种全新的生活。有了正确的心态，它可以是令人兴奋的。"

# game/BKcity_events.rpy:985
translate chinese_simplified city_slave_ab75b130:

    # "As she listens to you, a glimmer of hope appears in her eyes."
    "当她听你说话时，她的眼里闪过一丝希望。"

# game/BKcity_events.rpy:989
translate chinese_simplified city_slave_6fb22144:

    # slave "Really? Can a sex slave find happiness?"
    slave "真的吗？性奴隶也能找到幸福吗？"

# game/BKcity_events.rpy:991
translate chinese_simplified city_slave_d56b70e6:

    # you "Bliss, even. I have girls in my brothel who wouldn't do anything else for all the gold in Xeros."
    you "当然了。我的青楼里有几个姑娘，即使她们挣了足够多的钱，她们也不想去做别的事了。"

# game/BKcity_events.rpy:993
translate chinese_simplified city_slave_a0f8e770:

    # slave "I had never heard something like that before..."
    slave "我以前从未听说过这种事...."

# game/BKcity_events.rpy:995
translate chinese_simplified city_slave_85ac5c55:

    # slave "You have given me some hope. I will give it my best."
    slave "你给了我一些希望。我会尽我最大的努力。"

# game/BKcity_events.rpy:997
translate chinese_simplified city_slave_c7d97402:

    # you "And I am certain you will do great."
    you "我相信你会做得很好。"

# game/BKcity_events.rpy:999
translate chinese_simplified city_slave_190538b3:

    # slave "Thank you, Master [MC.name]."
    slave "谢谢你，[MC.name]先生。"

# game/BKcity_events.rpy:1005
translate chinese_simplified city_slave_0769663a:

    # "You tell her vivid details about the life of some of the famous sex slaves of Zan, explaining how they leveraged their situation to become legends in their own right."
    "你向她生动地讲述了一些著名的泽恩性奴的故事，介绍她们如何在逆境中一步一步成为传奇。"

# game/BKcity_events.rpy:1007
translate chinese_simplified city_slave_4b50ad06:

    # you "If you embrace your condition, you can do amazing things. You can be a godly vessel, harnessing the power of lust and vice and turning it into positive energy..."
    you "如果你能接受自己的现状，你就能做出惊人的事情。就像驾驭一艘神圣的船，驾驭欲望和邪恶的力量，把它变成正能量...."

# game/BKcity_events.rpy:1008
translate chinese_simplified city_slave_18ecba08:

    # you "It is a holy duty."
    you "这是神圣的事业。"

# game/BKcity_events.rpy:1010
translate chinese_simplified city_slave_d2f41821:

    # "She looks at you, eyes open wide in amazement."
    "她看着你，惊讶地睁大了眼睛。"

# game/BKcity_events.rpy:1012
translate chinese_simplified city_slave_f64ae40a:

    # slave "I never knew that being a sex slave could lead to such fulfilling lives!"
    slave "我从来都不知道一个性奴隶能过上如此充实的生活！"

# game/BKcity_events.rpy:1014
translate chinese_simplified city_slave_e5dee2b4:

    # you "You can not only fulfill your life, but that of others. Using your body to serve others is the holiest sacrifice there is."
    you "你不仅可以充实自己的生活，还可以充实他人的生活。用你的身体为他人服务是最神圣的牺牲。"

# game/BKcity_events.rpy:1016
translate chinese_simplified city_slave_57b4dddf:

    # you "And you can enjoy it, too."
    you "你也可以享受这个过程。"

# game/BKcity_events.rpy:1018
translate chinese_simplified city_slave_ea7feb9e:

    # slave "I can see how wrong I was now! When can I start? I want to do it!"
    slave "我现在明白我错得有多离谱了！我什么时候能开始？我要做爱！"

# game/BKcity_events.rpy:1026
translate chinese_simplified city_slave_eebb6fe5:

    # you "I could try an obedience spell."
    you "我可以试试催眠咒语。"

# game/BKcity_events.rpy:1028
translate chinese_simplified city_slave_ade99e4d:

    # slavegirl1 "A spell? Oooh, this is exciting!"
    slavegirl1 "催眠咒语？哦，太令人兴奋了！"

# game/BKcity_events.rpy:1030
translate chinese_simplified city_slave_dc65289a:

    # "You approach the bound girl, and swiftly cut a lock of her hair with your knife."
    "你走近那个被绑住的女孩，以迅雷不及掩耳之势剪掉了她的一绺头发。"

# game/BKcity_events.rpy:1034
translate chinese_simplified city_slave_9adcdda3:

    # slave "Watch it! You psycho!"
    slave "小心点！你这个神经病！"

# game/BKcity_events.rpy:1036
translate chinese_simplified city_slave_64a33bef:

    # "Holding the lock of hair firmly in your palm, you start incanting."
    "把那绺头发紧紧地握在手心，你开始念动咒语。"

# game/BKcity_events.rpy:1044
translate chinese_simplified city_slave_dc474f7c:

    # you "You will now do as I command! Obey your master!"
    you "你要服从我的命令！服从你的主人！"

# game/BKcity_events.rpy:1046
translate chinese_simplified city_slave_ca4d4c16:

    # slave "I command you to fuck off!"
    slave "我命令你滚蛋！"

# game/BKcity_events.rpy:1048
translate chinese_simplified city_slave_d4036871:

    # you "Damn, she's strong-willed..."
    you "该死的,她的意志力好坚定...."

# game/BKcity_events.rpy:1050
translate chinese_simplified city_slave_613e7676:

    # "She resists your attempt."
    "她抗拒着你的咒语。"

# game/BKcity_events.rpy:1056
translate chinese_simplified city_slave_8ac66e62:

    # you "Follow my bidding now, child."
    you "听我说，孩子。"

# game/BKcity_events.rpy:1060
translate chinese_simplified city_slave_e58e4d1a:

    # you "Resistance is futile..."
    you "抵抗是徒劳的..."

# game/BKcity_events.rpy:1062
translate chinese_simplified city_slave_99bff055_1:

    # slave "..."
    slave "..."

# game/BKcity_events.rpy:1064
translate chinese_simplified city_slave_9aa59e9b:

    # slave "Resistance..."
    slave "抵抗...."

# game/BKcity_events.rpy:1066
translate chinese_simplified city_slave_d919dde5:

    # slave "...is futile..."
    slave "...是徒劳的..."

# game/BKcity_events.rpy:1068
translate chinese_simplified city_slave_1cb9453f:

    # "She looks lost. Trying to elicit a reaction, you pinch her breast."
    "她看起来神志不清了。为了测试她的反应，你捏了捏她的胸部。"

# game/BKcity_events.rpy:1072
translate chinese_simplified city_slave_493c83fa:

    # "She shivers, but she doesn't fight you."
    "她的身体抖了一下，但她没有发怒。"

# game/BKcity_events.rpy:1074
translate chinese_simplified city_slave_ad5b4a5f:

    # you "I think it worked."
    you "看来咒语起作用了。"

# game/BKcity_events.rpy:1080
translate chinese_simplified city_slave_5c966fa6:

    # you "Hear my voice. I am the voice of your master."
    you "你是一只欲求不满的母狗。"

# game/BKcity_events.rpy:1084
translate chinese_simplified city_slave_81e3d044:

    # slave "You... You are the voice of my master..."
    slave "我....我是一只..欲求不满的母狗..."

# game/BKcity_events.rpy:1086
translate chinese_simplified city_slave_de31bc83:

    # you "Your master commands you to obey. You must do whatever is asked of you, whenever and wherever you are."
    you "你的舌头和乳头就像你的小穴一样敏感，你无时无刻都在发情，为了能得到主人的肉棒你什么都愿意做。"

# game/BKcity_events.rpy:1088
translate chinese_simplified city_slave_0a925e06:

    # slave "I must do whatever is asked of me, whenever and wherever I am."
    slave "敏感...发情....肉棒....啊{font=1.ttf}❤{/font}我要肉棒{font=1.ttf}❤{/font}"

# game/BKcity_events.rpy:1090
translate chinese_simplified city_slave_b7fdcb00:

    # you "You will never forget my command."
    you "当我说淫奴醒来时，你会忘记我们之间的对话，但你无法忘掉你淫荡的本性。"

# game/BKcity_events.rpy:1092
translate chinese_simplified city_slave_cc63b4b2:

    # slave "I will never forget your command."
    slave "我会忘掉我们之间的对话，我是淫荡的母狗{font=1.ttf}❤{/font}"

# game/BKcity_events.rpy:1094
translate chinese_simplified city_slave_256ac5e8:

    # you "Now, you may rest."
    you "淫奴醒来。"

# game/BKcity_events.rpy:1096
translate chinese_simplified city_slave_8ddcc16e:

    # "The girl looks up to you with bewildered eyes. It's like she notices you for the first time."
    "这个女孩睁开眼，用困惑的眼神看着你。好像她才刚刚看见你。"

# game/BKcity_events.rpy:1098
translate chinese_simplified city_slave_f950481d:

    # slave "Master, what can I do for you? I will do whatever you ask. *smile*"
    slave "主人，人家的小穴好痒啊，求求您用您的大肉棒给我止止痒吧{font=1.ttf}❤{/font}。*吐舌头*"

# game/BKcity_events.rpy:1104
translate chinese_simplified city_slave_bcd6f635:

    # you "I summon thee, succubus. Lend your powers of arousal and lust to that wretched slave."
    you "听我号令，魅魔。把你的血脉和欲望的力量赐予那个下贱的奴隶吧。"

# game/BKcity_events.rpy:1108
translate chinese_simplified city_slave_c1dacd9f:

    # "The hair in your palm whitens and the smell of sulfur floats in the air."
    "你掌心的头发化作一团白烟，一股硫磺的味道飘在空中。"

# game/BKcity_events.rpy:1110
translate chinese_simplified city_slave_91a39f4f:

    # slave "What... What is going on?"
    slave "什么...发生什么事了?"

# game/BKcity_events.rpy:1112
translate chinese_simplified city_slave_3db876d9:

    # "A dark halo surrounds the girl and seems to penetrate beneath her skin. She shivers uncontrollably."
    "一层层黑色的光环围绕着这个女孩的身体，似乎穿透了她的皮肤。她双手抱在胸口控制不住地颤抖。"

# game/BKcity_events.rpy:1116
translate chinese_simplified city_slave_78be7583:

    # slave "Oooh... What's happening to me..."
    slave "噢...好痛...好痒啊~{font=1.ttf}❤{/font}{font=1.ttf}❤{/font}"

# game/BKcity_events.rpy:1118
translate chinese_simplified city_slave_4c003d45:

    # "Her chains jingle as her tits seem to grow larger. Her nipples stand firmly erect, and juice starts running along her thighs."
    "她的链子叮当作响，她的乳房变得又大又挺，乳头渗出乳汁浸湿了衣服。皮肤变得白皙充满弹性，臀部变得挺翘，大腿上有不明液体向下流淌。"

# game/BKcity_events.rpy:1120
translate chinese_simplified city_slave_171152c2:

    # slave "I'm so horny!!! AAAAAH!"
    slave "那里好涨，好痒啊！！！啊啊{font=1.ttf}❤{/font}！*一只手捏着自己的乳头把玩，一只手塞进裙底抚摸阴蒂*"

# game/BKcity_events.rpy:1124
translate chinese_simplified city_slave_f53cddb0:

    # "She trembles as a strong orgasm washes over her."
    "当强烈的高潮冲过她的身体时，她止不住地抽搐。"

# game/BKcity_events.rpy:1126
translate chinese_simplified city_slave_1d713ca3:

    # slave "Someone fuck me! Please, anyone!!!"
    slave "好哥哥们来救救我！快把你们的大肉棒塞到我的口穴，奶穴，小穴里{font=1.ttf}❤{/font}！！！"

# game/BKcity_events.rpy:1130
translate chinese_simplified city_slave_24dcda9a:

    # slave "I'm a dirty whore, I'll do anything... Please... Fuck meeeeeeee!"
    slave "我是个离不开精液的母狗！把我当成你们的鸡巴套子用吧！"

# game/BKcity_events.rpy:1140
translate chinese_simplified city_slave_9f1451e7:

    # you "Sorry, but I can't help you."
    you "对不起，我恐怕爱莫能助。"

# game/BKcity_events.rpy:1142
translate chinese_simplified city_slave_8f297150:

    # slavegirl1 "I see."
    slavegirl1 "我明白了。"

# game/BKcity_events.rpy:1144
translate chinese_simplified city_slave_6aa02aa8:

    # slavegirl1 "Of course. It was a silly request. I apologize, Master."
    slavegirl1 "当然，很抱歉这个愚蠢的奴隶占用了您的时间，我对此向您表示歉意，先生。"

# game/BKcity_events.rpy:1159
translate chinese_simplified city_slave_aa59c0b5:

    # you "I'm sorry, it didn't work."
    you "对不起，看起来效果不是很理想。"

# game/BKcity_events.rpy:1161
translate chinese_simplified city_slave_ad7409c3:

    # "The slave trainer looks down."
    "奴隶训练师微微低下头。"

# game/BKcity_events.rpy:1165
translate chinese_simplified city_slave_b0d1501d:

    # slavegirl1 "That was a long shot. Thank you for trying."
    slavegirl1 "看来没人能驯服这只野猫了。还是感谢您做出的尝试。"

# game/BKcity_events.rpy:1171
translate chinese_simplified city_slave_fc7df733:

    # slavegirl1 "You did it! I hope she won't change her mind..."
    slavegirl1 "您是怎么做到的！希望她就这么一直乖下去。"

# game/BKcity_events.rpy:1173
translate chinese_simplified city_slave_a8201bf0:

    # slavegirl1 "Thank you so much for your help."
    slavegirl1 "非常感谢您的帮助。"

# game/BKcity_events.rpy:1179
translate chinese_simplified city_slave_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:1189
translate chinese_simplified city_slave_1f77043b:

    # slavegirl1 "Amazing! You succeeded where I failed... You are truly something."
    slavegirl1 "太神奇了！我用了那么多办法都没能....您真了不起。"

# game/BKcity_events.rpy:1193
translate chinese_simplified city_slave_6a0acc1f:

    # slavegirl1 "Perhaps you can help me with something else, if you have time..."
    slavegirl1 "如果您还有时间的话，介意帮我一个小忙吗。"

# game/BKcity_events.rpy:1195
translate chinese_simplified city_slave_79360ee2:

    # slavegirl1 "One of the girls needs advanced sexual training. Would you care to help me with it?"
    slavegirl1 "有一个女孩需要一些高级的性训练。您愿意帮我吗？"

# game/BKcity_events.rpy:1197
translate chinese_simplified city_slave_ad2f56b0:

    # slavegirl1 "She can do anything you ask."
    slavegirl1 "不管您让她做什么她都会照做的。"

# game/BKcity_events.rpy:1222
translate chinese_simplified city_slave_311afc8c:

    # slavegirl1 "I see, that's too bad then."
    slavegirl1 "我明白了，那真是太遗憾了。"

# game/BKcity_events.rpy:1228
translate chinese_simplified city_slave_a9baabcd_1:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:1236
translate chinese_simplified city_slave_b250f193:

    # slavegirl1 "This is a great choice! Let's get to it. *smile*"
    slavegirl1 "这是一个明智的选择！让我们开始吧。*微笑*"

# game/BKcity_events.rpy:1240
translate chinese_simplified city_slave_ffb55243:

    # slavegirl1 "There she is! Be a nice girl now, Master [MC.name] is going to take good care of you."
    slavegirl1 "她就在那！现在乖乖听话，[MC.name]先生会好好照顾你的。"

# game/BKcity_events.rpy:1250
translate chinese_simplified city_slave_99a292db:

    # slave "Oh!!!"
    slave "遵命主人！！！"

# game/BKcity_events.rpy:1254
translate chinese_simplified city_slave_cbaacbae:

    # slavegirl1 "Now, be a good girl and suck his dick off, like I showed you last time, ok?"
    slavegirl1 "现在，温柔地，用你的嘴把拉链拉开，让肉棒解放出来，就像我上次演示给你的那样，好吗？"

# game/BKcity_events.rpy:1258
translate chinese_simplified city_slave_36eacddf:

    # slavegirl1 "Master [MC.name] will now fuck your pussy. Brace yourself, now, he's got a big dick. Mmmh..."
    slavegirl1 "[MC.name]大人现在要操你的小穴了。打起精神来，用你的唾液润滑一下龟头，我知道他的鸡巴可能有点太大了。Mmmh...."

# game/BKcity_events.rpy:1262
translate chinese_simplified city_slave_0657eae3:

    # slavegirl1 "It's good we performed that enema together this morning, don't you think? Raise your ass a little... There!"
    slavegirl1 "还记得今天早上我帮你灌肠吗，是不是很舒服？抬起你的屁股...用你的双手掰开你的后庭！"

# game/BKcity_events.rpy:1266
translate chinese_simplified city_slave_ae115b8a:

    # slavegirl1 "Master [MC.name] will show you something new today. Make sure to enjoy it!"
    slavegirl1 "[MC.name]先生今天要教你学会些新技巧。尽情享受吧！"

# game/BKcity_events.rpy:1276
translate chinese_simplified city_slave_465730fc:

    # slave "Aaaaaah!!!"
    slave "啊{font=1.ttf}❤{/font}！！！"

# game/BKcity_events.rpy:1278
translate chinese_simplified city_slave_4d802672:

    # slavegirl1 "Take some time to get used to the feeling... Ease into it..."
    slavegirl1 "花点时间来适应这种感觉....慢慢习惯就好...."

# game/BKcity_events.rpy:1282
translate chinese_simplified city_slave_9d371e26:

    # "You come and go for a while. You can tell she is starting to enjoy it."
    "你掌握好节奏来回抽插。你可以看出她开始喜欢上这种感觉了。"

# game/BKcity_events.rpy:1284
translate chinese_simplified city_slave_f5e822a4:

    # slavegirl1 "There, that's better..."
    slavegirl1 "你做得很好，姑娘..."

# game/BKcity_events.rpy:1290
translate chinese_simplified city_slave_340bb775:

    # you "Let's not forget your other hole!"
    you "别忘了你的另一个洞！"

# game/BKcity_events.rpy:1298
translate chinese_simplified city_slave_a8f91e1b:

    # "You increase your pace."
    "你开始加快节奏和力度。"

# game/BKcity_events.rpy:1302
translate chinese_simplified city_slave_46143a51:

    # you "Oh, you're good..."
    you "哦,里面好紧..."

# game/BKcity_events.rpy:1304
translate chinese_simplified city_slave_d5de3b60:

    # slavegirl1 "Looks like you managed to turn Master [MC.name] on... *giggle*"
    slavegirl1 "看起来你成功地引起了[MC.name]先生的兴趣....*笑*"

# game/BKcity_events.rpy:1308
translate chinese_simplified city_slave_5d29cd48:

    # you "I'm close..."
    you "我要射出来了...."

# game/BKcity_events.rpy:1312
translate chinese_simplified city_slave_802d5805:

    # slave "Ohhhh, aaaahhh... AAAAAH!"
    slave "噢,啊哈....啊啊啊哈{font=1.ttf}❤{/font}！"

# game/BKcity_events.rpy:1321
translate chinese_simplified city_slave_6c6ac6f8:

    # you "HAAAAAAAAAAAAA!!!"
    you "哈哈哈哈哈哈！！！"

# game/BKcity_events.rpy:1327
translate chinese_simplified city_slave_3aa91c80:

    # "You shoot your load inside her mouth, and she gulps it down with a hiccup."
    "你把精液全都射进了她的喉咙，她在嘴里细细品味了一番后全部咽了下去。"

# game/BKcity_events.rpy:1329
translate chinese_simplified city_slave_e4ddfa39:

    # "Pulling out, you spread white cum all over her face."
    "拔出来后，你给她的脸做了一次免费的面膜保养。她连睁开眼睛都有些困难。"

# game/BKcity_events.rpy:1333
translate chinese_simplified city_slave_b9cb6847:

    # "You cum buckets inside her, and her body shakes in a massive orgasm."
    "你在她体内疯狂地射精，她的身体在高潮中颤抖。小腹微微隆起。"

# game/BKcity_events.rpy:1335
translate chinese_simplified city_slave_652cb634:

    # "Pulling out, you shoot your last load over her body."
    "你把最后一发射在她的身上。她的身体变得黏糊糊的。"

# game/BKcity_events.rpy:1339
translate chinese_simplified city_slave_b4ca92e6:

    # "She seems lost in the feeling of her two holes being raped."
    "她似乎迷失在这种双管齐下的快感中，两眼翻白。"

# game/BKcity_events.rpy:1341
translate chinese_simplified city_slave_45586e61:

    # you "You like it, don't you, slave? What if I push deeper!"
    you "喜欢吗，母狗？我还能插得更深呢！"

# game/BKcity_events.rpy:1345
translate chinese_simplified city_slave_21261637:

    # slave "AAAAH! UHHH!!!"
    slave "啊！呜！！！"

# game/BKcity_events.rpy:1347
translate chinese_simplified city_slave_9c0b235e:

    # slavegirl1 "Look like she's going to cum! This is so fun!"
    slavegirl1 "看起来她要憋不住了！真是太有趣了！"

# game/BKcity_events.rpy:1353
translate chinese_simplified city_slave_20f91c9b:

    # slave "AAAAAAAAAAAAAAH!!!!"
    slave "啊啊啊啊{font=1.ttf}❤{/font}啊啊啊啊啊{font=1.ttf}❤{/font}!!!!"

# game/BKcity_events.rpy:1362
translate chinese_simplified city_slave_52216f2d:

    # "Trembling with pleasure, the girl collapses in a pool of bodily fluids."
    "女孩用双手遮住自己的脸，害羞地摊倒在地上，地上很快形成了一滩水池。"

# game/BKcity_events.rpy:1366
translate chinese_simplified city_slave_15f2cc69:

    # you "That was good..."
    you "好爽啊...."

# game/BKcity_events.rpy:1372
translate chinese_simplified city_slave_3b4b082b:

    # you "Thank you, I enjoyed it."
    you "谢谢，我很享受这段美好时光。"

# game/BKcity_events.rpy:1374
translate chinese_simplified city_slave_2764ba73:

    # slavegirl1 "My pleasure! *smile*"
    slavegirl1 "这是我的荣幸！*微笑*"

# game/BKcity_events.rpy:1376
translate chinese_simplified city_slave_34169bac:

    # you "Maybe I could show the girls a thing or two back at the brothel..."
    you "也许我可以给青楼的姑娘们说说今天发生的事...."

# game/BKcity_events.rpy:1380
translate chinese_simplified city_slave_51a349cb:

    # "You have earned prestige. Some of your girls have increased their [act] stat."
    "你获得了一些声望。你的一些女孩提高了他们的[act]。"

# game/BKcity_events.rpy:1401
translate chinese_simplified city_slave_50201891:

    # slavegirl1 "That was incredible! You truly are a master among slave trainers!"
    slavegirl1 "这真是难以置信！您简直就是训奴大师！"

# game/BKcity_events.rpy:1405
translate chinese_simplified city_slave_6c7e6493:

    # slavegirl1 "I want to personally reward you... Miki, come over here!"
    slavegirl1 "让我用最高礼仪来答谢您....美琪，过来！"

# game/BKcity_events.rpy:1417
translate chinese_simplified city_slave_227495f0:

    # slavegirl2 "Mmmh, aaah..."
    slavegirl2 "嗯，啊...."

# game/BKcity_events.rpy:1419
translate chinese_simplified city_slave_77342fea:

    # slavegirl1 "This is so big..."
    slavegirl1 "这太大了..."

# game/BKcity_events.rpy:1427
translate chinese_simplified city_slave_849622ef:

    # slavegirl1 "And such a strong taste... I love it..."
    slavegirl1 "好浓的肉棒气息...这股味道让我神魂颠倒..."

# game/BKcity_events.rpy:1431
translate chinese_simplified city_slave_d5bb2c99:

    # slavegirl2 "Mmmmh..."
    slavegirl2 "嗯嗯..."

# game/BKcity_events.rpy:1433
translate chinese_simplified city_slave_eefaf412:

    # slavegirl1 "It's pulsating! It's so warm!"
    slavegirl1 "我能感受到肉棒的脉动！好烫啊{font=1.ttf}❤{/font}！"

# game/BKcity_events.rpy:1447
translate chinese_simplified city_slave_b502c3b5:

    # slavegirl1 "Aaaaaaah!!!"
    slavegirl1 "啊啊啊！！！"

# game/BKcity_events.rpy:1449
translate chinese_simplified city_slave_f3666851:

    # slavegirl2 "Oooooh!!!"
    slavegirl2 "哇！！！"

# game/BKcity_events.rpy:1462
translate chinese_simplified city_slave_f5245a47:

    # slavegirl1 "Master... You came so much!"
    slavegirl1 "先生....您还能再射这么多{font=1.ttf}❤{/font}！"

# game/BKcity_events.rpy:1464
translate chinese_simplified city_slave_f44ca558:

    # slavegirl2 "It's all over my face..."
    slavegirl2 "我脸上和身上到处都是...好浪费啊...*舔*"

# game/BKcity_events.rpy:1468
translate chinese_simplified city_slave_28ffc30e:

    # slavegirl1 "So delicious... More..."
    slavegirl1 "美味的...我还想要{font=1.ttf}❤{/font}..."

# game/BKcity_events.rpy:1470
translate chinese_simplified city_slave_3d8c5057:

    # "She licks the cum off her lips, giving you a salacious look."
    "她舔掉嘴唇上的精液，拉开肩膀上的吊带，给了你一个充满诱惑的眼神。"

# game/BKcity_events.rpy:1472
translate chinese_simplified city_slave_0ab5aa90:

    # slavegirl1 "The night has only just started..."
    slavegirl1 "夜晚才刚刚开始...."

# game/BKcity_events.rpy:1480
translate chinese_simplified city_slave_a9046625:

    # "You have earned prestige. Some of your girls have increased their sex stats.\nYou have expended all your actions for the day."
    "你获得了一些声望。你们的一些女孩提高了她们的性技能力。你耗尽了剩余的行动力。"

# game/BKcity_events.rpy:1483
translate chinese_simplified city_slave_c271f3ee:

    # "You have received a prestige boost from your [MC.interactions] spent AP."
    "你消耗的[MC.interactions]点行动力增加了你的声望。"

# game/BKcity_events.rpy:1509
translate chinese_simplified city_gamble_7d17a49f:

    # "You are strolling around the [loc], daydreaming, when you hear someone call out to you."
    "你正在[loc]闲逛，发呆，这时你听到有人叫你。"

# game/BKcity_events.rpy:1529
translate chinese_simplified city_gamble_8d2c768a:

    # ev_girl1 "Hello, mister! I'm bored... Won't you play with me? We can wage a few denars..."
    ev_girl1 "嘿,先生！有没有感到无聊....要不要试试和我来赌一把？赢家通吃...."

# game/BKcity_events.rpy:1531
translate chinese_simplified city_gamble_82f4a93b:

    # "A cute, innocent-looking girl is challenging you to a popular game."
    "一个性感、迷人的女孩邀请你来玩一个热门的玩法。"

# game/BKcity_events.rpy:1540
translate chinese_simplified city_gamble_8c163964:

    # you "A game? Sounds exciting!"
    you "赢家通吃吗？听起来很令人兴奋。"

# game/BKcity_events.rpy:1542
translate chinese_simplified city_gamble_b749aeee:

    # ev_girl1 "Yay! *smile*"
    ev_girl1 "不到最后，可不知道是谁通吃哦！*微笑*"

# game/BKcity_events.rpy:1546
translate chinese_simplified city_gamble_1c754eb2:

    # "You win the first few games, making quite a bit of money."
    "前几局你运气很好，赢了不少钱。"

# game/BKcity_events.rpy:1548
translate chinese_simplified city_gamble_31ca0d2f:

    # ev_girl1 "Haha, you really got me there mister!"
    ev_girl1 "哈哈，我有点汗流浃背了，难道您是个高手？！"

# game/BKcity_events.rpy:1550
translate chinese_simplified city_gamble_bd16486a:

    # you "Hey, this is fun!"
    you "怎么，难道你想说投降输一半？！"

# game/BKcity_events.rpy:1554
translate chinese_simplified city_gamble_19359337:

    # "You keep playing, but your luck runs out somehow."
    "你越玩越大，可不知怎么的，运气似乎不再那么好了。"

# game/BKcity_events.rpy:1556
translate chinese_simplified city_gamble_7de57ce5:

    # "Before you know it, your gains have been wiped out, and the girl is sitting in front of a pile of gold."
    "在你意识到问题之前，你之前赢来的钱连带你的一部分本金都已经输掉了，女孩面前堆起高高的金币山。"

# game/BKcity_events.rpy:1558
translate chinese_simplified city_gamble_b65f8599:

    # ev_girl1 "Well, mister, it looks like lady luck is favoring me now... Teehee..."
    ev_girl1 "看来幸运女神现在站在我这一边....嘻嘻嘻...."

# game/BKcity_events.rpy:1559
translate chinese_simplified city_gamble_40703e17:

    # ev_girl1 "But you can catch up!"
    ev_girl1 "不过你还是有翻盘的机会的！"

# game/BKcity_events.rpy:1561
translate chinese_simplified city_gamble_d6d23918:

    # you "Aw..."
    you "哦..."

# game/BKcity_events.rpy:1563
translate chinese_simplified city_gamble_5517eeab:

    # "Suddenly, you realize that sweet, innocent girl is not so innocent after all. She must be an experienced gambler. She's been playing you for a fool from the beginning."
    "此时，你才意识到这个可爱的女孩一点都不“可爱”。她一定是个有经验的赌徒。她从一开始就把你当凯子耍。"

# game/BKcity_events.rpy:1565
translate chinese_simplified city_gamble_ff747cb2:

    # ev_girl1 "Let's play another one!"
    ev_girl1 "再来一把！"

# game/BKcity_events.rpy:1582
translate chinese_simplified city_gamble_fbf8a65d:

    # you "All right, let's do this."
    you "谁怕谁！"

# game/BKcity_events.rpy:1586
translate chinese_simplified city_gamble_d57674e3:

    # "Concentrating on her facial expressions and body language, you begin to understand her winning strategy."
    "你开始仔细观察她的面部表情和肢体动作，你逐渐掌握了她的习惯。"

# game/BKcity_events.rpy:1588
translate chinese_simplified city_gamble_5c9dccc1:

    # "Putting this to your advantage, you manage to regain the edge and make up for your losses."
    "了解了她之后你重新占据了优势，她面前的金币一点一点的减少。"

# game/BKcity_events.rpy:1590
translate chinese_simplified city_gamble_cef78f8d:

    # "The girl looks unassured now, and she is losing her composure."
    "她看起来不再冷静，表情越来越凝重。"

# game/BKcity_events.rpy:1594
translate chinese_simplified city_gamble_de6b3fac:

    # ev_girl1 "Grrr, you're so lucky all of a sudden! But this time, I'll show you!"
    ev_girl1 "啊，看来你又找回了你的运气，不过我还有这一手！"

# game/BKcity_events.rpy:1596
translate chinese_simplified city_gamble_b2168319:

    # "She bets all of her remaining credit on her next move."
    "她把所有的筹码都推倒在桌上，气势十足。"

# game/BKcity_events.rpy:1598
translate chinese_simplified city_gamble_fcae88b9:

    # ev_girl1 "What do you say about that!"
    ev_girl1 "梭哈！你敢跟吗？"

# game/BKcity_events.rpy:1600
translate chinese_simplified city_gamble_54e9617b:

    # you "And... {w=1.0}I win!"
    you "恩...{w=1.0}你的声音很大可是牌不够大，我赢了！"

# game/BKcity_events.rpy:1606
translate chinese_simplified city_gamble_f31f9295:

    # ev_girl1 "Nooooooooo!!!"
    ev_girl1 "不---！！！"

# game/BKcity_events.rpy:1610
translate chinese_simplified city_gamble_e977888f:

    # "You try to focus and observe her play feverishly, looking for a weakness."
    "你试着集中注意力，仔细地观察着她的表情，寻找弱点。"

# game/BKcity_events.rpy:1612
translate chinese_simplified city_gamble_a75181bc:

    # "But she remains unpredictable. Your losses pile up."
    "但她的表情难以捉摸。你的筹码越来越少。"

# game/BKcity_events.rpy:1614
translate chinese_simplified city_gamble_becd0c06:

    # you "Oh no..."
    you "哦不..."

# game/BKcity_events.rpy:1618
translate chinese_simplified city_gamble_fd340498:

    # ev_girl1 "Hahaha! *evil laugh*"
    ev_girl1 "哈哈哈！*邪恶笑*"

# game/BKcity_events.rpy:1630
translate chinese_simplified city_gamble_2cb9dcbb:

    # you "Ok, but let's take a break."
    you "好吧，让我们中场休息一下。"

# game/BKcity_events.rpy:1632
translate chinese_simplified city_gamble_d4c4c437:

    # "Beaming with self-content, the girl gets up to buy herself a drink with your hard-earned money."
    "女孩欣然应允，用你输掉钱给她自己点了一杯鸡尾酒。"

# game/BKcity_events.rpy:1634
translate chinese_simplified city_gamble_1a3f7fa9:

    # you "This is the right time..."
    you "就是现在...."

# game/BKcity_events.rpy:1640
translate chinese_simplified city_gamble_56297960:

    # "Muttering a spell, you draw the shape of a pentagram into the air over the game table."
    "你念动咒语，牌桌上浮现出一个五芒星的图案。"

# game/BKcity_events.rpy:1642
translate chinese_simplified city_gamble_2dbfeada:

    # "The table glows faintly, the halo receding just in time for her to return to the table."
    "桌子发出微弱的光芒，光晕渐渐退去，她端着酒杯正好回到座位上。"

# game/BKcity_events.rpy:1646
translate chinese_simplified city_gamble_72e8fbb8:

    # ev_girl1 "So, where were we?"
    ev_girl1 "你要换种玩法吗？还是继续？"

# game/BKcity_events.rpy:1648
translate chinese_simplified city_gamble_9045087f:

    # you "I was about to kick your sweet ass, I believe!"
    you "我的牌运才刚刚开始呢！"

# game/BKcity_events.rpy:1650
translate chinese_simplified city_gamble_b9e6afb2:

    # ev_girl1 "Bring it on! *laugh*"
    ev_girl1 "是吗，我好害怕哦！*笑*"

# game/BKcity_events.rpy:1652
translate chinese_simplified city_gamble_40e5a4b3:

    # "The next few minutes see you winning round after round. The girl looks increasingly frustrated."
    "接下来的几局，你每一轮都在赢。女孩看起来越来越沮丧。"

# game/BKcity_events.rpy:1656
translate chinese_simplified city_gamble_d4f15bd0:

    # ev_girl1 "This is uncanny! But your luck will run out, eventually! *frown*"
    ev_girl1 "不过是一时走运！我不信你能一直这么赢下去！*皱眉*"

# game/BKcity_events.rpy:1658
translate chinese_simplified city_gamble_b3d16850:

    # "Your luck doesn't run out, however, and before long she has used up all her credit."
    "你的运气会不会用光是没办法知道了，但是女孩的筹码已然见底。"

# game/BKcity_events.rpy:1660
translate chinese_simplified city_gamble_7ed54380:

    # ev_girl1 "What the hell! You're cheating, I'm sure! *mad*"
    ev_girl1 "你到底是怎么做到的！你绝对出了老千！*疯狂*"

# game/BKcity_events.rpy:1662
translate chinese_simplified city_gamble_6f00c98e:

    # you "There, there, don't be a bad sport."
    you "好了，胜负乃赌桌常事，不要输不起。"

# game/BKcity_events.rpy:1666
translate chinese_simplified city_gamble_93270bbf:

    # "You wait for her to look away so that you can cast your spell undetected."
    "你等着她把目光移开，这样你就可以在不被发现的情况下施展你的魔法了。"

# game/BKcity_events.rpy:1668
translate chinese_simplified city_gamble_7dfa2122:

    # "While she is busy counting her gold with a satisfied look on her face, your start muttering a few words, raising your hand towards the game."
    "当她正忙着数她的金币，脸上露出满意的表情时，你开始喃喃自语，向牌桌伸出手来。"

# game/BKcity_events.rpy:1670
translate chinese_simplified city_gamble_c85b6ff4:

    # "The words don't come easily, however, and you have a hard time harnessing magical energy while doing your best to look casual."
    "然而，冗长的咒语没有那么好念，你也无法一边装作随意耳朵样子，一边驾驭好魔法元素。"

# game/BKcity_events.rpy:1674
translate chinese_simplified city_gamble_9dc3e641:

    # ev_girl1 "What are you doing?"
    ev_girl1 "你在做什么？"

# game/BKcity_events.rpy:1682
translate chinese_simplified city_gamble_da3b5fec:

    # you "Oh, uh, ahem... Just stretching, hahahahaha!"
    you "哦,嗯,嗯....只是胳膊酸了做个伸展运动，哈哈哈哈！"

# game/BKcity_events.rpy:1684
translate chinese_simplified city_gamble_a005b622:

    # ev_girl1 "You're acting weird... Don't try to cheat, now, ok?"
    ev_girl1 "你有点奇怪....别想着作弊，不然你知道会有什么下场的，对吧？"

# game/BKcity_events.rpy:1686
translate chinese_simplified city_gamble_6878d040:

    # you "Of course not, hahahahaha! *sweat*"
    you "当然不会，哈哈哈哈哈！*汗水*"

# game/BKcity_events.rpy:1688
translate chinese_simplified city_gamble_fa8448bd:

    # "You fail to cast your spell. Your luck just goes from bad to worse, and you end up with heavy losses."
    "你没能成功施咒。后面的赌局尽管你赢了几把，但还是损失惨重。"

# game/BKcity_events.rpy:1695
translate chinese_simplified city_gamble_d192838d:

    # you "It's been a fun game, but it's getting late. I'll play you another time."
    you "今天就到此为止了，天色已晚，我还有很多事要忙。"

# game/BKcity_events.rpy:1697
translate chinese_simplified city_gamble_dfd842f8:

    # "The girl makes a show of looking disappointed."
    "那女孩露出一脸失望的样子。"

# game/BKcity_events.rpy:1699
translate chinese_simplified city_gamble_fbd1f22d:

    # ev_girl1 "Aw, that's too bad, I'm sure you were about to win big... *sneer*"
    ev_girl1 "哦，真可惜，继续玩下去说不定你能翻盘的....*冷笑*"

# game/BKcity_events.rpy:1715
translate chinese_simplified city_gamble_04964a88:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:1726
translate chinese_simplified city_gamble_963ed26a:

    # you "All right honey, time to cough up the gold."
    you "好了，亲爱的，是时候把金币都吐出来了。"

# game/BKcity_events.rpy:1730
translate chinese_simplified city_gamble_f1b8b9a3:

    # "She looks at you with teary eyes."
    "她含泪看着你。"

# game/BKcity_events.rpy:1732
translate chinese_simplified city_gamble_3771127e:

    # ev_girl1 "Aw... You're robbing me blind..."
    ev_girl1 "啊....你简直就是个强盗...."

# game/BKcity_events.rpy:1742
translate chinese_simplified city_gamble_b1b89f60:

    # "You have earned %(gain)d gold."
    "你获得了 %(gain)d 枚金币。"

# game/BKcity_events.rpy:1750
translate chinese_simplified city_gamble_3a3405b9:

    # "She looks at you with panicked eyes."
    "她用惊恐的眼神看着你。"

# game/BKcity_events.rpy:1752
translate chinese_simplified city_gamble_41ab9b9e:

    # ev_girl1 "Uh... It's... I..."
    ev_girl1 "嗯....这是....我...."

# game/BKcity_events.rpy:1754
translate chinese_simplified city_gamble_d8259c57:

    # you "What now?"
    you "你什么？"

# game/BKcity_events.rpy:1756
translate chinese_simplified city_gamble_c6c7993a:

    # "She opens her purse. A mite flies out, but otherwise it is completely empty."
    "她打开了钱包，但里面空空如也。"

# game/BKcity_events.rpy:1758
translate chinese_simplified city_gamble_3c7d3a1e:

    # ev_girl1 "I do not have the gold to repay you, mister."
    ev_girl1 "我没办法兑付你的筹码，先生。"

# game/BKcity_events.rpy:1762
translate chinese_simplified city_gamble_a577ad0e:

    # you "What?"
    you "什么?"

# game/BKcity_events.rpy:1764
translate chinese_simplified city_gamble_4b439b96:

    # ev_girl1 "I don't have any money... This gig was supposed to earn me some..."
    ev_girl1 "我没有钱....我本来是想挣点外快的...."

# game/BKcity_events.rpy:1766
translate chinese_simplified city_gamble_ef9737fb:

    # "Tears run along her cheeks."
    "眼泪顺着她的脸颊流下。"

# game/BKcity_events.rpy:1768
translate chinese_simplified city_gamble_75088c84:

    # you "I'm sorry. But you have to pay up. Don't make me fetch the guard..."
    you "我很抱歉。但是你必须付钱。别让我去报警...."

# game/BKcity_events.rpy:1770
translate chinese_simplified city_gamble_491bbda7:

    # ev_girl1 "The guard? No, please! We can work this out!!!"
    ev_girl1 "报警？不，求你了！我们可以好好解决问题的！！"

# game/BKcity_events.rpy:1772
translate chinese_simplified city_gamble_24e6b6dd:

    # "She gives you a strange, devious look."
    "她咬了咬牙，下定了决心，随后向你抛出媚眼。"

# game/BKcity_events.rpy:1774
translate chinese_simplified city_gamble_c3e3c1f2:

    # ev_girl1 "I can make it up to you in another way..."
    ev_girl1 "我也有几分姿色，不如让人家肉偿给你...."

# game/BKcity_events.rpy:1784
translate chinese_simplified city_gamble_d2482226:

    # ev_girl1 "Oh, mister, you're being rough, aaha!!"
    ev_girl1 "哦，先生，你太粗鲁了，你要带我到哪去，啊嗯！！"

# game/BKcity_events.rpy:1786
translate chinese_simplified city_gamble_a46ec1aa:

    # you "You owe me a great deal, and I intend to collect, ha!"
    you "你欠我的太多了，我可得一分不少的收回来！趴好了！"

# game/BKcity_events.rpy:1788
translate chinese_simplified city_gamble_c2182a64:

    # "Pushing your dick inside her firm, slender body, you begin to pump in and out of her with rhythm."
    "你将肉棒对准了她的屁股，你隔着内裤不断的摩擦着她的小穴，她趴在桌上吐气如兰。"

# game/BKcity_events.rpy:1792
translate chinese_simplified city_gamble_0ec82c53:

    # ev_girl1 "Ooooh... I like it... Mister, your cock is huge, oh..."
    ev_girl1 "唔....好大....我从来没见过这么大的肉棒，哦...."

# game/BKcity_events.rpy:1794
translate chinese_simplified city_gamble_d89d6d72:

    # you "This is only the beginning..."
    you "这才只是开始...."

# game/BKcity_events.rpy:1796
translate chinese_simplified city_gamble_4736b5fc:

    # "Increasing your pace, you fuck her harder and harder."
    "你把她的内裤拽开，用力地挺动腰杆，肉棒穿过紧致的肉壁直达花心。"

# game/BKcity_events.rpy:1798
translate chinese_simplified city_gamble_1342b6c9:

    # "Her screams of passion fill the room as her juices drip down from her pussy."
    "她的淫叫声响彻房间，她的小穴变得泥泞不堪。"

# game/BKcity_events.rpy:1802
translate chinese_simplified city_gamble_d07cca1a:

    # ev_girl1 "You're ravaging me! I love it!"
    ev_girl1 "用力操我！好哥哥！人家的小穴好爽啊！"

# game/BKcity_events.rpy:1804
translate chinese_simplified city_gamble_4f8540f7:

    # you "Take this, you little slut! Uuuuaaaahhhhh!!!!"
    you "把我的精液都接好了，你这条欲求不满的母狗！唔啊哈哈哈！！！！"

# game/BKcity_events.rpy:1806
translate chinese_simplified city_gamble_03cd20eb:

    # ev_girl1 "HAAAAAAAAAAA!!!"
    ev_girl1 "HAAAAAAAAAAA!!!*翻白眼*"

# game/BKcity_events.rpy:1816
translate chinese_simplified city_gamble_be5e7738:

    # "She screams in delight as a powerful orgasm rocks her body back and forth."
    "她的身体伴随着一次又一次的高潮颤动不已，她兴奋地用手脚抱住你，双眼翻白，舌头外吐。"

# game/BKcity_events.rpy:1820
translate chinese_simplified city_gamble_cd6b38b6:

    # "You come loads inside of her, and she squeezes you dry with her tight hole."
    "你的肉棒在她的子宫口旋转剐蹭，她根本没经历过这种体验，一下子就泄了身，瘫软在地。你用精液灌满了她的小穴。"

# game/BKcity_events.rpy:1822
translate chinese_simplified city_gamble_a1e9c257:

    # ev_girl1 "Cum... So warm inside... Feels so good... Aaaah..."
    ev_girl1 "我要被玩坏了....我是一头只知道做爱的母猪~哼哧"

# game/BKcity_events.rpy:1830
translate chinese_simplified city_gamble_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:1837
translate chinese_simplified city_gamble_995a6198:

    # ev_girl1 "Time to pay up, now, mister!"
    ev_girl1 "现在该付钱了，先生！"

# game/BKcity_events.rpy:1839
translate chinese_simplified city_gamble_48a2c215:

    # you "Damn..."
    you "该死的...."

# game/BKcity_events.rpy:1853
translate chinese_simplified city_gamble_04964a88_1:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:1863
translate chinese_simplified city_gamble_cfb1a132:

    # you "Sorry, but I'm not one to waste time playing games."
    you "对不起，我没闲工夫陪你玩这种无聊的游戏。"

# game/BKcity_events.rpy:1865
translate chinese_simplified city_gamble_dba13d1b:

    # ev_girl1 "Really? That's too bad, I figured you were the playing type... Goodbye, then."
    ev_girl1 "真的吗？太可惜了，我还以为你是那种喜欢寻求刺激的人呢，要是你改主意了就回来找我。"

# game/BKcity_events.rpy:1880
translate chinese_simplified city_thief_b71bafed:

    # "As you pass a group of people near the [loc], you suddenly hear a scream."
    "当你经过[loc]附近的一群人时，你突然听到一声尖叫。"

# game/BKcity_events.rpy:1902
translate chinese_simplified city_thief_280c7553:

    # woman "Thief! A thief! Help me!"
    woman "小偷！有小偷！抓住他！"

# game/BKcity_events.rpy:1904
translate chinese_simplified city_thief_877ebc36:

    # "You see a sneaky cutpurse running down the street, holding the woman's belongings."
    "你看到一个慌张的小偷沿街狂奔，手上拿着一个包。"

# game/BKcity_events.rpy:1919
translate chinese_simplified city_thief_9ef2b7a3:

    # "You bolt after the thieving scum, quickly catching up thanks to your superior training."
    "你紧随其后，多亏了你平时的锻炼，你很快就追上了小偷。"

# game/BKcity_events.rpy:1921
translate chinese_simplified city_thief_7f383b19:

    # "Desperate to escape you, the thief starts climbing a building, heading towards the roof."
    "小偷不顾一切地想逃离你，跃上扶梯，向屋顶跑去。"

# game/BKcity_events.rpy:1923
translate chinese_simplified city_thief_addf0ba7:

    # you "Damn you!"
    you "该死！"

# game/BKcity_events.rpy:1925
translate chinese_simplified city_thief_27600d3e:

    # "Grabbing a rock from the ground, you expertly throw it as the wretch reaches for the top."
    "你从地上捡起一块石头，精准的砸中了小偷的后脑勺。"

# game/BKcity_events.rpy:1935
translate chinese_simplified city_thief_861e46bf:

    # "The thief falls down with a scream, and lays there, passed out. You recover the woman's purse."
    "小偷尖叫一声倒在地上，躺在那里，昏过去了。你拿回了那位女士的东西。"

# game/BKcity_events.rpy:1939
translate chinese_simplified city_thief_e8a36dec:

    # "You start running after the wicked thief, but the wretch is very fast."
    "你开始追赶那个小偷，但那个坏蛋跑得很快。"

# game/BKcity_events.rpy:1941
translate chinese_simplified city_thief_cee55968:

    # "You manage to keep up for a while, but you can feel your stamina running out."
    "你一路狂奔，小偷不知道从哪里突然掏出一条蕾丝内裤扔到了你的脸上。"

# game/BKcity_events.rpy:1943
translate chinese_simplified city_thief_48a2c215:

    # you "Damn..."
    you "该死的...."

# game/BKcity_events.rpy:1945
translate chinese_simplified city_thief_19de95e8:

    # "You finally give up on the chase, completely out of breath."
    "就在你被吸引注意力的时候，小偷已经消失不见。"

# game/BKcity_events.rpy:1955
translate chinese_simplified city_thief_9bb30149:

    # "Trying to keep the thief within your view, you start chanting the words of a powerful weakening spell."
    "为了不让小偷逃出你的视线范围，你开始念诵一个强大的束缚咒。"

# game/BKcity_events.rpy:1961
translate chinese_simplified city_thief_040f5609:

    # "You can see the thief running desperately down the street, then suddenly falling flat on the pavement, legs shaking uncontrollably."
    "小偷拼命地沿着街道跑，然后突然倒在人行道上，双腿不由自主地颤抖着。"

# game/BKcity_events.rpy:1963
translate chinese_simplified city_thief_a47c8b71:

    # "The wretch still tries to flee, creeping forward at an agonizing pace."
    "那个可怜虫还想逃跑，挣扎着向前爬去。"

# game/BKcity_events.rpy:1965
translate chinese_simplified city_thief_797a1b58:

    # "You leisurely walk up to the thief, whistling a popular bard song."
    "你不紧不慢地走向小偷，哼着家乡的小调。"

# game/BKcity_events.rpy:1967
translate chinese_simplified city_thief_baf8d032:

    # "The thief sees you and begs for mercy as you loom close."
    "小偷看到你一步一步逼近，向你乞求怜悯。"

# game/BKcity_events.rpy:1971
translate chinese_simplified city_thief_743a7ae0:

    # "Still whistling, you answer with a powerful blow to the the head. The skull gives in with a nasty crack."
    "你对小偷的求饶视而不见，对着太阳穴来了一拳。小偷应声倒地。"

# game/BKcity_events.rpy:1973
translate chinese_simplified city_thief_68537d33:

    # "You then retrieve the woman's belongings."
    "然后，你取回了那个女人的物品。"

# game/BKcity_events.rpy:1977
translate chinese_simplified city_thief_667e9dcc:

    # "You reach down and recover the woman's purse from the ground. You then snap your fingers, and the thug's limbs stop shaking."
    "你伸手从地上捡起那个女人的钱包。然后你打了一个响指，小偷的四肢停止颤抖。"

# game/BKcity_events.rpy:1979
translate chinese_simplified city_thief_4ce8bb4b:

    # "Swearing, the rascal jumps up and hightails it, as if a demon was giving chase."
    "小偷骂骂咧咧地站起身来，拔腿就跑，好像有个魔鬼在身后追赶。"

# game/BKcity_events.rpy:1983
translate chinese_simplified city_thief_00ef219e:

    # "You try to keep the thief in focus, but the constant leaping and dodging makes it too hard to aim a spell."
    "你试图让小偷集中注意力，但不断的跳跃和闪避让你很难瞄准锁定。"

# game/BKcity_events.rpy:1987
translate chinese_simplified city_thief_34c6d5bd:

    # "You curse as the thief finally disappears from your sight, hidden by a passing carriage."
    "你咒骂着，小偷还是从你的视线中消失了，爬上了一辆路过的马车扬长而去。"

# game/BKcity_events.rpy:1993
translate chinese_simplified city_thief_7a5c01cf:

    # you "This is none of my concern, really."
    you "这关我什么事，各人自扫门前雪，休管他人瓦上霜。"

# game/BKcity_events.rpy:1995
translate chinese_simplified city_thief_2feec416:

    # "You watch with indifference as the thief expertly makes way through the crowd and vanishes from your sight."
    "你冷漠地看着小偷熟练地穿过人群，从你的视线中消失。"

# game/BKcity_events.rpy:2002
translate chinese_simplified city_thief_071d6883:

    # woman "Oh thank you, thank you!"
    woman "哦，谢谢，谢谢！"

# game/BKcity_events.rpy:2004
translate chinese_simplified city_thief_75863963:

    # "She smiles gratefully at you, and hands you a pouch of gold for your trouble."
    "她握住你的手给了你一个香吻，递给你一袋金币，表示对你的谢意。"

# game/BKcity_events.rpy:2008
translate chinese_simplified city_thief_f61cb9b5:

    # you "Thank you, but this isn't necessary..."
    you "谢谢，但这没有必要...."

# game/BKcity_events.rpy:2010
translate chinese_simplified city_thief_ba41d60b:

    # woman "Please, I insist."
    woman "请收下吧，不然我的良心会不安的。"

# game/BKcity_events.rpy:2014
translate chinese_simplified city_thief_7596837c:

    # you "Nice!"
    you "既然夫人坚持，那在下就却之不恭了！"

# game/BKcity_events.rpy:2024
translate chinese_simplified city_thief_b1b89f60:

    # "You have earned %(gain)d gold."
    "你获得了 %(gain)d 枚金币。"

# game/BKcity_events.rpy:2033
translate chinese_simplified city_thief_a2a181ca:

    # "The thief disappears into a side street, heading for one of the gates leading to the slums."
    "小偷消失在一条小巷里，朝着通往贫民窟的路走去。"

# game/BKcity_events.rpy:2039
translate chinese_simplified city_thief_981f219b:

    # "You shrug and step back towards the [loc], the sound of the woman's distraught cries echoing in your ears."
    "你耸耸肩，向[loc]方向退去，耳边回荡着女人悲痛欲绝的哭喊声。"

# game/BKcity_events.rpy:2050
translate chinese_simplified city_wrestle_72ab9f57:

    # "You meet a group of sellswords sitting around a few tables, in a dodgy corner of the [loc]."
    "你遇到一群雇佣兵围坐在几张桌子旁，在一个破旧的角落里。"

# game/BKcity_events.rpy:2068
translate chinese_simplified city_wrestle_c22c192b:

    # warrior "Hey, you! Think you're strong? What do you say to a bout of arm wrestling?"
    warrior "嘿，新来的！想不想免费喝杯啤酒？只要你掰手腕赢过我我就请你喝一杯！"

# game/BKcity_events.rpy:2070
translate chinese_simplified city_wrestle_5a52de6c:

    # warrior "I wager a fistful of gold I could beat you fair and square right here, right now."
    warrior "就赌我手上的金币，我敢打赌你坚持不住十秒就会被我击败。"

# game/BKcity_events.rpy:2074
translate chinese_simplified city_wrestle_1bd940ab:

    # "A crowd of mercenaries and idlers watches you with interest as they await for your response."
    "一群雇佣兵和游手好闲的酒客饶有兴趣地看着你，等待你的回应。"

# game/BKcity_events.rpy:2086
translate chinese_simplified city_wrestle_74709d0e:

    # you "Let's see about that!"
    you "那就让我试试你有几斤几两！"

# game/BKcity_events.rpy:2088
translate chinese_simplified city_wrestle_f32dd71d:

    # "The crowd cheers and jeers as you sit down in front of the seasoned veteran."
    "你来到这个老兵对面坐下，大伙都围了过来看热闹。"

# game/BKcity_events.rpy:2090
translate chinese_simplified city_wrestle_1dab431f:

    # "One of the men fires his gun in the air to give the starting signal."
    "你们约定以叉子落地为号，酒保拿出一把叉子举在空中。"

# game/BKcity_events.rpy:2096
translate chinese_simplified city_wrestle_cc787f6f:

    # "Your muscles tighten up as you prepare to resist your opponent."
    "你的肌肉紧绷，青筋外露，面色通红。"

# game/BKcity_events.rpy:2105
translate chinese_simplified city_wrestle_f987c652:

    # "For a while, it looks like you are locked down and losing inch by inch to your adversary."
    "你们二人僵持不下，你的手腕一点一点地被他扳了过去。"

# game/BKcity_events.rpy:2107
translate chinese_simplified city_wrestle_2d814530:

    # "However, you are simply waiting for him to exhaust his energy."
    "然而，这不过是你的诱敌之策。"

# game/BKcity_events.rpy:2109
translate chinese_simplified city_wrestle_82789ed4:

    # "Sweat is beading off his face as he tries desperately to push through the last few inches."
    "当他使出吃奶的力气准备赢得这场比赛时。"

# game/BKcity_events.rpy:2111
translate chinese_simplified city_wrestle_0ff91404:

    # "You suddenly fight him off with all your strength, however, and slam his hand down on the table so strongly that the drinks spill."
    "你突然发力把他推开，他的手被你重重的拍在桌上，酒水都洒了出来。"

# game/BKcity_events.rpy:2119
translate chinese_simplified city_wrestle_50310af9:

    # warrior "Aaaargh!!!"
    warrior "该死!!!"

# game/BKcity_events.rpy:2123
translate chinese_simplified city_wrestle_f7ba7de8:

    # "Both of your arms are locked in a tight grip. For a while, it's hard to tell who's winning or losing."
    "你们的手臂紧紧地搭在一起。几十秒过去了，很难说谁赢谁输。"

# game/BKcity_events.rpy:2125
translate chinese_simplified city_wrestle_9002ecf7:

    # "However, you can feel your stamina slowly draining out. Your arm starts to shake. Your opponent is grinning now."
    "然而，你可以感觉到你的力气在慢慢耗尽。你的手臂开始颤抖。你的对手却十分轻松。"

# game/BKcity_events.rpy:2127
translate chinese_simplified city_wrestle_d95d8254:

    # warrior "Feeling a little tired, greenhorn?"
    warrior "坚持不住了，新手？"

# game/BKcity_events.rpy:2133
translate chinese_simplified city_wrestle_bce48510:

    # "With a final thump, the mercenary pushes your hand down onto the table. You are defeated."
    "最后一击，他把你的手推到桌子上。你输了。"

# game/BKcity_events.rpy:2135
translate chinese_simplified city_wrestle_5ee7ff9d:

    # you "Shit..."
    you "狗屎...."

# game/BKcity_events.rpy:2141
translate chinese_simplified city_wrestle_3e8a4b55:

    # you "Oh, a fight with a monkey. I hear they're strong, if a little intellectually-challenged."
    you "拜托，和一只猩猩打架。我听说他们很强壮，就是可惜脑子不太好使。"

# game/BKcity_events.rpy:2143
translate chinese_simplified city_wrestle_b2bea753:

    # "The crowd erupts into loud laughter at your jest. The warrior's eyes narrow."
    "听了你的笑话，人群爆发出一阵大笑。战士的眼睛眯了起来。"

# game/BKcity_events.rpy:2145
translate chinese_simplified city_wrestle_141f5b86:

    # warrior "Careful now, boy..."
    warrior "当心了，小屁孩...."

# game/BKcity_events.rpy:2147
translate chinese_simplified city_wrestle_cd163e53:

    # "Sitting down in front of him and getting ready, you give him a wink."
    "你在他面前坐下，冲他眨眼做鬼脸。"

# game/BKcity_events.rpy:2155
translate chinese_simplified city_wrestle_0fc6b13e:

    # "As the contest begins, you keep poking fun at your opponent."
    "比赛一开始，你就不停地取笑你的对手。"

# game/BKcity_events.rpy:2157
translate chinese_simplified city_wrestle_65ee0a62:

    # you "Hey, you've got a chunk of pork on your face. Ah, sorry, that's your nose."
    you "嘿，你脸上有块猪肉。啊，对不起，那是你的鼻子。"

# game/BKcity_events.rpy:2159
translate chinese_simplified city_wrestle_85598211:

    # warrior "Grrr..."
    warrior "咆哮声..."

# game/BKcity_events.rpy:2161
translate chinese_simplified city_wrestle_f9f43a26:

    # you "So, ever rubbed your mother's cunt with your ears?"
    you "那么，你有没有用耳朵摩擦过你妈妈的屁股？"

# game/BKcity_events.rpy:2163
translate chinese_simplified city_wrestle_a87365e5:

    # warrior "What??? No!!!"
    warrior "什么？？？那怎么可能！！！"

# game/BKcity_events.rpy:2165
translate chinese_simplified city_wrestle_129cdef2:

    # you "I see, you were born with a helmet then..."
    you "我明白了，你出生时就戴着头盔，所以耳朵碰不到...."

# game/BKcity_events.rpy:2169
translate chinese_simplified city_wrestle_36a2ce18:

    # "The mercenaries laugh louder with each of your jokes, and taunt the poor bastard. He seems to be completely losing his concentration."
    "人群中的笑声越来越大，他们嘲笑起那个可怜的家伙。他的注意力似乎不太集中。"

# game/BKcity_events.rpy:2171
translate chinese_simplified city_wrestle_dc656e1f:

    # "That's when you decide to make your push. Piling up against his arm, you send it flying into the table."
    "你抓住了他分神的瞬间。你猛然发力，一把将他的手腕扳倒在桌上。"

# game/BKcity_events.rpy:2177
translate chinese_simplified city_wrestle_0c5f398c:

    # you "Touché!"
    you "你输了！！"

# game/BKcity_events.rpy:2179
translate chinese_simplified city_wrestle_19079579:

    # warrior "What?? How could I lose!"
    warrior "什么？？我怎么会输给你！"

# game/BKcity_events.rpy:2183
translate chinese_simplified city_wrestle_b9943a91:

    # "Even as you try to get into his head, his superior strength threatens to overwhelm you. Beads of sweat start running into your eyes. This is going poorly."
    "即使你试图干扰他的思绪，他的超强力量也有压倒你的风险。汗珠开始流进你的眼睛。事情进展得很糟糕。"

# game/BKcity_events.rpy:2185
translate chinese_simplified city_wrestle_ce6831c7:

    # you "You know what? I'm... I'm stronger than you! You might as well give up."
    you "你知道吗？我....我比你强壮！我劝你还是放弃算了。"

# game/BKcity_events.rpy:2187
translate chinese_simplified city_wrestle_422a594d:

    # warrior "Oh yeah? I have another idea. How about I break your arm?"
    warrior "哦，是吗？我有另一个主意。要是你输了我就打断你的胳膊怎么样？"

# game/BKcity_events.rpy:2193
translate chinese_simplified city_wrestle_0ac056e2:

    # "With a thunderous grunt, the warrior slams your arm down on the table. It hurts like hell."
    "随着一声雷鸣般的怒吼，战士把你的手臂重重地摔在桌子上。疼得要命。"

# game/BKcity_events.rpy:2195
translate chinese_simplified city_wrestle_8a556914:

    # you "Aaaargh!!!"
    you "该死!!!"

# game/BKcity_events.rpy:2197
translate chinese_simplified city_wrestle_f8910b6b:

    # warrior "Hahahahaha!!!"
    warrior "哈哈哈哈！！！"

# game/BKcity_events.rpy:2203
translate chinese_simplified city_wrestle_fa530948:

    # you "I can't, I have... stuff to do."
    you "别打断我的胳膊，我上有老下有小，我还要养家糊口。"

# game/BKcity_events.rpy:2205
translate chinese_simplified city_wrestle_fb565e36:

    # warrior "I see. Then scram, weakling."
    warrior "随你怎么说好了。滚吧，细狗。"

# game/BKcity_events.rpy:2209
translate chinese_simplified city_wrestle_a6a9279d:

    # "The mercenaries scoff and jeer, calling you a coward and all sorts of unpleasant names as you pitifully make your way back to [dis]."
    "雇佣兵们嘲笑你，骂你是懦夫，还用各种难听的话辱骂你，而你则可怜兮兮地返回[dis]。"

# game/BKcity_events.rpy:2225
translate chinese_simplified city_wrestle_5a884e0e:

    # "The crowd roars and erupts in applause as the warrior gives you a dejected look."
    "老兵露出愤怒的眼神，人群中爆发出雷鸣般的掌声。"

# game/BKcity_events.rpy:2227
translate chinese_simplified city_wrestle_a48d4343:

    # you "Time to pay up."
    you "按照约定，金币是我的了。"

# game/BKcity_events.rpy:2229
translate chinese_simplified city_wrestle_0043fb82:

    # warrior "Fine, here's your gold... *frown*"
    warrior "好吧，这是你的金币....*皱眉*"

# game/BKcity_events.rpy:2239
translate chinese_simplified city_wrestle_b1b89f60:

    # "You have earned %(gain)d gold."
    "你获得了 %(gain)d 枚金币。"

# game/BKcity_events.rpy:2245
translate chinese_simplified city_wrestle_bc99f824:

    # warrior "I want my money now, wimp."
    warrior "把钱交出来，胆小鬼。"

# game/BKcity_events.rpy:2261
translate chinese_simplified city_wrestle_04964a88:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:2274
translate chinese_simplified city_cat_8af25751:

    # "While walking around the [loc], minding your own business, you hear a young woman call you."
    "当你在[loc]附近闲逛，做着自己的事时，你听到一个年轻女子叫你。"

# game/BKcity_events.rpy:2294
translate chinese_simplified city_cat_ea6e8d15:

    # ev_girl3 "Nyaa! Mister, help me!"
    ev_girl3 "先生，请帮帮我！"

# game/BKcity_events.rpy:2296
translate chinese_simplified city_cat_c25ad0ac:

    # you "What is it? ...and what's with the strange ears??"
    you "这是猫娘？....她的耳朵比我想的还要再可爱一点。"

# game/BKcity_events.rpy:2300
translate chinese_simplified city_cat_21c2f2e0:

    # ev_girl3 "It's Yuna! My best friend! I can't find her anywhere!"
    ev_girl3 "尤娜！我最好的朋友！我找不到她了！"

# game/BKcity_events.rpy:2302
translate chinese_simplified city_cat_aa8879a5:

    # ev_girl3 "She's been missing for days! She told me she was going out for a while, but..."
    ev_girl3 "她已经失踪好几天了！她告诉我她要出去一段时间，但是...."

# game/BKcity_events.rpy:2304
translate chinese_simplified city_cat_dda822f5:

    # you "Do you have any idea where she went? Did you talk to any of her other friends?"
    you "你知道她去哪儿了吗？你有问过其它认识尤娜的人吗？"

# game/BKcity_events.rpy:2306
translate chinese_simplified city_cat_eea52340:

    # ev_girl3 "Well... She's been fooling around with those cool cats lately..."
    ev_girl3 "嗯....她最近一直在和那些很酷的猫鬼混...."

# game/BKcity_events.rpy:2308
translate chinese_simplified city_cat_fbbe8633:

    # you "Cool cats?"
    you "很酷的猫？"

# game/BKcity_events.rpy:2310
translate chinese_simplified city_cat_65fd96dc:

    # ev_girl3 "There's Steve, the handsome dark one... Garfy, the fat, funny one... And the siamese twins..."
    ev_girl3 "有史蒂夫，那只英俊的黑猫....加菲，那个幽默的胖子....还有暹罗双胞胎...."

# game/BKcity_events.rpy:2312
translate chinese_simplified city_cat_2f7024d1:

    # you "Siamese twins?? That's rare... I don't remember ever meeting any of those guys..."
    you "暹罗双胞胎吗？？这可不常见....我不记得见过这些人....或者猫"

# game/BKcity_events.rpy:2316
translate chinese_simplified city_cat_63521e2b:

    # ev_girl3 "Oh, mister, what am I gonna do... What if she gets eaten?"
    ev_girl3 "噢，先生，我该怎么办....如果她被吃掉了怎么办？"

# game/BKcity_events.rpy:2318
translate chinese_simplified city_cat_7fcfbc3e:

    # you "Eaten? Calm down, I don't think anyone will eat your friend..."
    you "被吃？冷静点，我不认为有人会吃了你的朋友....*小声*不过她如果和我在一起确实会被吃干抹净"

# game/BKcity_events.rpy:2320
translate chinese_simplified city_cat_cbb5c79e:

    # ev_girl3 "But some people like eating cats!!! There's Alf, this dodgy dwarf, he..."
    ev_girl3 "但是有些人就喜欢吃猫！！阿尔夫，那个狡猾的小矮人，他...."

# game/BKcity_events.rpy:2322
translate chinese_simplified city_cat_c979b16a:

    # you "Cats??? Wait a minute... Your friend is a cat?"
    you "猫？？？等一下....你的朋友是一只猫？"

# game/BKcity_events.rpy:2324
translate chinese_simplified city_cat_9d607a1e:

    # ev_girl3 "Yes, of course! She's my best friend! We have such great conversations, you know, about life, love, nyaa... We share the same world view..."
    ev_girl3 "是的，当然！她是我最好的朋友！我们聊得很开心，你知道，关于生活、爱情、喵....我们有相同的世界观...."

# game/BKcity_events.rpy:2326
translate chinese_simplified city_cat_537ab76c:

    # you "Okaay... Right..."
    you "好....好吧...."

# game/BKcity_events.rpy:2337
translate chinese_simplified city_cat_c3b1c980:

    # you "Very well, then. I'll help you find her."
    you "很好，别担心。我会帮你找到她的。"

# game/BKcity_events.rpy:2339
translate chinese_simplified city_cat_d801513d:

    # ev_girl3 "Oh, mister, thank you, nyaaa! I think she went this way..."
    ev_girl3 "哦，先生，谢谢你，*嗅嗅*喵！我想她走了这条路...."

# game/BKcity_events.rpy:2345
translate chinese_simplified city_cat_11aa6670:

    # "You spend an hour searching high and low for the kitty."
    "你花了一个小时到处寻找尤娜。"

# game/BKcity_events.rpy:2354
translate chinese_simplified city_cat_7b1d55c6:

    # "Finally, you see the cat, lazily curling into a ball on the edge of a rooftop."
    "最后，你看到一只猫在屋顶上晒太阳，慵懒地蜷缩成一个肉球。"

# game/BKcity_events.rpy:2356
translate chinese_simplified city_cat_bb49702d:

    # ev_girl3 "Oh, it's her!"
    ev_girl3 "哦，是她！"

# game/BKcity_events.rpy:2365
translate chinese_simplified city_cat_fc9aa7de:

    # you "Hey, Yuna! Come down here you dumb critter!"
    you "嘿，杰克！过来，你这只小笨蛋！"

# game/BKcity_events.rpy:2367
translate chinese_simplified city_cat_aa939283:

    # "Your yelling startles the cat, who hisses at you before jumping from the roof into a side street and running off."
    "你的叫声把猫吓了一跳，它对你发出嘶嘶声，然后从屋顶跳到另一边消失不见。"

# game/BKcity_events.rpy:2371
translate chinese_simplified city_cat_99f689a2:

    # ev_girl3 "Nooo! Come back, Yuna, come baaaaack! Nyaaa!"
    ev_girl3 "不！回来吧，尤娜，回来啊！喵呜！"

# game/BKcity_events.rpy:2373
translate chinese_simplified city_cat_777b91ba:

    # "The strange girl runs after her pet, leaving you standing there with a dumbfounded look on your face."
    "那个奇怪的猫娘追着她的猫咪远去，留下你站在原地发呆。"

# game/BKcity_events.rpy:2383
translate chinese_simplified city_cat_9e0ced25:

    # you "Come here, kitty kitty... Yuna, please come here, there's nothing to be afraid of..."
    you "过来，小猫咪....尤娜，请过来，没什么好怕的...."

# game/BKcity_events.rpy:2387
translate chinese_simplified city_cat_4cff14e4:

    # "The kitty gives you a lascivious look and yawns. She lazily stretches her limbs, then jumps down at your feet with effortless grace."
    "猫咪给了你一个鄙视的眼神，打了个呵欠。懒洋洋地舒展着四肢，然后毫不费力地优雅地跳到你的脚边。"

# game/BKcity_events.rpy:2389
translate chinese_simplified city_cat_c969d1cf:

    # ev_girl3 "Yuna!!! You're here! I'm so happy!!!"
    ev_girl3 "尤娜！！！你在这儿！我总算找到你了！！！"

# game/BKcity_events.rpy:2395
translate chinese_simplified city_cat_71f73778:

    # "Doing your best impersonation of a cat's meow, you call out to the cat in her own language."
    "你试着模仿猫的叫声，你用猫的母语对它大喊大叫。"

# game/BKcity_events.rpy:2399
translate chinese_simplified city_cat_575dc3c9:

    # you "Meow!!!"
    you "喵！！喵呜！"

# game/BKcity_events.rpy:2403
translate chinese_simplified city_cat_8db54dc5:

    # "Yuna raises her head and looks at you with great curiosity."
    "尤娜抬起头，好奇地看着你。"

# game/BKcity_events.rpy:2405
translate chinese_simplified city_cat_b2cc1a33:

    # "She leaps down into your ready arms. You give her a pat on the head, and she starts purring. You gently put her on the ground."
    "她跃入你的怀抱。你拍拍她的头，她咕噜咕噜地叫。你轻轻地把她放在地上。"

# game/BKcity_events.rpy:2407
translate chinese_simplified city_cat_6b6728e4:

    # ev_girl3 "Amazing! You are a cat lover too, I can tell! Nyaa!"
    ev_girl3 "太神奇了！看得出来，你也是个爱猫人士！喵！"

# game/BKcity_events.rpy:2420
translate chinese_simplified city_cat_70e1a4d7:

    # you "I could locate your 'friend' using a spell. That might work."
    you "我可以用咒语找到你的“朋友”。这可能会奏效。"

# game/BKcity_events.rpy:2424
translate chinese_simplified city_cat_9df3ba9f:

    # ev_girl3 "Really? That's so cool! Thank you mister, nyaaah!"
    ev_girl3 "真的吗？那是太酷了！谢谢你，喵喵！"

# game/BKcity_events.rpy:2426
translate chinese_simplified city_cat_b61348fb:

    # "She looks at you expectantly."
    "她期待地看着你。"

# game/BKcity_events.rpy:2428
translate chinese_simplified city_cat_332f1f8f:

    # you "Hum, hum. First, let me gather some personal item of her. A collar, perhaps?"
    you "哼，哼。首先，让我收集一些她的私人物品作为媒介。也许，一个纽扣？"

# game/BKcity_events.rpy:2430
translate chinese_simplified city_cat_fba7a045:

    # ev_girl3 "Haha, mister, that's kinky! We don't do this kind of stuff. But I can find you some of her hair, I guess."
    ev_girl3 "哈哈，先生，真怪！我们不做这种事。但我想我能找到她的一些毛发。"

# game/BKcity_events.rpy:2432
translate chinese_simplified city_cat_a8912ef4:

    # "Holding the lock of hair firmly, you start chanting."
    "紧紧抓住那绺毛发，你开始吟唱。"

# game/BKcity_events.rpy:2440
translate chinese_simplified city_cat_4f7a97bf:

    # you "Come hereforth, you furry beast! Come to daddy!"
    you "遗失之物，指名所往！猫咪喵咪呜！"

# game/BKcity_events.rpy:2444
translate chinese_simplified city_cat_a0517fa6:

    # ev_girl3 "What... What's this sound?"
    ev_girl3 "什么....这是什么动静？"

# game/BKcity_events.rpy:2446
translate chinese_simplified city_cat_25f7a449:

    # "You are startled to hear a barking noise in the distance, closing in fast."
    "你听到远处传来猛烈的狗吠声，而且正在迅速逼近。"

# game/BKcity_events.rpy:2448
translate chinese_simplified city_cat_e770ceee:

    # ev_girl3 "Oh no... It's..."
    ev_girl3 "哦，不....这是...."

# game/BKcity_events.rpy:2452
translate chinese_simplified city_cat_8ad8d44f:

    # ev_girl3 "DOGS!!!"
    ev_girl3 "一大堆狗！！！"

# game/BKcity_events.rpy:2454
translate chinese_simplified city_cat_335bf053:

    # "A pack of flea-ridden street dogs appears down the street, charging in your direction."
    "一群满身跳蚤的流浪狗出现在街上，向你冲过来。"

# game/BKcity_events.rpy:2458
translate chinese_simplified city_cat_8e328a26:

    # ev_girl3 "Hiiiii!!! I hate dogs, nyaaa!"
    ev_girl3 "Hiiiii！！！我讨厌狗，喵！"

# game/BKcity_events.rpy:2464
translate chinese_simplified city_cat_7b5d8682:

    # "The girl jumps and runs away with feline agility. You ponder whether or not to follow her, but your reflexion is cut short by dozens of paws bumping you to the ground as the dogs proceeds to lick you mercilessly."
    "女孩像猫一样敏捷地逃开了。你在考虑是否要跟着她，但你还没来得及细想，狗狗们就把你撞倒在地上，开始尽情地舔你。"

# game/BKcity_events.rpy:2468
translate chinese_simplified city_cat_5ae7d7af:

    # you "Haaa! No!!! It tickles!!!"
    you "Haaa！不！！！好痒！！！"

# game/BKcity_events.rpy:2472
translate chinese_simplified city_cat_8bae13a6:

    # "You have got fleas."
    "你被弄得满身都是跳蚤。"

# game/BKcity_events.rpy:2478
translate chinese_simplified city_cat_1d82668c:

    # you "I summon thee, ye little feline furball! Thy friend misses thee..."
    you "心之所向，猫之所往...."

# game/BKcity_events.rpy:2482
translate chinese_simplified city_cat_63a801bd:

    # "Long minutes pass by. The girl gives you an inquisitive look."
    "过了一段时间。那个女孩好奇地看了你一眼。"

# game/BKcity_events.rpy:2484
translate chinese_simplified city_cat_49eb5da7:

    # ev_girl3 "Did it work? It's like nothing happened..."
    ev_girl3 "咒语起作用了吗？好像什么都没发生...."

# game/BKcity_events.rpy:2486
translate chinese_simplified city_cat_82ed3050:

    # you "I'm sorry. I think..."
    you "我很抱歉。我以为...."

# game/BKcity_events.rpy:2490
translate chinese_simplified city_cat_617eadfd:

    # ev_girl3 "..."
    ev_girl3 "..."

# game/BKcity_events.rpy:2494
translate chinese_simplified city_cat_91fdb66f:

    # ev_girl3 "Yuna!!!"
    ev_girl3 "尤娜！！！"

# game/BKcity_events.rpy:2507
translate chinese_simplified city_cat_fd86ec75:

    # "A cute, slender cat is lazily walking towards you."
    "一只可爱的小猫懒洋洋地朝你走来。"

# game/BKcity_events.rpy:2513
translate chinese_simplified city_cat_4643c4be:

    # you "Nine tails, nine lives, I summon thee, Neko spirit... Bring me thy kin, do us a favor and we will repay thee..."
    you "九尾，九命，我召唤你，猫猫精灵....把你的亲人带来，帮我们个忙，我们会报答你的...."

# game/BKcity_events.rpy:2517
translate chinese_simplified city_cat_852da9ef:

    # ev_girl3 "You're glowing! Your scent is strange... Musky..."
    ev_girl3 "你容光焕发！你的气味很奇怪....麝香的...."

# game/BKcity_events.rpy:2536
translate chinese_simplified city_cat_7eea6700:

    # "As if appearing from thin air, the courtyard now fills with cats."
    "仿佛从天而降，院子里现在到处都是猫。"

# game/BKcity_events.rpy:2538
translate chinese_simplified city_cat_9d58bf54:

    # ev_girl3 "Amazing! Coral! Mr. Kitty! Steve... The siamese twins!"
    ev_girl3 "太神奇了！珊瑚！HelloKitty先生！史蒂夫....暹罗双胞胎！"

# game/BKcity_events.rpy:2540
translate chinese_simplified city_cat_2b93463b:

    # "A herd of cats has answered your call. They gather at your feet and start purring, rubbing themselves against your legs."
    "一群猫响应了你的召唤。它们聚集在你的脚边，喵喵地叫着，在你的腿边蹭来蹭去。"

# game/BKcity_events.rpy:2544
translate chinese_simplified city_cat_91fdb66f_1:

    # ev_girl3 "Yuna!!!"
    ev_girl3 "尤娜！！！"

# game/BKcity_events.rpy:2548
translate chinese_simplified city_cat_aeeb291c:

    # "A cute, regal-looking cat come out of a bush and walks leisurely towards you."
    "一只可爱的、看起来充满威严的猫从灌木丛中走出来，悠闲地向你走来。"

# game/BKcity_events.rpy:2550
translate chinese_simplified city_cat_3416a6ed:

    # ev_girl3 "She's here! Oh, mister, you're a genius!"
    ev_girl3 "她在这里！噢，先生，你真是个天才！"

# game/BKcity_events.rpy:2563
translate chinese_simplified city_cat_0c967b4d:

    # you "Ahem, I don't have time to look for your pet. Anyway, I'm sure she's fine."
    you "我没有时间去找你的宠物。她不会有事的，你就在家等她回来吧。"

# game/BKcity_events.rpy:2565
translate chinese_simplified city_cat_e6a510b8:

    # ev_girl3 "No, waaait! You must help me!"
    ev_girl3 "不，等等！求你帮帮我！"

# game/BKcity_events.rpy:2567
translate chinese_simplified city_cat_d18dd64e:

    # "Ignoring her, you hasten to walk away from the deranged girl."
    "你懒得理会她，离开了那个急得发疯的女孩。"

# game/BKcity_events.rpy:2580
translate chinese_simplified city_cat_bc08d581:

    # "Yuna rubs herself against the girl's legs and starts purring with a noise rivalling a snoring dragon."
    "尤娜在女孩的腿上蹭来蹭去，咕噜咕噜地叫起来，就像一只打呼噜的龙。"

# game/BKcity_events.rpy:2584
translate chinese_simplified city_cat_e4f211b5:

    # ev_girl3 "Oh, Yuna, you silly whore! I was here worried out of my mind, and all the while you were out there fooling around with male cats!"
    ev_girl3 "哦，尤娜，你这个偷腥猫！我在这里担心得发疯，而你一直在外面和公猫鬼混！"

# game/BKcity_events.rpy:2586
translate chinese_simplified city_cat_50abf0f8:

    # ev_girl3 "Well, at least you had fun..."
    ev_girl3 "至少你玩得很开心"

# game/BKcity_events.rpy:2591
translate chinese_simplified city_cat_c658bc12:

    # ev_girl3 "Thanks, mister, I was so worried... It was lucky you were here to help!"
    ev_girl3 "谢谢你，先生，我很担心....你能来帮忙真是太幸运了！"

# game/BKcity_events.rpy:2593
translate chinese_simplified city_cat_a266484b:

    # ev_girl3 "Here, please have this. I've had this forever and I don't use it."
    ev_girl3 "给，请拿着这个。这个我已经用了很久了，现在它是你的了。"

# game/BKcity_events.rpy:2605
translate chinese_simplified city_cat_3d687d5f:

    # bk_error "Couldn't generate Item for rank [_rank]"
    bk_error "无法生成[_rank]品质的道具"

# game/BKcity_events.rpy:2611
translate chinese_simplified city_cat_1a608fa9:

    # "You walk the girl and her cat back to their place."
    "你跟着女孩和她的猫来到他们的住处。"

# game/BKcity_events.rpy:2613
translate chinese_simplified city_cat_af7a0f21:

    # ev_girl3 "Thank you, mister, you were amazing."
    ev_girl3 "谢谢你，先生，你太棒了。"

# game/BKcity_events.rpy:2617
translate chinese_simplified city_cat_6f50c6ec:

    # ev_girl3 "Yuna would also like to thank you..."
    ev_girl3 "尤娜也要感谢你...."

# game/BKcity_events.rpy:2619
translate chinese_simplified city_cat_6cf2b05f:

    # you "Yuna? Err, I don't think it's necessary..."
    you "尤娜？呃，我觉得没有必要...."

# game/BKcity_events.rpy:2621
translate chinese_simplified city_cat_1264adb3:

    # ev_girl3 "It's very necessary! You won't be disappointed, believe me... *wink*"
    ev_girl3 "这是非常必要的！相信我，你不会失望的....*眨眼*"

# game/BKcity_events.rpy:2623
translate chinese_simplified city_cat_95cc9628:

    # "Turning to look at the cat, you gasp in amazement."
    "你转过身来看着那只猫，她令你大吃一惊。"

# game/BKcity_events.rpy:2637
translate chinese_simplified city_cat_e01181ea:

    # yuna "Hello, mister!"
    yuna "你好，先生！"

# game/BKcity_events.rpy:2639
translate chinese_simplified city_cat_67d4bb67:

    # "A very human, very sexy, and very naked Yuna is looking at you with hungry eyes."
    "一个很有人情味，很性感，很赤裸的猫娘正用饥渴的眼神看着你。"

# game/BKcity_events.rpy:2643
translate chinese_simplified city_cat_2d912c92:

    # yuna "Yuna is in heat... It burns... Mister, won't you help Yuna? Nyaah..."
    yuna "尤娜发情了....它欲火焚身....先生，你不帮帮尤娜吗？喵...."

# game/BKcity_events.rpy:2645
translate chinese_simplified city_cat_84974854:

    # yuna "Yuna has been with many cats, but now, Yuna wants a human dick... Yuna is dripping down there, just to think about a big, fat human cock..."
    yuna "尤娜一直和猫咪在一起玩耍，但是现在，尤娜想要和人类的鸡巴一起玩耍....尤娜的那里一直在滴水，它好像很想要你的人类大鸡巴...."

# game/BKcity_events.rpy:2647
translate chinese_simplified city_cat_52d8142e:

    # yuna "Mister, won't you help a poor lonely pussy?"
    yuna "先生，你不愿意帮助一个可怜的孤独的猫咪吗？"

# game/BKcity_events.rpy:2649
translate chinese_simplified city_cat_eb44d7e6:

    # "You do not waste time giving her an answer. Soon, the air is filled with the wet sound of your love-making."
    "你没有浪费时间思考，直接脱下了衣服。很快，空气中就充满了淫靡的声音和气味。"

# game/BKcity_events.rpy:2653
translate chinese_simplified city_cat_390b9719:

    # yuna "Yuna is so horny... Yuna needed a cock so bad... Nyaaaaah!"
    yuna "尤娜是淫荡的小猫咪....用大鸡巴喂饱尤娜吧....Nyaaaaah！"

# game/BKcity_events.rpy:2655
translate chinese_simplified city_cat_65feaf5f:

    # "You both enjoy it tremendously. It isn't long before climax..."
    "情欲越涨越高，你们抱在一起，翻滚缠绵....她的尾巴紧紧地缠住你的肉棒，你撬开她的贝齿，用力吮吸她的蜜汁，两只手在她的身上尽情抚摸。"

# game/BKcity_events.rpy:2661
translate chinese_simplified city_cat_852ecace:

    # yuna "Haaahaaaaaa!!!"
    yuna "Haaahaaaaaa!乳头还有小豆豆!那里从来没人玩弄过!"

# game/BKcity_events.rpy:2669
translate chinese_simplified city_cat_6fb426dd:

    # yuna "Yuna is so happy... Aaaaah..."
    yuna "尤娜好开心....要变成离不开鸡巴的笨猫了....我记住你的气味了"

# game/BKcity_events.rpy:2677
translate chinese_simplified city_cat_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:2684
translate chinese_simplified city_cat_e01181ea_1:

    # yuna "Hello, mister!"
    yuna "你好，先生！"

# game/BKcity_events.rpy:2686
translate chinese_simplified city_cat_c235fa2b:

    # "A very human-like, very sexy, and very naked Yuna is looking at you with hungry eyes."
    "一个酷似人类，非常性感，全身赤裸的猫娘——尤娜用饥渴的眼神看着你。"

# game/BKcity_events.rpy:2690
translate chinese_simplified city_cat_80d93bbb:

    # "But it isn't all... You feel a hand reach down your crotch, freeing your dick from your pants."
    "但这还没完....你感觉到一只柔软的小手伸到你的裆部，把你的鸡巴从裤子里解放出来。"

# game/BKcity_events.rpy:2694
translate chinese_simplified city_cat_9bb965a8:

    # ev_girl3 "Mind if I join? I can't control myself right now..."
    ev_girl3 "介意我加入吗？看着你们我也有些情不自禁...."

# game/BKcity_events.rpy:2696
translate chinese_simplified city_cat_05aae2b0:

    # "It seems like you have little choice but to oblige..."
    "显然这里没有第二个选择，你很爽快地答应了...."

# game/BKcity_events.rpy:2706
translate chinese_simplified city_cat_58566848:

    # ev_girl3 "Such a big cock... Mister, isn't it too big for Yuna?"
    ev_girl3 "把这么大的肉棒塞进喉咙....先生，对尤娜来说你的肉棒是不是太大了？"

# game/BKcity_events.rpy:2708
translate chinese_simplified city_cat_49c34f11:

    # yuna "Nooo! It's just perfect! Go ahead, mister, fuck Yuna's pussy as hard as you can! Ahaa!!!"
    yuna "不！越大越好！来吧，先生，用尽全力蹂躏尤娜的猫娘小穴吧！啊哈！！！"

# game/BKcity_events.rpy:2710
translate chinese_simplified city_cat_672ec378:

    # you "So tight... Hmmm..."
    you "这么紧....嗯...."

# game/BKcity_events.rpy:2716
translate chinese_simplified city_cat_e2352d1b:

    # yuna "Yuna can't take it anymore... Yuna is cumming..."
    yuna "尤娜好想念你的大肉棒....尤娜要去了...."

# game/BKcity_events.rpy:2718
translate chinese_simplified city_cat_a47c436c:

    # ev_girl3 "Good girl, Yuna! Squeeze the nice mister's cock hard!"
    ev_girl3 "干得好，尤娜！用你的肉壁狠狠地夹住他的肉棒！"

# game/BKcity_events.rpy:2720
translate chinese_simplified city_cat_030686f5:

    # you "Ooooh..."
    you "哦哦哦..."

# game/BKcity_events.rpy:2734
translate chinese_simplified city_cat_5db132a6:

    # "You blow a thick load of cum into the catgirl's pussy."
    "你抱住尤娜的腰，使劲向里顶，她浑身抽搐。随后你拽住她的两只胳膊向后牵拉，尤娜的背向后挺，这个姿势下你的肉棒又能再向内挺进。"

# game/BKcity_events.rpy:2736
translate chinese_simplified city_cat_4d7e7ccf:

    # yuna "Aw... Yuna's pussy is so full... Full of hot cum... Nyaaah..."
    yuna "啊....顶到尤娜的花心了....子宫里被灌满了精液牛奶....喵...."

# game/BKcity_events.rpy:2740
translate chinese_simplified city_cat_36f9b415:

    # ev_girl3 "Oh, I want some too... *licking her lips*"
    ev_girl3 "哦，能不能给我也喝一点你的牛奶....*舔她的嘴唇*"

# game/BKcity_events.rpy:2742
translate chinese_simplified city_cat_fe2ffae4:

    # you "*pant* *pant*"
    you "*喘息* *喘息*"

# game/BKcity_events.rpy:2762
translate chinese_simplified city_cat_91734fa9:

    # ev_girl3 "Mister, this isn't finished!!! Let's go for another round!"
    ev_girl3 "这还只是开胃前菜！！接下来才是正餐，用你的肉棒喂饱我吧！"

# game/BKcity_events.rpy:2764
translate chinese_simplified city_cat_3d4288ab:

    # "Your cock is still fully erect. These two pussycat sluts are just too damn sexy."
    "你低下头亲吻她白皙的下巴和脖颈，慢慢又滑入她胸前，不断亲吻舔舐锁骨，用嘴咬开她的衣襟，拱开里面的丝绸亵衣。"

# game/BKcity_events.rpy:2766
translate chinese_simplified city_cat_8f9a1c62:

    # you "Of course... Come here and spread your legs... Show me everything... That's it..."
    you "用手将硕大的乳瓜从亵衣内捧出，白花花的大奶子和嫣红涨硬的乳尖漏出...."

# game/BKcity_events.rpy:2768
translate chinese_simplified city_cat_47128241:

    # "You start fucking her, pumping in and out with your large cock, drilling deep into her tight cunt."
    "两瓣浑圆肥硕的臀部高高翘起。你挺动腰肢，将浓浊精液全部射入。"

# game/BKcity_events.rpy:2770
translate chinese_simplified city_cat_62f43f09:

    # "It isn't long before you are ready to cum a second time."
    "二人痉挛着高潮，双腿高高抬起，足尖绷紧，变成一条直线。"

# game/BKcity_events.rpy:2772
translate chinese_simplified city_cat_3714e19b:

    # you "Haaaaa!"
    you "哈啊啊啊！"

# game/BKcity_events.rpy:2776
translate chinese_simplified city_cat_cbafcc88:

    # ev_girl3 "Hiiiii!!!"
    ev_girl3 "嘿嘿嘿！！！"

# game/BKcity_events.rpy:2780
translate chinese_simplified city_cat_9a2e12a9:

    # yuna "Nyaaaaaaaah!!!"
    yuna "尼玛啊啊啊！！！"

# game/BKcity_events.rpy:2784
translate chinese_simplified city_cat_69bf38ed:

    # "You pull out as you cum, spurting semen all over the girl's pussy."
    "你气喘如牛。将轩双腿扛在肩上，胯下一顶一顶，不断抽插她的小穴，两颗卵蛋尽情的抽打她的臀部。"

# game/BKcity_events.rpy:2796
translate chinese_simplified city_cat_85f5b86f:

    # ev_girl3 "Creampie!!! I love cream..."
    ev_girl3 "在我里面射出来！！！要怀上小宝宝了...."

# game/BKcity_events.rpy:2800
translate chinese_simplified city_cat_914e2d9f:

    # yuna "Ooooh... Yuna wants to lick it clean... Nyaaah..."
    yuna "唔....漏出来好多，尤娜要把它舔干净....喵...."

# game/BKcity_events.rpy:2802
translate chinese_simplified city_cat_1c7fd903:

    # "The two girls kneel at your feet, and hungrily lap every last drop from your shaft, then sharing a mouthful of semen in a passionate kiss."
    "两个女孩跪在你的脚下，饥渴地舔着身上的每一滴精液，深情地和你舌吻。"

# game/BKcity_events.rpy:2806
translate chinese_simplified city_cat_1f30e4d6:

    # "It is already nightfall when you leave the two napping catgirls, covered your with sticky semen and purring in their sleep."
    "当她们累倒睡着时，天色已暗，她们用胳膊和大腿缠住你的身体，在睡梦中发出呼噜呼噜的声音。"

# game/BKcity_events.rpy:2814
translate chinese_simplified city_cat_fcfe1e89:

    # "You have earned prestige. You have no remaining actions for the day."
    "你获得了一些声望。你耗尽了所有的行动力。"

# game/BKcity_events.rpy:2817
translate chinese_simplified city_cat_c271f3ee:

    # "You have received a prestige boost from your [MC.interactions] spent AP."
    "你消耗的[MC.interactions]点行动力增加了你的声望。"

# game/BKcity_events.rpy:2823
translate chinese_simplified city_cat_9ae56eb3:

    # "Sighing, you walk back towards the [loc], cursing the unfathomable stupidity of furry animals."
    "你叹了口气，走回[loc]，咒骂着毛茸茸的动物们不可思议的愚蠢。"

# game/BKcity_events.rpy:2834
translate chinese_simplified city_secret_7a223c83:

    # "As you turn into an alley near the [loc], you notice a shady figure standing in a dark corner."
    "当你拐进[loc]附近的一条小巷时，你注意到一个可疑的身影站在黑暗的角落里。"

# game/BKcity_events.rpy:2852
translate chinese_simplified city_secret_2d86ee51:

    # "You instinctively retreat in the shadows to observe. You want to figure out what's going on before showing yourself."
    "你本能地躲在暗处观察。你想在自己暴露之前弄清楚发生了什么。"

# game/BKcity_events.rpy:2854
translate chinese_simplified city_secret_767f2603:

    # "The man isn't paying attention to his surroundings, anyway. He is fiddling with some sort of mechanism on the wall."
    "不管你伪装的如何，他都没有发现你。他在摆弄墙上的某种机械装置。"

# game/BKcity_events.rpy:2860
translate chinese_simplified city_secret_bd254d44:

    # "You hear a slow rumble, then a heavy thump. The man steps back from the wall, and looks around cautiously. He then walks away, trying to look casual."
    "你听到一阵缓慢的隆隆声，然后是一声沉重的石头撞击声。那人从墙边退了回来，环顾四周。然后他走开了，假装成一个路过的人。"

# game/BKcity_events.rpy:2862
translate chinese_simplified city_secret_c29ec78b:

    # "Once you are sure he is gone, you walk up to the place where he was standing."
    "你确定他走后，来到他刚才出来的地方。"

# game/BKcity_events.rpy:2866
translate chinese_simplified city_secret_86b6b0a5:

    # "Looking at the wall, you see nothing at first. Upon close examination, thin lines seem to mark the shape of a secret door. If there is an opening mechanism, however, you don't see it."
    "观察墙壁，起初好像没什么特别的。仔细摸索，似乎能看到一扇门的轮廓。但是，就算发现了门，如果没解锁机关，门也不会打开。"

# game/BKcity_events.rpy:2877
translate chinese_simplified city_secret_e8730471:

    # you "There's nothing that can't be solved with a little violence."
    you "一力降十会，看看是这扇门结实还是我的身体结实。"

# game/BKcity_events.rpy:2879
translate chinese_simplified city_secret_8db41ea1:

    # "Taking the measure of the wall and its resistance before making your move, you take a step back."
    "在你准备破门前，你目测了一下距离和门的位置，然后向后退了几步。"

# game/BKcity_events.rpy:2881
translate chinese_simplified city_secret_73032446:

    # you "Kyaaaah!!!"
    you "呀啊啊啊啊啊!!!"

# game/BKcity_events.rpy:2883
translate chinese_simplified city_secret_243d4c23:

    # "Using all your strength, you dash your shoulder into the door."
    "用尽你所有的力气，用你的肩膀撞到门上。"

# game/BKcity_events.rpy:2895
translate chinese_simplified city_secret_ec07890f:

    # "The lock snaps with a cracking sound, and the battered door slams open before you. You barely avoid falling down a steep stairway right behind the door."
    "锁啪的一声弹开了，破旧的门被你撞开。你差点从门后的楼梯上摔下来。"

# game/BKcity_events.rpy:2897
translate chinese_simplified city_secret_2ed6fda0:

    # you "I wonder where this leads..."
    you "不知道这会不会触发什么机关...."

# game/BKcity_events.rpy:2903
translate chinese_simplified city_secret_fe2f02f7:

    # you "Ouch!!!" with vpunch
    you "哎哟！！！" with vpunch

# game/BKcity_events.rpy:2905
translate chinese_simplified city_secret_ab58b721:

    # "You slam into the wall, doing no visible damage, except to your shoulder and pride."
    "你撞在墙上，除了你的肩膀和自尊，别的东西都没有造成明显的伤害。"

# game/BKcity_events.rpy:2913
translate chinese_simplified city_secret_2641943b:

    # "Stepping back, you take a pouch from your belt."
    "退后一步，你从腰带上取下一个袋子。"

# game/BKcity_events.rpy:2915
translate chinese_simplified city_secret_bcfb6305:

    # "You take a handful of silver dust from the pouch, which you spread on the wall before you."
    "你从袋子里取出一些显影之尘。"

# game/BKcity_events.rpy:2917
translate chinese_simplified city_secret_69e68765:

    # you "I am the seeker of secrets! None shall be hidden from my all-seeing eye..."
    you "我是解密大师！没有秘密可以瞒过我的眼睛...."

# game/BKcity_events.rpy:2919
translate chinese_simplified city_secret_7fbf3529:

    # you "Open sesame!"
    you "芝麻开门！"

# game/BKcity_events.rpy:2927
translate chinese_simplified city_secret_6e4d077d:

    # "The silver powder shines with incandescent light, illuminating the wall's surface."
    "银粉在阳光的照耀下，发出炫目的光芒覆盖了墙壁的表面。"

# game/BKcity_events.rpy:2931
translate chinese_simplified city_secret_43c514fe:

    # "Eventually the light recedes, except over the right side of the wall, where it delineates a hidden runic seal."
    "最后，光线逐渐消退，在墙壁右侧，出现一个隐藏的符文封印。"

# game/BKcity_events.rpy:2937
translate chinese_simplified city_secret_98a43605:

    # "Easily deciphering the rune, you apply pressure on the correct spot. The wall slides open, revealing a dark staircase."
    "破译这种简单符文对你来说轻而易举，你在正确的位置释放魔力。墙壁向一侧滑开，露出一个黑暗的通道。"

# game/BKcity_events.rpy:2941
translate chinese_simplified city_secret_807c93c3:

    # "A sudden gust of wind clears out all of the silver dust. Cursing, you reach inside your pouch, but there is not enough powder left."
    "一阵突如其来的狂风吹走了所有的显影之尘。真倒霉，你把手伸进袋子里摸索，但剩下的粉末不多了。"

# game/BKcity_events.rpy:2945
translate chinese_simplified city_secret_6e5f3372:

    # you "Damn... This powder isn't cheap..."
    you "该死的....这种粉可不便宜...."

# game/BKcity_events.rpy:2952
translate chinese_simplified city_secret_74964b19:

    # "You don't see any easy way to open it."
    "你想不到任何打开它的方法。"

# game/BKcity_events.rpy:2954
translate chinese_simplified city_secret_ea9cc5de:

    # you "This is a waste of time. Let's just go."
    you "这完全是浪费时间。该走了。"

# game/BKcity_events.rpy:2967
translate chinese_simplified city_secret_5185d497:

    # "You carefully follow the steps down, eventually reaching a dark cellar."
    "你小心地沿着台阶往下走，发现里面别有洞天，简直就是个迷宫。"

# game/BKcity_events.rpy:2979
translate chinese_simplified city_secret_42cfde6a:

    # "The place is eerily quiet as you move about and search the rooms."
    "当你四处走动并搜索房间时，这个地方出奇地安静。"

# game/BKcity_events.rpy:2981
translate chinese_simplified city_secret_a07d688a:

    # "There isn't much to be found in this labyrinth. You decide to follow some footprints in the dust."
    "在这个迷宫里找不到什么东西。你决定跟着地上的脚印走。"

# game/BKcity_events.rpy:2983
translate chinese_simplified city_secret_9adee427:

    # "The trail leads you to a chest, which looks like it was used recently."
    "脚印最后消失在一个箱子面前，看起来最近有人打开过箱子。"

# game/BKcity_events.rpy:2985
translate chinese_simplified city_secret_a09bde46:

    # "Inside, you find something that could be of use to you."
    "你在箱子里发现了对你有用的道具。"

# game/BKcity_events.rpy:2997
translate chinese_simplified city_secret_3d687d5f:

    # bk_error "Couldn't generate Item for rank [_rank]"
    bk_error "无法生成[_rank]品质的道具"

# game/BKcity_events.rpy:3003
translate chinese_simplified city_secret_8a1ad209:

    # "You emerge into a candle-lit room, full of intriguing contraptions."
    "你走进一间点着蜡烛的房间，里面满是有趣的装置。"

# game/BKcity_events.rpy:3005
translate chinese_simplified city_secret_d4e1d540:

    # "You can hear muffled moans coming from a corner of the room."
    "你听到附近有微弱的呻吟声传来，空气中弥漫着精液的腥臭味。"

# game/BKcity_events.rpy:3015
translate chinese_simplified city_secret_b4151c49:

    # "You see a girl there, bound and gagged. She sees you, but doesn't seem to react to your presence."
    "循声摸过去，你看到一个被绑起来，被口球堵住了嘴的女孩。她也看到了你，但似乎对你的存在没有任何反应。。"

# game/BKcity_events.rpy:3017
translate chinese_simplified city_secret_0e568b0c:

    # "The air is thick with the smell of moisture and semen."
    "空气中弥漫着精液的味道。"

# game/BKcity_events.rpy:3019
translate chinese_simplified city_secret_d5dec428:

    # you "Do you need help?"
    you "你需要帮助吗？"

# game/BKcity_events.rpy:3023
translate chinese_simplified city_secret_b751bc5d:

    # "She ignores you, instead concentrating on something that's happening to her."
    "她无视了你，把注意力集中在感受身体上的变化。"

# game/BKcity_events.rpy:3027
translate chinese_simplified city_secret_ecf3532f:

    # "You can hear the vibration of a sex toy, plugged deep inside the girl."
    "你听到了自慰棒的振动声，它就插在女孩小穴深处。"

# game/BKcity_events.rpy:3031
translate chinese_simplified city_secret_14e020c1:

    # ev_girl1 "Aaaaaaaaaah!!!"
    ev_girl1 "啊啊啊啊啊!!!"

# game/BKcity_events.rpy:3035
translate chinese_simplified city_secret_1a1e27d3:

    # "She reaches a quiet orgasm as you watch, apparently the latest in a long series."
    "在你欣赏的时候，她又一次迎来了高潮，不过因为口球她发不出明显的声音。显然她已经高潮过很多次了。"

# game/BKcity_events.rpy:3037
translate chinese_simplified city_secret_562e0d6a:

    # "You notice something unexpected, shoved into one of the girl's slutty holes."
    "你注意到那个女孩的小穴里塞着什么东西。"

# game/BKcity_events.rpy:3039
translate chinese_simplified city_secret_7c244c01:

    # you "Do you mind?"
    you "介意我拔出来吗？"

# game/BKcity_events.rpy:3045
translate chinese_simplified city_secret_ecda54b4:

    # "You retrieve the item. It could be useful, after Sill gives it a good washing."
    "你拔出一个沾满淫水的道具。让希露洗一洗应该可以给姑娘们用。"

# game/BKcity_events.rpy:3061
translate chinese_simplified city_secret_3d687d5f_1:

    # bk_error "Couldn't generate Item for rank [_rank]"
    bk_error "无法生成[_rank]品质的道具"

# game/BKcity_events.rpy:3065
translate chinese_simplified city_secret_e9413567:

    # "There's nothing more you can do. Grumbling, you head back towards the [loc]."
    "你已经无能为力了。你一边抱怨，一边往[loc]方向走去。"

# game/BKcity_events.rpy:3080
translate chinese_simplified city_gypsy_133717b4:

    # "Strolling around the [loc], you meet a lone girl in exotic clothes, with a worried look on her face."
    "在[loc]附近闲逛时，你遇到一个穿着异国风情服装的孤独女孩，脸上带着担忧的神情。"

# game/BKcity_events.rpy:3123
translate chinese_simplified city_gypsy_81b477d7:

    # ev_girl2 "Hey, you! [nickname]!"
    ev_girl2 "嘿，好心人！[nickname]！"

# game/BKcity_events.rpy:3125
translate chinese_simplified city_gypsy_ac3493fd:

    # you "Uh? Me?"
    you "呃?我自己?"

# game/BKcity_events.rpy:3127
translate chinese_simplified city_gypsy_787b1498:

    # ev_girl2 "Yes! [nickname]! I need your help. Please?"
    ev_girl2 "是的！[nickname]！我需要你的帮助。帮帮我好吗？"

# game/BKcity_events.rpy:3129
translate chinese_simplified city_gypsy_8184bfec:

    # "You look at her with suspicion. She has an exotic look about her. She doesn't look like she's from around here."
    "你用怀疑的目光看着她。她的打扮充满异国情调。她看起来不像本地人。"

# game/BKcity_events.rpy:3131
translate chinese_simplified city_gypsy_1d6c2446:

    # ev_girl2 "My wagon broke down on the way to the market. It's stuck here, in this deserted place."
    ev_girl2 "我的马车在去市场的路上抛锚了。它被卡在这里，这个偏僻的地方。"

# game/BKcity_events.rpy:3133
translate chinese_simplified city_gypsy_407a8e5e:

    # "You can see her wagon down in a small street. It's in a sorry state, stuck in the mud with a broken wheel."
    "你可以在路旁看到她的马车。它陷在泥巴里，轮毂坏了。"

# game/BKcity_events.rpy:3135
translate chinese_simplified city_gypsy_94fc1c7e:

    # ev_girl2 "I can't find any customers here, and without customers I cannot get the money I need to get the repairs done!"
    ev_girl2 "这里可做不了生意，没有顾客付钱我就没办法支付修理马车的账单！"

# game/BKcity_events.rpy:3137
translate chinese_simplified city_gypsy_d4fad9bf:

    # you "And what is it that you do?"
    you "你是做什么的？"

# game/BKcity_events.rpy:3139
translate chinese_simplified city_gypsy_4910c724:

    # ev_girl2 "I'm a fortune teller. Want to know the exact place, cause and depth of agony of your inevitable death? I can tell you."
    ev_girl2 "我是个占卜师。想不想知道你的寿命，姻缘？我可以为你解惑。"

# game/BKcity_events.rpy:3141
translate chinese_simplified city_gypsy_0f169813:

    # you "Erm, no, thanks..."
    you "嗯，不用了，谢谢...."

# game/BKcity_events.rpy:3143
translate chinese_simplified city_gypsy_eef0eb53:

    # ev_girl2 "So, [nickname_l]. Will you help me?"
    ev_girl2 "那么，[nickname_l]。你愿意帮我吗？"

# game/BKcity_events.rpy:3154
translate chinese_simplified city_gypsy_acdd4596:

    # you "I could always try to fix your ride... Let me have a look."
    you "我可以试试能不能把马车修好....让我看看。"

# game/BKcity_events.rpy:3156
translate chinese_simplified city_gypsy_cb4959c6:

    # ev_girl2 "Thank you, [nickname_l]! You're a dear!"
    ev_girl2 "谢谢你，[nickname_l]！你真是我的救星！"

# game/BKcity_events.rpy:3158
translate chinese_simplified city_gypsy_60fce3e9:

    # "You try to fix the broken cart to the best of your ability, but you're not sure it can hold together."
    "你想尽办法去修理那辆坏了的马车，但你不确定能不能修好。"

# game/BKcity_events.rpy:3160
translate chinese_simplified city_gypsy_e9054f2d:

    # "Now you must pull the cart out of the mud. This requires some heavy lifting."
    "现在你必须把车从泥巴里拖出来。这需要不小的力气。"

# game/BKcity_events.rpy:3162
translate chinese_simplified city_gypsy_fa73f4ec:

    # you "HAAAAAAAA..."
    you "哈啊啊啊...."

# game/BKcity_events.rpy:3174
translate chinese_simplified city_gypsy_d2cb010c:

    # "The cart is too heavy, and your hands slip. You fall down in the mud, cursing."
    "车子太重了，你脱手了，跌倒在泥里，咒骂着。"

# game/BKcity_events.rpy:3176
translate chinese_simplified city_gypsy_c445c883:

    # you "Fuck!!!"
    you "他妈的！！！"

# game/BKcity_events.rpy:3178
translate chinese_simplified city_gypsy_4fc4b582:

    # ev_girl2 "Ooooh... The wheel broke down again... And you've splashed mud all over the place!"
    ev_girl2 "唔....马车又陷进去了....你还把泥巴溅得到处都是！"

# game/BKcity_events.rpy:3180
translate chinese_simplified city_gypsy_6ed7ba4d:

    # you "Well, excuse me, I was just trying to help..."
    you "对不起，我只是想帮你...."

# game/BKcity_events.rpy:3186
translate chinese_simplified city_gypsy_da9c56eb:

    # "You push the cart with all your strength, trying different angles to get it unstuck."
    "你用尽全力推着车，试着从不同的角度把它推出去。"

# game/BKcity_events.rpy:3192
translate chinese_simplified city_gypsy_0a2a8633:

    # "The wagon rocks back and forth, and you hear the sound of broken glass coming from the inside."
    "马车前后摇晃着，你听到里面有玻璃打碎的声音。"

# game/BKcity_events.rpy:3194
translate chinese_simplified city_gypsy_607c39b0:

    # ev_girl2 "Hey!!! My philters!!!"
    ev_girl2 "嘿！！！我的媚药！！！"

# game/BKcity_events.rpy:3200
translate chinese_simplified city_gypsy_2393573f:

    # "After a few minutes, however, your efforts are rewarded. The cart rolls grudgingly out of the mud."
    "幸好，几分钟后，你的努力得到了回报。马车缓慢地从泥坑里推了出来。"

# game/BKcity_events.rpy:3202
translate chinese_simplified city_gypsy_527330c6:

    # ev_girl2 "You did it!!!"
    ev_girl2 "你做到了！！！"

# game/BKcity_events.rpy:3204
translate chinese_simplified city_gypsy_f7f10345:

    # you "*pant*, *pant*"
    you "*喘息*,*喘息*"

# game/BKcity_events.rpy:3206
translate chinese_simplified city_gypsy_a35326b2:

    # "It looks like the cart's damaged wheel can hold long enough to make it to the market."
    "看来，这个破损的轮毂足够支撑这辆马车行驶到市区里摆摊，然后等待工匠修复了。"

# game/BKcity_events.rpy:3214
translate chinese_simplified city_gypsy_b06d6038:

    # "Using your considerable might, you lift the cart off the ground and safely put it back on the pavement."
    "你使出九牛二虎之力，直接把车抱起抬出泥坑，安全平稳地放回人行道上。"

# game/BKcity_events.rpy:3216
translate chinese_simplified city_gypsy_4f4b8598:

    # ev_girl2 "You did it, [nickname_l]!!! It's amazing!!!"
    ev_girl2 "你做到了，[nickname_l]！！！你简直就是大力神再世！！！"

# game/BKcity_events.rpy:3218
translate chinese_simplified city_gypsy_f2984926:

    # you "*pant* It was easy... *pant*"
    you "*呼气* 这很容易....*拍拍手上的灰*"

# game/BKcity_events.rpy:3220
translate chinese_simplified city_gypsy_fcd41708:

    # ev_girl2 "Let me touch those big muscles of yours... Oh, they're nice..."
    ev_girl2 "让我摸摸你壮硕的肌肉....哦，这完美的线条和硬度...."

# game/BKcity_events.rpy:3230
translate chinese_simplified city_gypsy_618ee2db:

    # you "I can go to the main street, and tell people about your services. Word of mouth could help."
    you "我可以到街上，告诉人们这里有个占卜师。也许他们会过来找你占卜。"

# game/BKcity_events.rpy:3232
translate chinese_simplified city_gypsy_61e9d2fa:

    # ev_girl2 "Oh..."
    ev_girl2 "哦..."

# game/BKcity_events.rpy:3234
translate chinese_simplified city_gypsy_149b5791:

    # ev_girl2 "You're right, [nickname_l]! It's certainly worth trying."
    ev_girl2 "也许可行，[nickname_l]！当然值得一试。"

# game/BKcity_events.rpy:3238
translate chinese_simplified city_gypsy_85f3ffa9:

    # "You walk back to the main street near the [loc], and start telling people about the fortune teller."
    "你走回[loc]附近的主街，开始向人们讲述算命先生的事。"

# game/BKcity_events.rpy:3240
translate chinese_simplified city_gypsy_f111166d:

    # you "Come with me, friends, meet the amazing lass who can reveal your future! Destiny awaits, just a few steps away!"
    you "跟我来吧，朋友们，见见这个神奇的占卜师，她能预知你的未来！想要掌控自己的命运吗，就在几步之遥！"

# game/BKcity_events.rpy:3248
translate chinese_simplified city_gypsy_58d67c08:

    # "Most people ignore you, however, until you run into a group of Arios worshippers."
    "然而，路人对你视若无睹，直到你遇到一群太阳神教的信徒。"

# game/BKcity_events.rpy:3250
translate chinese_simplified city_gypsy_daef8ba2:

    # man "Heretic! This foul heretic is advertising the services of a witch!"
    man "异教徒！这个肮脏的异教徒在宣传一个女巫的巫术！"

# game/BKcity_events.rpy:3252
translate chinese_simplified city_gypsy_e1fe1bd6:

    # "You get into an argument with the man, while a crowd of curious onlookers gathers around you."
    "你和那个领头的男人吵了起来，一群好事的旁观者围在你身边。"

# game/BKcity_events.rpy:3254
translate chinese_simplified city_gypsy_5cb7c6d8:

    # man "No doubt he wants to lead us into an alley so he can slit our throat! Down with the heretic!"
    man "他想把我们带到一条小巷里，然后割断我们的喉咙放血！打倒这个该死的异教徒！"

# game/BKcity_events.rpy:3256
translate chinese_simplified city_gypsy_209f759c:

    # "The crowd roars in agreement. Things are starting to look nasty as the man becomes more threatening."
    "群众开始起哄。随着男子激烈的言辞，情况开始变得糟糕起来。"

# game/BKcity_events.rpy:3258
translate chinese_simplified city_gypsy_b7762874:

    # man "Come with us!"
    man "一起上！"

# game/BKcity_events.rpy:3262
translate chinese_simplified city_gypsy_3803a96a:

    # "You flash your weapon, and the crowd takes a step back. Before they can react, you jump into a side street and make your escape."
    "你亮出武器，周围的人群向后退了一步。他们还没反应过来，你就闪到一条小巷里逃跑了。"

# game/BKcity_events.rpy:3266
translate chinese_simplified city_gypsy_3599bd33:

    # you "That was close..."
    you "真险啊，差点就被...."

# game/BKcity_events.rpy:3272
translate chinese_simplified city_gypsy_12d9bfd1:

    # "You don't have much success at first, but with time, you are able to send a few customers her way."
    "刚开始没几个人搭理你，但随着时间的推移，你引起了一些人的兴趣。"

# game/BKcity_events.rpy:3274
translate chinese_simplified city_gypsy_524e7104:

    # "You hope it will be enough for her to make ends meet."
    "希望这些顾客能够让占卜师赚到足够多的钱去修理马车。"

# game/BKcity_events.rpy:3278
translate chinese_simplified city_gypsy_e68988da:

    # "After an hour or so, you come back to the wagon. You run into one of the customers, exiting the broken wagon with a large smile on his face."
    "大约一个小时后，你回到泥坑里的马车旁。你碰见一个刚好从马车里出来的顾客，他脸上带着灿烂的笑容从马车里走出来。"

# game/BKcity_events.rpy:3282
translate chinese_simplified city_gypsy_bc350c26:

    # you "Hello there! Did you get enough to get by?"
    you "嘿，你赚够钱了吗？"

# game/BKcity_events.rpy:3284
translate chinese_simplified city_gypsy_aabe3f92:

    # ev_girl2 "I did! Some customers paid extra... It helped."
    ev_girl2 "钱足够了！有些顾客付了额外的钱....他们很慷慨。"

# game/BKcity_events.rpy:3286
translate chinese_simplified city_gypsy_47e4dbd6:

    # "She looks a bit uneasy and straightens her clothes as she speaks."
    "她看上去有些心虚，一边说一边把衣领拉直。"

# game/BKcity_events.rpy:3292
translate chinese_simplified city_gypsy_30a179bc:

    # "Using your great people skills, you quickly convince a few passersby to listen to you."
    "利用你出色的人际交往技巧，你很快就吸引了一群路人。"

# game/BKcity_events.rpy:3294
translate chinese_simplified city_gypsy_82dfb4ae:

    # "The crowd grows even larger when a bunch of sailors come to listen, fresh off their ship."
    "一群刚回港的水手也好奇地围了过来，周围的人越来越多。"

# game/BKcity_events.rpy:3296
translate chinese_simplified city_gypsy_6ac98a16:

    # man "Hey, let's get our fortune told! I wanna know when I'm gonna get laid!"
    man "嘿，给我算算！我想知道我什么时候能和一个漂亮的女人做爱！"

# game/BKcity_events.rpy:3298
translate chinese_simplified city_gypsy_ccb1a892:

    # "People pour into the alley, eager to get their fortune told."
    "人们涌进小巷，抢着算命。"

# game/BKcity_events.rpy:3302
translate chinese_simplified city_gypsy_e0991a37:

    # "You come back to the girl's wagon after a while, happy with the business you sent her way."
    "过了一会儿，你回到女孩的马车旁，她对你带来这么多的客人表示感谢。"

# game/BKcity_events.rpy:3306
translate chinese_simplified city_gypsy_f9264ca4:

    # ev_girl2 "Hey, it's you!!! I made a killing! It's amazing!"
    ev_girl2 "嘿，你！！！我大赚了一笔钱！多亏了你！"

# game/BKcity_events.rpy:3308
translate chinese_simplified city_gypsy_6bce16ef:

    # "She hugs you and kisses your cheek."
    "她紧紧抱住你，亲吻你的脸颊。"

# game/BKcity_events.rpy:3310
translate chinese_simplified city_gypsy_f75186ef:

    # ev_girl2 "It's all thanks to you, [nickname_l]! I'm really grateful..."
    ev_girl2 "都是你的功劳，[nickname_l]！我真的很感激...."

# game/BKcity_events.rpy:3318
translate chinese_simplified city_gypsy_992ff48b:

    # you "I'm sorry, but I can't do anything about it. Best of luck with your problem."
    you "对不起，但我无能为力。祝你好运。"

# game/BKcity_events.rpy:3320
translate chinese_simplified city_gypsy_ba785b70:

    # ev_girl2 "Hey, waaaait!"
    ev_girl2 "嘿，等等！"

# game/BKcity_events.rpy:3338
translate chinese_simplified city_gypsy_930f560c:

    # "You go back to the [loc]."
    "你回到[loc]。"

# game/BKcity_events.rpy:3344
translate chinese_simplified city_gypsy_60b73731:

    # ev_girl2 "Thank you, [nickname_l], you did me a favor! Here, wait a second..."
    ev_girl2 "谢谢，[nickname_l]，你帮了我一个大忙！在这稍微等一下...."

# game/BKcity_events.rpy:3346
translate chinese_simplified city_gypsy_7d532edc:

    # "The girl rummages through her wagon for a minute, then comes out, holding a dusty item."
    "那姑娘翻遍了她的马车，走了出来，手里拿着一个满是灰尘的东西。"

# game/BKcity_events.rpy:3348
translate chinese_simplified city_gypsy_16a968c2:

    # ev_girl2 "Here, have this."
    ev_girl2 "收下吧，这个我珍藏很久了。"

# game/BKcity_events.rpy:3362
translate chinese_simplified city_gypsy_3d687d5f:

    # bk_error "Couldn't generate Item for rank [_rank]"
    bk_error "无法生成[_rank]品质的道具"

# game/BKcity_events.rpy:3367
translate chinese_simplified city_gypsy_8c372b81:

    # "She leans into your ear and whispers."
    "她靠近你的耳朵，低声细语。"

# game/BKcity_events.rpy:3369
translate chinese_simplified city_gypsy_4dc9a6cd:

    # ev_girl2 "You saved me, [nickname_l], so I will show you something interesting... Follow me..."
    ev_girl2 "你救了我，[nickname_l]，想不想测测你的桃花运....跟我来...."

# game/BKcity_events.rpy:3395
translate chinese_simplified city_gypsy_3eb9711f:

    # ev_girl2 "Oh, [nickname_l]! Give it to me... Mmmh..."
    ev_girl2 "哦，[nickname_l]！把那个给我....嗯...."

# game/BKcity_events.rpy:3401
translate chinese_simplified city_gypsy_5fe5687c:

    # ev_girl2 "Aaaaaah! I... I didn't know you liked this sort of things..."
    ev_girl2 "啊！我....我不知道你喜欢这种东西...."

# game/BKcity_events.rpy:3403
translate chinese_simplified city_gypsy_106d629b:

    # you "You seem to enjoy it just as much as me..."
    you "看起来你也不讨厌...."

# game/BKcity_events.rpy:3405
translate chinese_simplified city_gypsy_384154a6:

    # "Her eyes are glistening with lust. You can't hold back much longer."
    "她的眼睛闪烁着欲望的光芒。你不能再憋了。"

# game/BKcity_events.rpy:3409
translate chinese_simplified city_gypsy_380348d4:

    # "She screams with delight as you proceed to cum all over her face."
    "她在你的胯下高潮。酡红的双颊，微微张开的小嘴，迷离的神情，因为高潮喘气而上下起伏雪白胸乳。以及，高潮时紧紧收缩，仿佛有无数双小手攥着他肉棒的蜜穴。。"

# game/BKcity_events.rpy:3413
translate chinese_simplified city_gypsy_929d633d:

    # you "Aaaaah!!!"
    you "你的肉棒在她蜜穴里搅弄，顶入后，龟头盯着花芯上部，腰杆扭一扭，龟头研磨花芯，让她受不住的轻吟一声，再果断将肉棒拔出。她的小穴在这时候会紧紧缠住肉棒，肉棒拔出，穴内许多汁水也被带出!!"

# game/BKcity_events.rpy:3419
translate chinese_simplified city_gypsy_fdbf20d3:

    # ev_girl2 "I'm cumming!!! Hiiiiii!!!"
    ev_girl2 "我要去了！！！啊～～啊～～嗯嗯～～啊～～！！！"

# game/BKcity_events.rpy:3423
translate chinese_simplified city_gypsy_34b60ad2:

    # ev_girl2 "Your dick is so big and tasty... Give me your juice..."
    ev_girl2 "黏腻的蜜汁溅射在她的蜜部，让两人交合的地方狼藉不堪。但她已经顾不得这些。被男人肏得全身软绵绵的，啊啊嗯嗯的轻声呻吟着，美眸闭上，绝美的娇颜满是红晕。享受着交媾的快感...."

# game/BKcity_events.rpy:3425
translate chinese_simplified city_gypsy_ae7875cc:

    # you "Oh, it's good... Oooooh..."
    you "你的腰杆慢慢挺动，胯下肉棒匀速抽插着蜜穴，两人的交合动作让马车微微震动，她变得越来越敏感，蜜穴被肉棒插得发出滋滋的水声，那天籁似的轻微呻吟声没有停下来过。"

# game/BKcity_events.rpy:3431
translate chinese_simplified city_gypsy_d12156fb:

    # ev_girl2 "Oooh!!!"
    ev_girl2 "她洁白的贝齿紧咬嫣红的下唇，只从琼鼻和喉咙深处发出因交媾欢愉的呻吟声！"

# game/BKcity_events.rpy:3435
translate chinese_simplified city_gypsy_87a82292:

    # "You cum hard thanks to the girl's expert technique, and she looks with satisfaction at your spurting cock."
    "她全身赤裸的躺在你的胯下，小穴一抽一抽的达到高潮，阴道内的嫩肉紧紧的缠住他的肉棒。神情迷离，清冷不再。"

# game/BKcity_events.rpy:3439
translate chinese_simplified city_gypsy_5f2e05d3:

    # "The girl gently licks the cum off your shaft while you take a short rest."
    "这才是刚刚开始，接下来，她还要再被你肏弄几个小时，摆出多个姿势，被抽插得高潮连连。前面的蜜穴，后面的菊蕾，高耸雪白的乳峰，浑圆饱满的臀部，盈盈一握的腰肢，还有香甜的嘴唇。她的全身上下，都要被你玩弄一遍！"

# game/BKcity_events.rpy:3443
translate chinese_simplified city_gypsy_c380fa0a:

    # ev_girl2 "Mmmmh..."
    ev_girl2 "嗯嗯..."

# game/BKcity_events.rpy:3445
translate chinese_simplified city_gypsy_0e33a685:

    # ev_girl2 "I almost forgot... I have this I no longer need. I'm sure you can put it to good use."
    ev_girl2 "我差点忘了....我不再需要这个了。我相信你能好好利用它。"

# game/BKcity_events.rpy:3447
translate chinese_simplified city_gypsy_dfdf7298:

    # ev_girl2 "Now... I hope you're ready for round two! *wink*"
    ev_girl2 "她张开着双腿让你插她，雪白晶莹的玉趾紧紧的收缩，躺在床上被撞得上下前后的颠簸。*看来这将是一场恶战*"

# game/BKcity_events.rpy:3464
translate chinese_simplified city_gypsy_3d687d5f_1:

    # bk_error "Couldn't generate Item for rank [_rank]"
    bk_error "无法生成[_rank]品质的道具"

# game/BKcity_events.rpy:3478
translate chinese_simplified city_rob_22642a73:

    # "As you pass through one of the [loc]'s less reputable parts, you see a woman barring your way."
    "当你经过[loc]一个不太体面的地区时，你会看到一个女人挡住了你的去路。"

# game/BKcity_events.rpy:3500
translate chinese_simplified city_rob_ee0d4625:

    # woman "Hey there, Mister! Going somewhere?"
    woman "嘿，先生！有空吗？"

# game/BKcity_events.rpy:3504
translate chinese_simplified city_rob_a4d44ee9:

    # woman "Don't be in such a hurry..."
    woman "别急着走嘛...."

# game/BKcity_events.rpy:3506
translate chinese_simplified city_rob_f98fd8d4:

    # "The woman is armed, and casually pointing her weapon at you."
    "那个女人全副武装，随意地用她的武器指着你。"

# game/BKcity_events.rpy:3508
translate chinese_simplified city_rob_27314a95:

    # woman "This pouch of gold is weighting you down, methinks. Why don't you hand it over, and be on your way?"
    woman "看看你的钱包，那么沉。为什么不把它交给我，好让你轻松一点呢？"

# game/BKcity_events.rpy:3523
translate chinese_simplified city_rob_1ea17f50:

    # "You are not going to let yourself be pushed over by a girl."
    "你不会让自己被一个女孩抢劫。"

# game/BKcity_events.rpy:3525
translate chinese_simplified city_rob_343b1821:

    # you "Out of my way, bitch, or I'll crush you like a worm."
    you "让开，我没工夫和你玩过家家，否则我会把你扒光了按在地上狠狠地打屁股。"

# game/BKcity_events.rpy:3535
translate chinese_simplified city_rob_0bc7db9d:

    # "She laughs."
    "她大笑起来。"

# game/BKcity_events.rpy:3537
translate chinese_simplified city_rob_30edc188:

    # woman "Oh, a stubborn one. Have it your way, sucker!"
    woman "哦，随你怎么说好了，接招吧！"

# game/BKcity_events.rpy:3539
translate chinese_simplified city_rob_0d497728:

    # "She takes an attack stance. You reach for your weapon."
    "她摆出攻击姿态。你立刻伸手准备掏出自己的武器。"

# game/BKcity_events.rpy:3545
translate chinese_simplified city_rob_1cf9a909:

    # "With lightning speed, the woman leaps in the air above you."
    "女人以闪电般的速度绕到了你的侧身。"

# game/BKcity_events.rpy:3547
translate chinese_simplified city_rob_281147bb:

    # woman "KYAAAAAAAAAAH!!!"
    woman "呀啊啊啊啊!!!"

# game/BKcity_events.rpy:3553
translate chinese_simplified city_rob_9f0ea931:

    # "You have no time to react before she clubs you on the head with the blunt of her weapon. You fall down in the dirt, nearly passing out."
    "你还没有反应过来，她就用武器打中了你的头。你倒在地上，差点晕过去。"

# game/BKcity_events.rpy:3555
translate chinese_simplified city_rob_7d3fb1b2:

    # woman "Taking on the queen of bandits! You're lucky to be alive..."
    woman "今天就饶你一命，下次再敢有非分之想...."

# game/BKcity_events.rpy:3559
translate chinese_simplified city_rob_264126d8:

    # woman "Now, if you'll excuse me, I have other weaklings to attend to."
    woman "希望你下次能主动点上贡，不要逼我亲自动手，我还得去找下一个目标，再见啦~"

# game/BKcity_events.rpy:3563
translate chinese_simplified city_rob_ca8c1ab5:

    # "She swiftly liberates your purse from your belt."
    "她粗暴地把你的钱包从腰带上扯下。"

# game/BKcity_events.rpy:3567
translate chinese_simplified city_rob_4a0f2c55:

    # woman "Thank you! Have a nice day!"
    woman "祝你今天有个好心情！"

# game/BKcity_events.rpy:3569
translate chinese_simplified city_rob_e814e33f:

    # "When you come back to your senses, she's long gone."
    "当你恢复清醒时，她早就走远了。"

# game/BKcity_events.rpy:3587
translate chinese_simplified city_rob_04964a88:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:3592
translate chinese_simplified city_rob_3ae276c8:

    # "She takes a second look at you."
    "她又仔细地上下打量了你。"

# game/BKcity_events.rpy:3594
translate chinese_simplified city_rob_dd019ae4:

    # woman "Humph... Maybe this won't be as easy as I thought. *mutter*"
    woman "哼....看来你还有两把刷子。*咕哝*"

# game/BKcity_events.rpy:3598
translate chinese_simplified city_rob_172dbbf1:

    # woman "All right! I'm in a good mood today, you're in luck!"
    woman "好吧！本小姐今天心情很好，你很走运！"

# game/BKcity_events.rpy:3600
translate chinese_simplified city_rob_5b462fec:

    # woman "You can go, but next time, you will have to pay my toll."
    woman "这次就先放过你，下次可就没这么走运了。"

# game/BKcity_events.rpy:3602
translate chinese_simplified city_rob_39b396c2:

    # you "Whatever. *sigh*"
    you "随你怎么说吧，你开心就好。*叹息*"

# game/BKcity_events.rpy:3606
translate chinese_simplified city_rob_efb96f01:

    # "You push past her and make your way back to the [loc]."
    "你推开她，返回[loc]。"

# game/BKcity_events.rpy:3610
translate chinese_simplified city_rob_2da2f8a4:

    # woman "Who do you think you are! I'll teach you!!! I'll..."
    woman "你以为你是谁？让我给你上一课吧！！！看我的...."

# game/BKcity_events.rpy:3624
translate chinese_simplified city_rob_890e88c2:

    # "In one fell swoop, you draw your sword and smash her weapon out of her hands."
    "你快速地拔出佩剑，一刺一钩，她的武器一瞬间就被你击落在地。"

# game/BKcity_events.rpy:3628
translate chinese_simplified city_rob_310ab429:

    # "She squeals and looks at her empty hands in disbelief."
    "她尖叫着，不敢相信地看着被打落的武器。"

# game/BKcity_events.rpy:3630
translate chinese_simplified city_rob_4f4dae69:

    # woman "F... Fast... How..."
    woman "屁股...打的力度能轻点么..."

# game/BKcity_events.rpy:3632
translate chinese_simplified city_rob_1d846d90:

    # "You point your weapon at her throat."
    "你用你的武器指着她的喉咙。"

# game/BKcity_events.rpy:3634
translate chinese_simplified city_rob_adaac038:

    # you "Shut up. You're coming with me now, we'll have a word with the guard."
    you "闭嘴。跟我去卫兵那报道吧，姑娘家家的做什么强盗。"

# game/BKcity_events.rpy:3638
translate chinese_simplified city_rob_9f18653f:

    # woman "The guard? Noooo! They'll rape and torture me! Please, Mister, I'll do anything!"
    woman "卫兵？他！他们会折磨我的！求你了，大哥哥，让我做什么都行！"

# game/BKcity_events.rpy:3642
translate chinese_simplified city_rob_dae0ec55:

    # you "..."
    you "..."

# game/BKcity_events.rpy:3644
translate chinese_simplified city_rob_f609fb29:

    # "You take a good look at her. She's got a nice figure."
    "你终于有功夫仔细观察她。她的身材很好，颜值也不低。"

# game/BKcity_events.rpy:3646
translate chinese_simplified city_rob_5d1f9b1a:

    # you "Anything, uh?"
    you "你说你什么都愿意做？"

# game/BKcity_events.rpy:3650
translate chinese_simplified city_rob_c85476fc:

    # woman "Yes..."
    woman "是的..."

# game/BKcity_events.rpy:3652
translate chinese_simplified city_rob_29eb521d:

    # you "Follow me, then."
    you "好吧，跟我来，我来给你好好上一课。"

# game/BKcity_events.rpy:3660
translate chinese_simplified city_rob_a0830602:

    # "You decide to charm your way out of this."
    "你决定用你的人格魅力解决问题。"

# game/BKcity_events.rpy:3662
translate chinese_simplified city_rob_4dd3fc1e:

    # you "Well, hello, my lady. *smile*"
    you "幸会，美丽的小姐。*微笑*"

# game/BKcity_events.rpy:3670
translate chinese_simplified city_rob_ac221403:

    # you "I, uh... I bet your dad was a thief too!"
    you "我，嗯....我猜你的父亲也是个大盗！"

# game/BKcity_events.rpy:3674
translate chinese_simplified city_rob_2497c403:

    # woman "Oh, really, why?"
    woman "哦，为什么这样想？"

# game/BKcity_events.rpy:3676
translate chinese_simplified city_rob_1c0eb605:

    # you "Because he stole all of the stars in the night sky... And put them into your eyes! Hahaha... *nervous*"
    you "因为他偷走了夜空中所有的星星....然后放进了你的眼睛里！你的眼睛是如此迷人....*紧张*"

# game/BKcity_events.rpy:3678
translate chinese_simplified city_rob_2a5463be:

    # woman "..."
    woman "..."

# game/BKcity_events.rpy:3680
translate chinese_simplified city_rob_f1dad0a2:

    # you "Stars? In your eyes? It means you have beautiful eyes, you see... get it?"
    you "这是一种比喻，你看，你的眼睛里有星星，星星会闪烁，你的眼睛也....你明白了吗？"

# game/BKcity_events.rpy:3684
translate chinese_simplified city_rob_54730e63:

    # "Her eyes narrow. She raises her weapon and points it at your face."
    "她吐了口唾沫。举起武器指着你的脸。"

# game/BKcity_events.rpy:3688
translate chinese_simplified city_rob_0cee55a9:

    # woman "I should kick your ass just for using such a cheesy pick up line!!! Hand over your purse, NOW!"
    woman "我应该对着你的屁股来上一脚，你的情话土得让我想吐！！现在把你的钱包交出来！"

# game/BKcity_events.rpy:3690
translate chinese_simplified city_rob_d6d23918:

    # you "Aw..."
    you "哦..."

# game/BKcity_events.rpy:3692
translate chinese_simplified city_rob_808e866c:

    # woman "And make sure to empty your pockets, too..."
    woman "还有口袋，把裤子和衣服的口袋都翻出来...."

# game/BKcity_events.rpy:3696
translate chinese_simplified city_rob_aa255b8d:

    # "You reluctantly hand over all your gold."
    "你不情愿地交出了所有的金币。"

# game/BKcity_events.rpy:3700
translate chinese_simplified city_rob_19cd19c7:

    # woman "So long, sucker! *laugh*"
    woman "翻个口袋都要这么久！*笑*"

# game/BKcity_events.rpy:3712
translate chinese_simplified city_rob_04964a88_1:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:3718
translate chinese_simplified city_rob_3badc2a5:

    # you "I know you."
    you "我认识你。"

# game/BKcity_events.rpy:3720
translate chinese_simplified city_rob_1bd51fc7:

    # woman "You do?"
    woman "你认识他?"

# game/BKcity_events.rpy:3722
translate chinese_simplified city_rob_bf9426fa:

    # you "You're the one they call the queen of bandits, aren't you? Whose beauty and cunning is the talk of all the town?"
    you "你就是那个传说中的女飞贼，不是吗？大家都在说的那个美貌与智慧并存的女飞贼？"

# game/BKcity_events.rpy:3728
translate chinese_simplified city_rob_ccc043dc:

    # woman "Well, err..."
    woman "呃,嗯..."

# game/BKcity_events.rpy:3732
translate chinese_simplified city_rob_8be76df3:

    # woman "Yes! That's me, of course, haha!"
    woman "没错！正是在下，哈哈！"

# game/BKcity_events.rpy:3734
translate chinese_simplified city_rob_167fe98a:

    # you "They say you got out of a thousand traps, and evaded arrest a hundred times! You made a mockery of the guard and are the darling of the people..."
    you "他们说没有机关能拦住你，你从卫兵手中逃过超出一百次的追捕！你戏耍着卫兵，劫富济贫...."

# game/BKcity_events.rpy:3736
translate chinese_simplified city_rob_bc36bcc4:

    # woman "Oh, well... This is a little exaggerated... *blush*"
    woman "哦，好吧....可能有点小夸张....*脸红*"

# game/BKcity_events.rpy:3738
translate chinese_simplified city_rob_f413dc9b:

    # you "They also said you were a real beauty, and now I can see they were telling the truth..."
    you "他们还说你是就是美丽的化身，现在我才明白他们说的是实话...."

# game/BKcity_events.rpy:3742
translate chinese_simplified city_rob_95b67283:

    # woman "Oh, what a gentleman... Ok, you may go. Spread the word of my generosity in those taverns of yours, will ya!"
    woman "哦，说到我心坎里去了....好吧，你可以走了。别忘了喝酒的时候也向其他人介绍介绍我，好吗？"

# game/BKcity_events.rpy:3744
translate chinese_simplified city_rob_69cf745b:

    # you "Of course, my lady. *smile*"
    you "当然，美丽的小姐。*微笑*"

# game/BKcity_events.rpy:3748
translate chinese_simplified city_rob_f02a6c95:

    # "You head back to the [loc] without incident."
    "你平安返回[loc]。"

# game/BKcity_events.rpy:3752
translate chinese_simplified city_rob_3c8f2fe4:

    # you "Are you really going to rob me? A second time?"
    you "你真的要抢劫我吗？又一次？"

# game/BKcity_events.rpy:3756
translate chinese_simplified city_rob_f34ed0f0:

    # woman "What do you mean, a second time?"
    woman "你说又一次是什么意思？"

# game/BKcity_events.rpy:3760
translate chinese_simplified city_rob_ed8348e6:

    # you "Because you already stole my heart..."
    you "因为你已经偷走了我的心...."

# game/BKcity_events.rpy:3764
translate chinese_simplified city_rob_88cdb07c:

    # woman "Uh, what?"
    woman "哦，什么？"

# game/BKcity_events.rpy:3766
translate chinese_simplified city_rob_24965e94:

    # you "I haven't seen a girl so fair in this entire kingdom. My lady, you're the jewel of the [loc]."
    you "我在这整个王国里都没见过如此美丽的姑娘。我的女士，您是[loc]的瑰宝。"

# game/BKcity_events.rpy:3768
translate chinese_simplified city_rob_3ae8a6e1:

    # woman "I am... What?"
    woman "我是....什么？"

# game/BKcity_events.rpy:3770
translate chinese_simplified city_rob_7cfbceb6:

    # you "Your fair skin and beautiful smile is reason enough for anyone to drop at your feet in adoration."
    you "你白皙的皮肤和美丽的笑容足以让任何人拜倒在你的脚下。"

# game/BKcity_events.rpy:3772
translate chinese_simplified city_rob_de534082:

    # "You take a knee."
    "你单膝跪地。"

# game/BKcity_events.rpy:3776
translate chinese_simplified city_rob_04736b7f:

    # woman "Oh..."
    woman "哦..."

# game/BKcity_events.rpy:3780
translate chinese_simplified city_rob_d6432bc2:

    # woman "No one... No one has ever talked to me like this..."
    woman "没有人....从来没有人这样跟我说话...."

# game/BKcity_events.rpy:3782
translate chinese_simplified city_rob_55455558:

    # woman "You're quite handsome yourself, if I may say so... *blush*"
    woman "你长得也不赖，如果我可以这么说的话....*脸红*"

# game/BKcity_events.rpy:3784
translate chinese_simplified city_rob_de158ee5:

    # you "My lady, you're even more beautiful when you're blushing.."
    you "小姐，你脸红的时候更加迷人了。"

# game/BKcity_events.rpy:3788
translate chinese_simplified city_rob_33cafd22:

    # woman "Oh, you devil..."
    woman "哦，你可真会说话...."

# game/BKcity_events.rpy:3790
translate chinese_simplified city_rob_8ad91dba:

    # you "Come on, why don't we move from this wretched place to somewhere more... Comfortable?"
    you "来吧，为什么要在这个令人不适的地方聊天呢....和我一起去个更加舒服的地方如何？"

# game/BKcity_events.rpy:3792
translate chinese_simplified city_rob_5a0ab639:

    # "You offer your arm. She looks on hesitantly."
    "你伸出手臂邀请。她犹豫地看着。"

# game/BKcity_events.rpy:3796
translate chinese_simplified city_rob_13cec699:

    # "Eventually, she smiles and sheathes her weapon. She takes your arm and you lead her away from the [loc]."
    "最后，她露出笑容，收起了武器。她挽着你的胳膊，你带着她离开了[loc]。"

# game/BKcity_events.rpy:3805
translate chinese_simplified city_rob_e25d6411:

    # "You decide it's best to avoid a confrontation."
    "你决定避免与她发生正面冲突。"

# game/BKcity_events.rpy:3809
translate chinese_simplified city_rob_a9d9bd91:

    # you "I don't want no trouble. Here, have this."
    you "我不想惹麻烦。拿到钱就赶快离开吧。"

# game/BKcity_events.rpy:3813
translate chinese_simplified city_rob_fdc59ac9:

    # woman "Good, very good! I knew you were a reasonable man."
    woman "好，很好！我就知道你是个明事理的人。"

# game/BKcity_events.rpy:3825
translate chinese_simplified city_rob_04964a88_2:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:3842
translate chinese_simplified city_rob_da6db8c0:

    # woman "Mister... Oh!!!"
    woman "你怎么这么猛，好大....哦！！！"

# game/BKcity_events.rpy:3844
translate chinese_simplified city_rob_9f8fdc49:

    # you "Who would have thought... The queen of bandits is also the queen of sluts... Hmmpf..."
    you "魁梧的身躯往下落，弯曲粗壮的肉棒狠狠的插入她的小穴内，一插到底。她后仰着白皙的天鹅颈，颈项上经脉凸出，仰着臻首发出淫叫，被这一插给插得魂飞魄散。"

# game/BKcity_events.rpy:3846
translate chinese_simplified city_rob_28d5f91b:

    # woman "Oh, ah, don't say that... *blush*"
    woman "每一下都是势大力沉的重击，响亮的啪啪声异常清脆悦耳，她被撞得翻眼白，先是双手双脚死死的抵住床面，大张着腿承受魁梧精壮的你的猛插，每次抽插都是水流不止...."

# game/BKcity_events.rpy:3848
translate chinese_simplified city_rob_8148c643:

    # you "I am not finished with you..."
    you "她似乎受不住了，变为双手双脚的缠住你。但你没有抱住她，反而继续紧绷着屁股，狠命的顶撞她。每一下都是凶狠的顶撞，让肉棒重重的插入，龟头顶到穴内深处的花芯上...."

# game/BKcity_events.rpy:3852
translate chinese_simplified city_rob_369f454d:

    # "You keep teasing and fucking her in different positions until she reaches her limit."
    "你喘着粗气，没有去看她，身躯往下压在她身上，开始快速有力的抽插。粗壮的肉棒染上一层白浆，在美丽的蜜穴内飞速进出。"

# game/BKcity_events.rpy:3856
translate chinese_simplified city_rob_70db8e4f:

    # woman "Oh, no... I'm cumming again, aaaah!"
    woman "哦，不....我要被干死了，饶了我吧好哥哥，啊！"

# game/BKcity_events.rpy:3862
translate chinese_simplified city_rob_a185120a:

    # "She squirts like a fountain as she cums once more. Exhausted, she falls to the floor in a pool of bodily fluids."
    "被插得红肿的肥嫩白虎穴，沾满了白沫，肉唇颤动了几下后，猛地张开，喷出一大股的汁液。"

# game/BKcity_events.rpy:3868
translate chinese_simplified city_rob_f0349cbd:

    # woman "Aaaaah..."
    woman "啊啊..."

# game/BKcity_events.rpy:3879
translate chinese_simplified city_rob_b21c27d9:

    # "You let her rest for a while at the brothel, then send her on her way."
    "她被日得从手足无措的尖叫，到手脚痉挛的发出荷荷喘息声，最后，她竟然用双足紧紧踩在床面，下体用力往上抬，你扎着马步半蹲着顶她的小穴。滋滋水声中，一道道黏腻的汁液从半空中两人交合的地方溅下。"

# game/BKcity_events.rpy:3883
translate chinese_simplified city_rob_03eece81:

    # "After you have your way with her, you have her thrown out into the street, naked and reeking of semen."
    "当你完事之后，你把她直接丢到了街上。她全身赤裸，大腿上不断地有白色精液流下，她一边傻笑一边趴下撅起屁股，左右摇摆向你献媚。"

# game/BKcity_events.rpy:3885
translate chinese_simplified city_rob_a9baabcd:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:3891
translate chinese_simplified city_rob_fd3aa280:

    # "You make your escape while she lays there sleeping."
    "她奄奄一息地趴在那，小穴里精液不断地流出，形成一团白色的水坑。嘴里还嘀咕着“肉棒，最棒了！”。你提起裤子，留下她一个人在那里。"

# game/BKcity_events.rpy:3893
translate chinese_simplified city_rob_a9baabcd_1:

    # "You have earned prestige."
    "你获得了一些声望。"

# game/BKcity_events.rpy:3906
translate chinese_simplified city_ambush_e2c1b18e:

    # "Walking through the [loc] looking for a shortcut, you suddenly get a sinking feeling in your stomach."
    "穿过[loc]寻找捷径时，你突然感到胃里一阵翻腾。"

# game/BKcity_events.rpy:3926
translate chinese_simplified city_ambush_334cf4ff:

    # "You hear a commotion, and turn to see a vicious thug rushing towards you."
    "远处传来一阵骚动，你转身看见一个凶恶的暴徒朝你冲来。"

# game/BKcity_events.rpy:3934
translate chinese_simplified city_ambush_6f7c2d10:

    # "It's an ambush!"
    "这是埋伏？还是一群走投无路的歹徒呢？"

# game/BKcity_events.rpy:3946
translate chinese_simplified city_ambush_12bd2f28:

    # "You won't give up without a fight."
    "不管他是什么人，你都不会坐以待毙。"

# game/BKcity_events.rpy:3950
translate chinese_simplified city_ambush_fd39755e:

    # you "Bring it on!"
    you "一起上吧！"

# game/BKcity_events.rpy:3958
translate chinese_simplified city_ambush_73e99845:

    # "You exchange quick blows with the ruffian, matching each of his assaults with an attack of your own."
    "你和歹徒迅速地对了一拳，须臾之间你们挥出十余拳，双方不相上下，拳风四溅呼呼作响。"

# game/BKcity_events.rpy:3982
translate chinese_simplified city_ambush_155ea4f8:

    # "Your last blow is enough to make your opponent lose his footing momentarily. Seeing an opening, you plunge and strike a deadly blow."
    "你抓住他的破绽，用胳膊招架住他的冲拳，向外一推让他失去平衡。你把握住机会，给了他致命的一击。"

# game/BKcity_events.rpy:3992
translate chinese_simplified city_ambush_d2daa834:

    # "Bandit" "Aaargh!!!"
    "土匪" "啊啊啊！！！"

# game/BKcity_events.rpy:3994
translate chinese_simplified city_ambush_f134b62c:

    # "You opponent falls flat into the dirt, lifeless. The rest of the scum scatters in fear."
    "你的对手倒在地上，失去生机。其余的混混在恐惧中四散奔逃。"

# game/BKcity_events.rpy:3998
translate chinese_simplified city_ambush_608c2c15:

    # you "Anybody else wants to negotiate?"
    you "还有人想和我的拳头谈谈吗？"

# game/BKcity_events.rpy:4006
translate chinese_simplified city_ambush_7e9b4a3d:

    # "You escape unscathed and reach the relative safety of the [loc]."
    "你毫发无损地逃脱，到达了相对安全的[loc]。"

# game/BKcity_events.rpy:4020
translate chinese_simplified city_ambush_9bb797a1:

    # "You dodge an incoming blow and barely manage to parry another one."
    "你躲过了一个歹徒的攻击，却没能躲过另一个。"

# game/BKcity_events.rpy:4022
translate chinese_simplified city_ambush_6e12d02c:

    # you "Damn you, lowlife!"
    you "卑鄙小人，就只会偷袭吗！"

# game/BKcity_events.rpy:4024
translate chinese_simplified city_ambush_a1f47868:

    # "As you prepare to retaliate, however, an unseen opponent hits you from behind with a sap."
    "然而，当你准备反击时，一个看不见的敌人从后面用木棍对着你的后脑勺来了一棍。"

# game/BKcity_events.rpy:4030
translate chinese_simplified city_ambush_666400aa:

    # you "Ouch!!!"
    you "哎哟！！！"

# game/BKcity_events.rpy:4036
translate chinese_simplified city_ambush_16f34bd0:

    # "You see stars and fall down on the ground. Everything turns to black."
    "你眼冒金星。视野逐渐变黑。"

# game/BKcity_events.rpy:4044
translate chinese_simplified city_ambush_123ca023:

    # "You wake hours later, with a crippling headache and an empty purse."
    "几个小时后你醒了过来，头痛欲裂，钱包也空了。"

# game/BKcity_events.rpy:4056
translate chinese_simplified city_ambush_b73ae8cf:

    # "You have lost %(loss)d gold. You are wounded and lose your remaining actions for the day."
    "你损失了 %(loss)d 枚金币。你受伤了，你耗尽了今天的行动力。"

# game/BKcity_events.rpy:4067
translate chinese_simplified city_ambush_c89cbe27:

    # you "Come on, punk, make my day."
    you "来吧，别让我感到无聊。"

# game/BKcity_events.rpy:4069
translate chinese_simplified city_ambush_60b31a61:

    # "Staff in hand, you stand amongst your opponents, reciting the words of a powerful spell."
    "手持权杖，站在歹徒面前，你吟唱了一个强大的咒语。"

# game/BKcity_events.rpy:4079
translate chinese_simplified city_ambush_784dff99:

    # "A whirlwind of magic engulfs you, blinding your opponents."
    "魔法旋风吞没了他们，刮起的风沙让他们几乎失明。"

# game/BKcity_events.rpy:4081
translate chinese_simplified city_ambush_6b56f577:

    # you "THUNDER!!!"
    you "雷霆万钧！！！"

# game/BKcity_events.rpy:4104
translate chinese_simplified city_ambush_8f83324b:

    # "Chain lightning strikes all around you. Your opponent is hit and burnt to a crisp."
    "电闪雷鸣。你的对手被击中并烧成焦炭。"

# game/BKcity_events.rpy:4108
translate chinese_simplified city_ambush_99d30435:

    # man "Aha-ha-haw!"
    man "妈妈!"

# game/BKcity_events.rpy:4110
translate chinese_simplified city_ambush_ad1f7aea:

    # "Overwhelmed with shock and awe, the surviving bandits scatter in all directions, running for their lives."
    "幸存的歹徒惊恐万分，四散奔逃。"

# game/BKcity_events.rpy:4112
translate chinese_simplified city_ambush_1f90cc0b:

    # you "You've been... Thunderstruck!"
    you "你已经....受到了神罚！"

# game/BKcity_events.rpy:4118
translate chinese_simplified city_ambush_dc842bb3:

    # "You return to the [loc] without further incident."
    "你顺利返回[loc]，没有发生其他意外。"

# game/BKcity_events.rpy:4122
translate chinese_simplified city_ambush_06fdb63b:

    # you "KA!!!"
    you "雷！！！"

# game/BKcity_events.rpy:4124
translate chinese_simplified city_ambush_38d3feab:

    # you "ME!!!"
    you "霆！！！"

# game/BKcity_events.rpy:4126
translate chinese_simplified city_ambush_a634c3b3:

    # you "HA..."
    you "万...."

# game/BKcity_events.rpy:4130
translate chinese_simplified city_ambush_7e6d9cc5:

    # "Before you get a chance to finish your incantation, however, your opponent is upon you."
    "可惜，在你吟唱完你的咒语之前，他们就已经出手了。"

# game/BKcity_events.rpy:4138
translate chinese_simplified city_ambush_2537f429:

    # you "ME... OUCH!!!"
    you "我....哎哟！！！"

# game/BKcity_events.rpy:4142
translate chinese_simplified city_ambush_74f77e89:

    # "The rogue hits you in the stomach, knocking you right out of breath."
    "歹徒一拳打在你的肚子上，把你打得上气不接下气。"

# game/BKcity_events.rpy:4144
translate chinese_simplified city_ambush_83f807e5:

    # man "Disarm him!"
    man "别让他有机会还手！"

# game/BKcity_events.rpy:4150
translate chinese_simplified city_ambush_39753097:

    # "The thugs kick your staff out of your hands, and proceed to give a good old-fashioned beating."
    "歹徒把你的法杖踢到一边，并继续对你进行殴打。"

# game/BKcity_events.rpy:4168
translate chinese_simplified city_ambush_4777ce1d:

    # you "Ouch! Arrh! Raah!!!"
    you "哎哟！啊！啊哈！！！"

# game/BKcity_events.rpy:4174
translate chinese_simplified city_ambush_c1b11258:

    # "They keep going until you have no fight left inside of you."
    "他们会一直打下去，直到你放弃反抗。"

# game/BKcity_events.rpy:4176
translate chinese_simplified city_ambush_ba73a5fa:

    # you "Stop..."
    you "停下...."

# game/BKcity_events.rpy:4180
translate chinese_simplified city_ambush_d9d7a526:

    # "They leave you crumpled on the floor in a sorry state, with an empty purse and an empty bladder."
    "他们把你丢在地上，你的衣服破破烂烂，钱包也被夺走了。"

# game/BKcity_events.rpy:4182
translate chinese_simplified city_ambush_e37b1b31:

    # you "*sob*"
    you "*哭泣*"

# game/BKcity_events.rpy:4198
translate chinese_simplified city_ambush_b73ae8cf_1:

    # "You have lost %(loss)d gold. You are wounded and lose your remaining actions for the day."
    "你损失了 %(loss)d 枚金币。你受伤了，你耗尽了今天的行动力。"

# game/BKcity_events.rpy:4209
translate chinese_simplified city_ambush_de5538e6:

    # you "Aaaaaah!!!!"
    you "啊！！！！"

# game/BKcity_events.rpy:4215
translate chinese_simplified city_ambush_2bb7d284:

    # "You run away as quickly as possible, dodging throwing knives and arrows."
    "你转身拔腿就跑，躲避朝你袭来的飞刀和箭矢。"

# game/BKcity_events.rpy:4217
translate chinese_simplified city_ambush_addf0ba7:

    # you "Damn you!"
    you "该死！"

# game/BKcity_events.rpy:4219
translate chinese_simplified city_ambush_368e7679:

    # "The bandits are hot on your trail. You have to find a way to throw them off."
    "强盗们紧追不舍。你必须想办法摆脱他们。"

# game/BKcity_events.rpy:4221
translate chinese_simplified city_ambush_027edb5f:

    # you "I guess I have no choice..."
    you "我想我别无选择...."

# game/BKcity_events.rpy:4223
translate chinese_simplified city_ambush_19fc3801:

    # "Reaching for your purse, you start throwing denars into the air."
    "你掏出钱包，朝着空中洒出金币。"

# game/BKcity_events.rpy:4227
translate chinese_simplified city_ambush_0a1db7f4:

    # man "Gold! It's gold!"
    man "金币！是金币！"

# game/BKcity_events.rpy:4229
translate chinese_simplified city_ambush_33b60461:

    # "As expected, the men soon give up on their pursuit, stopping to gather the gold you've left behind."
    "不出所料，这些人很快就停下追你的步伐，弯下腰来捡你丢出的金币。"

# game/BKcity_events.rpy:4235
translate chinese_simplified city_ambush_3599bd33:

    # you "That was close..."
    you "真险啊，差点就被...."

# game/BKcity_events.rpy:4247
translate chinese_simplified city_ambush_04964a88:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:4260
translate chinese_simplified city_mob_533141a1:

    # "After an uneventful visit to the [loc], you find a large mob barring the road on your way back."
    "在对[loc]进行了一次平静的访问后，你发现回程路上有一大群暴徒挡住了道路。"

# game/BKcity_events.rpy:4278
translate chinese_simplified city_mob_cd39c36d:

    # man "Down! Down with demon worshippers and heretics!"
    man "打倒他们！打倒恶魔的崇拜者和异教徒！"

# game/BKcity_events.rpy:4280
translate chinese_simplified city_mob_35eba98f:

    # man "They hide amongst our people, spreading vice and corruption in their filthy taverns and whorehouses..."
    man "他们就躲藏在我们之中，他们在肮脏的酒馆和青楼里扩散邪恶，腐化心灵...."

# game/BKcity_events.rpy:4282
translate chinese_simplified city_mob_853d1124:

    # "The crowd cheers frantically. You try to push through, but the mob is unwilling to let you pass."
    "人群疯狂地呐喊。你想挤过去，但是他们太过拥挤，你寸步难行。"

# game/BKcity_events.rpy:4286
translate chinese_simplified city_mob_e07c58f0:

    # "Somebody grabs you by the collar."
    "有人抓住你的衣领。"

# game/BKcity_events.rpy:4288
translate chinese_simplified city_mob_b510dc91:

    # man "Wait!!! I recognize him!"
    man "等等！！！我认得他！"

# game/BKcity_events.rpy:4292
translate chinese_simplified city_mob_6c748b57:

    # man "He's one of the brothel owners! The scum of [place]!!!"
    man "他竟然是妓院老板之一！真是[place]的渣滓！！！"

# game/BKcity_events.rpy:4294
translate chinese_simplified city_mob_b7447831:

    # "An angry chatter emerges from the crowd. People are blocking your way on every side, some trying to get ahold of you."
    "人群中传出愤怒的咆哮。人们把你团团围住，有些人还朝你丢烂番茄和臭鸡蛋。"

# game/BKcity_events.rpy:4296
translate chinese_simplified city_mob_747efbfb:

    # "You push back desperately. They are too many for you to fight them off, however."
    "你拼命地推开人群。然而，他们人多势众，你毫无办法。"

# game/BKcity_events.rpy:4298
translate chinese_simplified city_mob_6dc22a32:

    # you "Fellas, I..."
    you "伙计们，我...."

# game/BKcity_events.rpy:4311
translate chinese_simplified city_mob_3175b40d:

    # man "You! Confess! You're a devil worshipper, aren't you! A mage, a witch! Maybe even a druid!"
    man "你！承认吧！你是个魔鬼崇拜者，不是吗？还是说你是一个法师，一个巫师！甚至可能是德鲁伊！"

# game/BKcity_events.rpy:4313
translate chinese_simplified city_mob_6527bb82:

    # you "You're making a mistake, good sir..."
    you "你弄错了，这是个误会...."

# game/BKcity_events.rpy:4315
translate chinese_simplified city_mob_bc5fe762:

    # "Waving your hand in front of him, you cast a minor controlling spell."
    "你在他面前挥挥手，悄悄地释放了一个精神控制咒。"

# game/BKcity_events.rpy:4317
translate chinese_simplified city_mob_b6e89e07:

    # you "You need to let me go. I am not the one you are looking for."
    you "让我离开。我不是你要找的那个人。"

# game/BKcity_events.rpy:4327
translate chinese_simplified city_mob_7c0d9d13:

    # man "We... need to let him go. This is not the druid we are looking for."
    man "我们....我得让他走。他不是我们要找的德鲁伊。"

# game/BKcity_events.rpy:4329
translate chinese_simplified city_mob_c30de29a:

    # "The crowd wavers in surprise. Some people question the man's judgement."
    "人群因惊讶而沉默。有些人质疑他的判断。"

# game/BKcity_events.rpy:4331
translate chinese_simplified city_mob_7abda4d9:

    # man "I said let him go! It's not him. End of story."
    man "我说让他走！我们找错人了，到此为止吧，别让太阳神为我们蒙羞。"

# game/BKcity_events.rpy:4333
translate chinese_simplified city_mob_918b6e4a:

    # "People grumble and curse, but they respect the elder's command. They reluctantly let you go, and you make it out more or less unharmed."
    "人们不甘地攥紧了拳头，但他们尊重长者的命令。他们不情愿地让你走了，而你安然无恙。"

# game/BKcity_events.rpy:4339
translate chinese_simplified city_mob_304746b8:

    # man "Oh, but I know who you are! Heretic!!! We'll give you a lesson you won't forget!"
    man "哦，但我知道你是谁！异教徒！！！让我们好好教训教训你！"

# game/BKcity_events.rpy:4346
translate chinese_simplified city_mob_1b1b8f26:

    # "Stepping on a nearby crate, you turn to face the angry crowd."
    "踏上附近的板条箱，你转身面对愤怒的人群。"

# game/BKcity_events.rpy:4348
translate chinese_simplified city_mob_4aa1c879:

    # you "Good people of Zan! Listen to me."
    you "泽恩的人民！听我说。"

# game/BKcity_events.rpy:4356
translate chinese_simplified city_mob_20e93590:

    # you "Yes, brothels are places of sin and debauchery..."
    you "是的，青楼也许是个罪恶和放荡的地方...."

# game/BKcity_events.rpy:4358
translate chinese_simplified city_mob_db0ad6cd:

    # "The crowd grumbles menacingly."
    "人群中传来许多抱怨。"

# game/BKcity_events.rpy:4360
translate chinese_simplified city_mob_ffa329fc:

    # you "But think about it: whoring has been, is and will always be a part of our lives."
    you "但想想看:无论过去、现在和将来性生活都是人类繁衍存续不可缺少的一环。"

# game/BKcity_events.rpy:4362
translate chinese_simplified city_mob_9e4730f3:

    # you "Have you ever seen a place in Xeros without a hooker? It doesn't exist."
    you "你见过不做爱就能怀孕的女性吗？它不存在。"

# game/BKcity_events.rpy:4364
translate chinese_simplified city_mob_315ed600:

    # "People mutter in agreement. The elder steps forward to confront you."
    "人们咕哝着表示同意。长者走上前来打断你。"

# game/BKcity_events.rpy:4366
translate chinese_simplified city_mob_5f1322bd:

    # man "Lies! Your kind is an abomination in the eyes of Arios!"
    man "你这是强词夺理！在太阳神的眼中，你的这种行为是不洁的！"

# game/BKcity_events.rpy:4368
translate chinese_simplified city_mob_45205e62:

    # you "Oh, really? But what does Arios teach us? To seek the light. If you refuse the sex trade to be out in the open, it will only move to the shadows. Into the realm of Shalia, far from Arios's reach. Is that what you want?"
    you "哦，真的吗？但是太阳神想让人们做什么呢？去寻找光明。如果你扼杀掉人类本能的欲望，他们只会放弃信仰，转而投奔暗影女神莎莉娅。远离太阳神，这就是你想要的吗？"

# game/BKcity_events.rpy:4370
translate chinese_simplified city_mob_924122b6:

    # man "Well, err... No, I meant..."
    man "好吧，呃....不，我的意思是...."

# game/BKcity_events.rpy:4372
translate chinese_simplified city_mob_b0ccecda:

    # "The crowd is warming up to your argument."
    "群众冷静下来思考，觉得你说的似乎很有道理。"

# game/BKcity_events.rpy:4374
translate chinese_simplified city_mob_af0457ee:

    # you "Who here hasn't sinned at least once? As long as brothels remain out in the open, they are sanctuaries of the Light just like any other place."
    you "这里谁是完全干净的？青楼就像其他地方一样，是光明的，是受太阳神庇佑的。"

# game/BKcity_events.rpy:4376
translate chinese_simplified city_mob_9577fc55:

    # "Men in the crowd start to show approval. The elder is struggling to find arguments, but the mob isn't listening to him anymore."
    "人群中有些人表示赞同。老头儿急的直挠头，但群众已经不再听从他了。"

# game/BKcity_events.rpy:4378
translate chinese_simplified city_mob_d82dedae:

    # you "Very well, friends, now you see why brothels are so important. Come to the brothel, tonight, and there will be a special discount for Arios worshippers!"
    you "很好，朋友们，现在你们明白青楼为什么那么重要了。今晚到青楼来吧，太阳神的信徒会享有特别的折扣！"

# game/BKcity_events.rpy:4382
translate chinese_simplified city_mob_8220b2d2:

    # "The crowd cheers and applauds your speech. The elder steps back, looking dejected."
    "观众为你的演讲鼓掌喝彩。长者落寞地转身离去，看上去很沮丧。"

# game/BKcity_events.rpy:4388
translate chinese_simplified city_mob_e4deca1f:

    # you "He lies!"
    you "他在撒谎！"

# game/BKcity_events.rpy:4390
translate chinese_simplified city_mob_64c93e8c:

    # you "My establishment is not a whorehouse! It is a reputable [cover], and I am a lawful citizen!"
    you "我不是开青楼的！这是一个合法合规的[cover]，我是一个合法的公民！"

# game/BKcity_events.rpy:4392
translate chinese_simplified city_mob_1fbc1693:

    # man "It is a brothel! I went there on a, ahem, an errand, and I saw half-naked girls rubbing themselves against a customer. And the prices were outrageous, too! Not that I'd care, of course, but..."
    man "那就是青楼！我曾经路过那里，我看到几个衣衫不整的女孩在一个顾客身上蹭来蹭去。而且价格也太离谱了！当然，这与我无关，但是...."

# game/BKcity_events.rpy:4394
translate chinese_simplified city_mob_97c895b4:

    # "Other witnesses come out of the crowd. Accusations fly. No one seems to believe you."
    "其他目击者陆续从人群中走出来指证你。似乎没有人相信你。"

# game/BKcity_events.rpy:4396
translate chinese_simplified city_mob_cb8bc9d2:

    # you "Good people!"
    you "大家听我解释！"

# game/BKcity_events.rpy:4398
translate chinese_simplified city_mob_f99b3da1:

    # "Your cries of protest are drowned by angry booing and jeering."
    "你抗议的呼声被愤怒的嘘声和嘲笑淹没了。"

# game/BKcity_events.rpy:4402
translate chinese_simplified city_mob_3be50249:

    # man "Catch him!"
    man "抓住他！"

# game/BKcity_events.rpy:4408
translate chinese_simplified city_mob_0a5f47cd:

    # you "Oh, wait! What's this?"
    you "哦，等等！这是什么？"

# game/BKcity_events.rpy:4410
translate chinese_simplified city_mob_eff675e3:

    # "Opening your pouch, you throw a fistful of gold in the air."
    "你打开了你的钱包，你抓起一把金币扔到空中。"

# game/BKcity_events.rpy:4416
translate chinese_simplified city_mob_cbb9609b:

    # "Forgetting all about heretics and righteousness, the crowd erupts into a massive brawl, trying to grab some of the money."
    "所有人此刻都忘记了什么异端和正义，人群扭打成一团，都在抢地上的金币。"

# game/BKcity_events.rpy:4418
translate chinese_simplified city_mob_00a55a35:

    # man "People of Zan! Listen! Listen to me!!!"
    man "大家别乱！听！听我说！！！"

# game/BKcity_events.rpy:4420
translate chinese_simplified city_mob_56f56aa2:

    # "The man is unable to restore order. He gives you a murderous look. You wink at him and elbow your way out of the crowd, safely retreating towards the [loc]."
    "那人无法恢复秩序。他恶狠狠地瞪了你一眼。你朝他眨了眨眼，用胳膊肘挤出人群，安全地退回到[loc]。"

# game/BKcity_events.rpy:4436
translate chinese_simplified city_mob_04964a88:

    # "You have lost %(loss)d gold."
    "你损失了 %(loss)d 枚金币。"

# game/BKcity_events.rpy:4456
translate chinese_simplified city_mob_e57edd9b:

    # "Deprived of a scapegoat, the mob instead corners a group of women pilgrims."
    "没有了替罪羊，暴徒们把怒火宣泄在一群女人身上。"

# game/BKcity_events.rpy:4474
translate chinese_simplified city_mob_d90b21cb:

    # "They rape them mercilessly, accusing them of withcraft."
    "他们撕碎了她们的衣服，把她们按在地上狠狠地抽插，嘴里还喊着这是太阳神的旨意。"

# game/BKcity_events.rpy:4476
translate chinese_simplified city_mob_8874c0e7:

    # man "That will teach you, you evil demon-worshipping bitches! Hmm..."
    man "给你们一个教训，你们这些崇拜恶魔的婊子！嗯..用力舔.."

# game/BKcity_events.rpy:4488
translate chinese_simplified city_mob_ce040dd1:

    # "You retreat to the safety of the [loc]."
    "你退守到安全的[loc]。"

# game/BKcity_events.rpy:4492
translate chinese_simplified city_mob_a20e77e1:

    # "The angry mob starts beating you, spitting on you and ripping at your clothing as you do your best to escape."
    "愤怒的暴民开始殴打你，向你吐口水，撕扯你的衣服，而你则尽力逃跑。"

# game/BKcity_events.rpy:4510
translate chinese_simplified city_mob_b347d1cf:

    # you "No... Not the face!!!"
    you "不....不要打脸！！！"

# game/BKcity_events.rpy:4512
translate chinese_simplified city_mob_fa2bb8a9:

    # "Protecting yourself the best you can, you barely make it through to the other side of the crowd."
    "你用双臂互助脑袋，但你很难从人群中穿过。"

# game/BKcity_events.rpy:4518
translate chinese_simplified city_mob_19abe4ab:

    # you "Ouch! Aaah!"
    you "哎哟！啊！"

# game/BKcity_events.rpy:4520
translate chinese_simplified city_mob_a6a55e78:

    # "You run off as fast as you can, followed by curses and rotten fruits thrown by the angry mob."
    "你以最快的速度逃跑，愤怒的人群一边追着你一边朝你扔腐烂的水果。"

# game/BKcity_events.rpy:4522
translate chinese_simplified city_mob_10e49672:

    # "You do not stop running until you have reached the safety of your brothel."
    "直到你跑进青楼里，确认身后没有追兵你才停止奔跑。"

# game/BKcity_events.rpy:4524
translate chinese_simplified city_mob_9f1c09dd:

    # "Pausing to catch your breath and lick your wounds, you notice your purse is missing."
    "你停下喘气，包扎伤口，你发现你的钱包不见了。"

# game/BKcity_events.rpy:4526
translate chinese_simplified city_mob_0b364580:

    # you "Hell..."
    you "简直就是地狱...."

# game/BKcity_events.rpy:4542
translate chinese_simplified city_mob_a490fd10:

    # "You have lost %(loss)d gold. You are tired and lose your remaining actions for the day."
    "你损失了%(loss)d枚金币。你精疲力尽，你耗尽了所有的行动力。"

# game/BKcity_events.rpy:4589
translate chinese_simplified city_none_e589ac84:

    # "[text1]"
    "[text1]"

# game/BKcity_events.rpy:4604
translate chinese_simplified city_gossip_c4c8a8d4:

    # "On your way through the [loc], you overhear [text1] from [actor]."
    "在穿过[loc]时，你无意中听到[actor]说了[text1]。"

# game/BKcity_events.rpy:4619
translate chinese_simplified city_luck_7fab135b:

    # "Strolling around the [loc], you notice something shiny on the ground."
    "在[loc]附近闲逛时，你注意到地上有闪闪发光的东西。"

# game/BKcity_events.rpy:4621
translate chinese_simplified city_luck_6eae37c9:

    # you "What's this... Oh! It's a purse of gold! Nice!"
    you "这是什么....哦！这是一袋金币！真走运！"

# game/BKcity_events.rpy:4625
translate chinese_simplified city_luck_d5210ceb:

    # "You have received %(gain)d gold."
    "你获得了 %(gain)d 枚金币。"

# game/BKcity_events.rpy:4629
translate chinese_simplified city_luck_856d3e40:

    # npc "Oh no... Where is it? It must be around here..."
    npc "哦，不....在哪里？一定在这附近...."

# game/BKcity_events.rpy:4631
translate chinese_simplified city_luck_266a6db4:

    # "Someone seems to be looking for their lost belongings."
    "有人似乎在寻找他丢失的东西。"

# game/BKcity_events.rpy:4633
translate chinese_simplified city_luck_012a58c6:

    # npc "Mister, can you help me? I lost my purse... I really need it..."
    npc "先生，你能帮帮我吗？我的钱包丢了....我真的需要它...."

# game/BKcity_events.rpy:4641
translate chinese_simplified city_luck_1545fa72:

    # you "Are you looking for a bag of gold? Maybe one containing, like, %(gain)d gold?"
    you "你在找一袋金币吗？也许是一个含有，比如，%(gain)d 枚金币？"

# game/BKcity_events.rpy:4643
translate chinese_simplified city_luck_0ec82954:

    # npc "Yes, Sir... Have you seen it anywhere?"
    npc "是的，先生....你在哪儿见过它吗？"

# game/BKcity_events.rpy:4645
translate chinese_simplified city_luck_254d3caf:

    # you "Nope. Not at all. Tough luck, friend."
    you "哦很抱歉，我没见到过，朋友。"

# game/BKcity_events.rpy:4647
translate chinese_simplified city_luck_d84c1181:

    # npc "Aw..."
    npc "哦..."

# game/BKcity_events.rpy:4653
translate chinese_simplified city_luck_6218b925:

    # you "Oh, so it's yours... Here, I found this."
    you "哦，原来是你丢的....这里，我找到了这个。"

# game/BKcity_events.rpy:4657
translate chinese_simplified city_luck_af655f88:

    # "You gave back %(gain)d gold."
    "你给了他 %(gain)d 枚金币。"

# game/BKcity_events.rpy:4659
translate chinese_simplified city_luck_d5448762:

    # npc "Oh, thank you! It's good to see that gentlemen still exist in this rotten city."
    npc "噢，谢谢你！很高兴看到在这个腐朽的城市里还有像你这样的绅士。"

# game/BKcity_events.rpy:4704
translate chinese_simplified slave_beach_event_f54a88e7:

    # "By the [loc] is a long stretch of white sand where the people of [dis] come to relax. As you pass by, you spot a familiar sight."
    "在[loc]附近有一片绵长的白色沙滩，[dis]的人们会来这里休闲放松。当你路过时，你会看到一个熟悉的景象。"

# game/BKcity_events.rpy:4711
translate chinese_simplified slave_beach_event_d8076f8d:

    # "You remember that today is her day off."
    "你记得今天是她的休息日。"

# game/BKcity_events.rpy:4713
translate chinese_simplified slave_beach_event_b86cad14:

    # you "Hey, if it isn't [girl.name]. What's up?"
    you "嘿，这不是[girl.name]吗。你怎么在这？"

# game/BKcity_events.rpy:4718
translate chinese_simplified slave_beach_event_125c68e8:

    # "You chat amiably with [girl.name] for a few minutes. She seems pleased."
    "你和[girl.name]愉快地聊了几分钟。她似乎很高兴。"

# game/BKcity_events.rpy:4722
translate chinese_simplified slave_beach_event_74494c95:

    # "You talk with [girl.name] about odds and ends. She relaxes a little."
    "你和[girl.name]谈论了一些零碎的话题。她感到很放松。"

# game/BKcity_events.rpy:4726
translate chinese_simplified slave_beach_event_7b34cb37:

    # "Frowning, you scold [girl.name] on principle for her careless attitude. She looks down."
    "你皱着眉头，斥责[girl.name]没有照顾好自己，她太粗心了。她听你这么说便往身下看。"

# game/BKcity_events.rpy:4729
translate chinese_simplified slave_beach_event_3202ba13:

    # "Looking at [girl.name], you notice that she looks really hot in her tight beach wear... This gives you a few ideas."
    "看着[girl.name]，你注意到她的比基尼十分性感....你有了一个好主意。"

# game/BKcity_events.rpy:4731
translate chinese_simplified slave_beach_event_693314da:

    # you "Listen..."
    you "听...."

# game/BKcity_events.rpy:4736
translate chinese_simplified slave_beach_event_11fdb519:

    # extend "" nointeract
    extend "" nointeract

# game/BKcity_events.rpy:4739
translate chinese_simplified slave_beach_event_0add0ae4:

    # you "The sun is getting high, I wouldn't want you to get sunburnt. Let me help you..."
    you "太阳越来越烈了，我不想让你被晒伤。让我来帮你...."

# game/BKcity_events.rpy:4744
translate chinese_simplified slave_beach_event_dfcfefd5:

    # girl.char "Oil? No!!!"
    girl.char "石油？不！！！"

# game/BKcity_events.rpy:4746
translate chinese_simplified slave_beach_event_3f65e064:

    # "She recoils in horror. You wonder what happened."
    "她吓得往后缩。你想知道发生了什么。"

# game/BKcity_events.rpy:4749
translate chinese_simplified slave_beach_event_c752d3f7:

    # "You remember she hates oil. Damn."
    "你记得她讨厌油。该死。"

# game/BKcity_events.rpy:4754
translate chinese_simplified slave_beach_event_6363db8b:

    # girl.char "Yeah, no, I don't think so. I'll do it myself."
    girl.char "不，我不这么认为。我会自己做。"

# game/BKcity_events.rpy:4756
translate chinese_simplified slave_beach_event_39570823:

    # girl.char "Oh... O-Okay..."
    girl.char "哦……好的……"

# game/BKcity_events.rpy:4759
translate chinese_simplified slave_beach_event_62bfef84:

    # girl.char "Oh, you're right. But you don't need to worry about it, I'll do it myself."
    girl.char "哦，你说得对。不过你不用担心，我会自己做的。"

# game/BKcity_events.rpy:4761
translate chinese_simplified slave_beach_event_5473c3ac:

    # girl.char "Well, it's nice of you to suggest it. Please do."
    girl.char "谢谢你的建议。请便。"

# game/BKcity_events.rpy:4765
translate chinese_simplified slave_beach_event_f6634f7e:

    # "She lays down on her back as you warm the oil between your hands. As you start rubbing it on her exposed skin, you feel some warmth in your pants."
    "你摩擦双手，让防晒油没那么凉，她仰面躺下，两座山峰矗立在你面前。你的双手接触到她肌肤的每一寸，她传来几声嘤咛，身体微微颤抖。"

# game/BKcity_events.rpy:4773
translate chinese_simplified slave_beach_event_d104f8c5:

    # girl.char "Thank you, Master, it's nice..."
    girl.char "谢谢师父，真好……"

# game/BKcity_events.rpy:4775
translate chinese_simplified slave_beach_event_79dc27c9:

    # "Running your hands against her arms, back, then thighs, you turn the oiling into a veritable massage. She doesn't stop you."
    "你的手从脖颈出发，绕过乳峰外缘到达肚脐，再从腰一路滑到大腿，你把涂防晒油变成了按摩，她十分受用。"

# game/BKcity_events.rpy:4779
translate chinese_simplified slave_beach_event_4592937c:

    # "You can feel her muscles relax under your experienced hands, as you inch closer to her erogenous parts. Perhaps you could push your luck further?"
    "你可以感觉到她的肌肉在你熟练地手法下逐渐放松，当你靠近她的私密部位时，也许你可以得寸进尺一下？"

# game/BKcity_events.rpy:4786
translate chinese_simplified slave_beach_event_11fdb519_1:

    # extend "" nointeract
    extend "" nointeract

# game/BKcity_events.rpy:4807
translate chinese_simplified slave_beach_event_47dc25c3:

    # "Pretending you're still massaging her, you bring your hands closer and closer to her buttocks. She says nothing, although you can feel some tension build up."
    "你假借按摩的名义对她上下其手，你的手搭在她的翘臀上。她扭了下屁股，没有反抗，你能感觉到她有些紧张。"

# game/BKcity_events.rpy:4809
translate chinese_simplified slave_beach_event_c3c3d77f:

    # "After pouring more oil on her backside, you suddenly squeeze both of her ass cheeks, bringing her swimsuit upwards so it looks like a thong."
    "手指顺着脊椎一路压下，你突然把手放在她的腰间，把她的泳裤向上一提，泳衣一下子变成了高叉，勒紧了她的小穴。"

# game/BKcity_events.rpy:4814
translate chinese_simplified slave_beach_event_77de4c0a:

    # "As you massage her back, you untie her top, as if by accident. She doesn't react, although you can feel her tense up a little."
    "你又把手挪到她的肩膀下，三下五除二地解开了泳衣的扣子，就好像是不小心蹭到的。她闭上了眼睛，放弃了抵抗。"

# game/BKcity_events.rpy:4816
translate chinese_simplified slave_beach_event_26483073:

    # "Sliding your oily hands around her, you reach for her tits and softly rub her mounds, lightly brushing against her nipples."
    "你用沾满防晒油的手，伸进她胸前的泳衣，粗糙又滑腻的大手在她的一对玉乳上来回摩擦，指尖轻轻地夹着乳首。"

# game/BKcity_events.rpy:4821
translate chinese_simplified slave_beach_event_e99b823d:

    # "Spreading her legs apart under the guise of the massage, you rub the insides of her thighs, inching your way up. She doesn't resist, seemingly waiting to see where this goes next."
    "为了不被别人看出来，你假装按摩大腿，你从足底慢慢地向上摸索，小腿、大腿、股间，最后到了小穴。"

# game/BKcity_events.rpy:4823
translate chinese_simplified slave_beach_event_c88763ca:

    # "Casually, you then push her swimsuit's crotch aside, sliding a couple of fingers inside her."
    "然后，你随意地把她的泳衣下端拉到一边漏出了湿润的小穴，一次塞进了三只手指。"

# game/BKcity_events.rpy:4827
translate chinese_simplified slave_beach_event_721fea8a:

    # you "Let me massage here too..."
    you "小穴里的淋巴按摩一下有助于放松...."

# game/BKcity_events.rpy:4842
translate chinese_simplified slave_beach_event_39c46cf2:

    # "She breathes heavily and her eyes get glassy, but she doesn't stop you as you keep touching her."
    "她呼吸急促，眼神迷茫，但她并没有阻止你的行为。"

# game/BKcity_events.rpy:4849
translate chinese_simplified slave_beach_event_abd95a48:

    # "Moving her swimsuit out of the way, you spread her buttcheeks and expose her gaping asshole. She gasps as you spit in it, using your saliva to continue the massage."
    "把她的泳衣移开，你扒开她的臀瓣，露出她紧致娇嫩的菊穴。你伸出舌头朝着里面探去，她牙关紧闭，你用你的唾液润滑了一下。"

# game/BKcity_events.rpy:4852
translate chinese_simplified slave_beach_event_255fd0af:

    # "Sand gets on her exposed tits as you proceed to fondle them, playfully pinching her nipples. She moans harder every time you do it."
    "当你继续抚摸她充血的乳头时，沙子沾到了她的乳头上，你轻轻地捏着她的乳头。沙子的触感让她更加敏感，你的手指每次摩擦，她都会娇喘一声。"

# game/BKcity_events.rpy:4855
translate chinese_simplified slave_beach_event_a9b98d2f:

    # "Her love juice starts pouring out as you pump two, then three fingers inside her. She arches her butt towards you, as if trying to invite your hand deeper inside her."
    "当你把两个，甚至三个手指放进她的小穴里时，小穴变成汩汩的清泉。她扭动腰肢令臀部更加挺翘，好像想配合你的手指深入她的花心。"

# game/BKcity_events.rpy:4857
translate chinese_simplified slave_beach_event_28be6530:

    # girl.char "Aaaah! [emo_heart]"
    girl.char "啊啊啊！[emo_heart]"

# game/BKcity_events.rpy:4860
translate chinese_simplified slave_beach_event_f853db99:

    # "She loves this and moans louder and louder, enjoying herself tremendously."
    "她开始享受这一切，呻吟声越来越大，越来越放荡。"

# game/BKcity_events.rpy:4865
translate chinese_simplified slave_beach_event_6adefaa7:

    # "She grits her teeth, weakly fighting your touch, confused by how it makes her feel."
    "她咬紧牙关，瘫软的身躯已经无力抵抗，她的神志已经濒临崩溃，只差一点了。"

# game/BKcity_events.rpy:4871
translate chinese_simplified slave_beach_event_7859f076:

    # "Under the hot sun, she seems lost in the sensations that wash over her. The fact that other people may see you increases her arousal. She cannot get enough of this, and her breathing intensifies as you increase your pace."
    "炎炎烈日下，她似乎迷失在那股包裹住她的感觉中。一想到此时可能有很多人正在看着她，她就更加兴奋。她开始变得不满足现状，你的节奏越来越快，她的呼吸越来越急促。"

# game/BKcity_events.rpy:4881
translate chinese_simplified slave_beach_event_23b03eb3:

    # "You can see she is enjoying herself too much, and decide it's time to bring this massage to a happy ending."
    "她的身体扭动地越来越快，是时候给这次按摩画上一个完美的句号了。"

# game/BKcity_events.rpy:4888
translate chinese_simplified slave_beach_event_8a41b161:

    # girl.char "Aaah, ah, AAAAAH!!!"
    girl.char "啊 ..."

# game/BKcity_events.rpy:4892
translate chinese_simplified slave_beach_event_00dded58:

    # "Love juice squirts out of her pussy as you expertly drive her to orgasm. Her whole body shakes, then she falls over the beach towel like a ragdoll, spent."
    "你熟练的手法不断地刺激着她的G点，她的小穴抽搐不止，爱液如潮水般涌出。她全身颤抖，然后像个洋娃娃一样倒在沙滩的毯子上，筋疲力尽。"

# game/BKcity_events.rpy:4906
translate chinese_simplified slave_beach_event_2d7049c8:

    # "She is almost about to cum, and you decide to stop here, to teach her a lesson."
    "感受到她即将高潮，你决定停下来，给她一个教训。"

# game/BKcity_events.rpy:4908
translate chinese_simplified slave_beach_event_36104a17:

    # girl.char "What? Is it over? But..."
    girl.char "什么？结束了吗？可是……"

# game/BKcity_events.rpy:4911
translate chinese_simplified slave_beach_event_255c6ada:

    # "Her face is flushed, and love juice drips down her thighs. It seems like she loves being pushed to the brink."
    "她的脸涨得通红，爱液顺着她的大腿流下来。她似乎喜欢被顶到花心的感觉。"

# game/BKcity_events.rpy:4916
translate chinese_simplified slave_beach_event_ada9ca75:

    # "She is livid, and she looks like she's about to burst into tears. It looks like she hates being denied orgasm like that."
    "她咬着嘴唇，看起来快要哭了。看来她很想高潮。"

# game/BKcity_events.rpy:4927
translate chinese_simplified slave_beach_event_5a190256:

    # you "That was fun..."
    you "真是有趣的景色...."

# game/BKcity_events.rpy:4932
translate chinese_simplified slave_beach_event_cc252f77:

    # "[girl.fullname]'s {b}[stat1]{/b} and {b}[stat2]{/b} have increased."
    "[girl.fullname]的 {b}[stat1]{/b} 和 {b}[stat2]{/b} 提升了"

# game/BKcity_events.rpy:4938
translate chinese_simplified slave_beach_event_48a2c215:

    # you "Damn..."
    you "该死的...."

# game/BKcity_events.rpy:4942
translate chinese_simplified slave_beach_event_4c934cd5:

    # "Disappointed, you take your leave."
    "你感觉到很失望，你打算离开。"

# game/BKcity_events.rpy:4945
translate chinese_simplified slave_beach_event_5b968ef8:

    # "Satisfied with your little massage session, you dry your hands and take your leave. As [girl.name] says goodbye, her face is flushed and she looks flustered. Her sensitivity has increased."
    "在按摩环节结束后，你擦干双手，然后收拾东西离开。当[girl.name]和你道别时，她的脸颊红扑扑的，看起来有些慌乱。她的敏感度提高了。"

# game/BKcity_events.rpy:4952
translate chinese_simplified slave_beach_event_4c934cd5_1:

    # "Disappointed, you take your leave."
    "你感觉到很失望，你打算离开。"

# game/BKcity_events.rpy:4957
translate chinese_simplified slave_beach_event_0ae096ea:

    # you "I'm feeling horny right now. Let's fuck."
    you "天气这么热，你不想一起来点刺激的运动吗？比如做爱。"

# game/BKcity_events.rpy:4959
translate chinese_simplified slave_beach_event_9041689f:

    # girl.char "What?!? Here?"
    girl.char "什么？！这里？"

# game/BKcity_events.rpy:4967
translate chinese_simplified slave_beach_event_ebd18616:

    # you "Yes, here! I don't care who's watching... Let's do it!"
    you "对啊，就在这里！不要在意别人的目光，尽情地享受吧！"

# game/BKcity_events.rpy:4973
translate chinese_simplified slave_beach_event_c7ec28eb:

    # "She pushes you away with surprising strength and runs off the beach."
    "她用惊人的力量把你推开，然后跑出了海滩。"

# game/BKcity_events.rpy:4978
translate chinese_simplified slave_beach_event_f21ba248:

    # girl.char "In... In public? I mean... Sure..."
    girl.char "在……在公共场合？我的意思是……当然……"

# game/BKcity_events.rpy:4980
translate chinese_simplified slave_beach_event_cac033de:

    # "Before she has a chance to change her mind, you push her down and get yourself ready."
    "在她改变主意之前，你把她推倒，脱光了衣服。"

# game/BKcity_events.rpy:4992
translate chinese_simplified slave_beach_event_aab7e42f:

    # "She darts away from you, leaving you looking stupid with a useless boner."
    "她摔开你的手，拿起毯子裹着身体逃走了，留下你一个人赤裸着身体，挺着个肉棒在风中凌乱。"

# game/BKcity_events.rpy:4996
translate chinese_simplified slave_beach_event_1f40409e:

    # you "Let us hide behind some rocks..."
    you "那里有一块礁石，到那后面没人能看到我们的...."

# game/BKcity_events.rpy:4999
translate chinese_simplified slave_beach_event_82c4f436:

    # girl.char "What are you talking about? Who cares about people watching? Do me now!" with vpunch
    girl.char "你在说什么？谁在乎别人看？现在就来！" with vpunch

# game/BKcity_events.rpy:5001
translate chinese_simplified slave_beach_event_b891e11a:

    # "She pushes you down in the sand, almost ripping your pants off."
    "她把你推倒在沙滩上，一把脱下了你的泳裤。"

# game/BKcity_events.rpy:5006
translate chinese_simplified slave_beach_event_71c0b1b5:

    # girl.char "Hiding? Why? It's all the same to me... Let's just stay here."
    girl.char "躲起来？为什么？对我来说都一样……我们就待在这儿吧。"

# game/BKcity_events.rpy:5008
translate chinese_simplified slave_beach_event_cbebd128:

    # "Surprised but aroused by her request, you waste no time before moving on her."
    "你对她的大胆感到惊讶，但这可不是细究的时候，你径直走了过去。"

# game/BKcity_events.rpy:5015
translate chinese_simplified slave_beach_event_2551903f:

    # "You take her hand and lead her behind a large boulder. As soon as you're hidden from the crowd, your hands are all over her."
    "你牵着她的手，把她带到一块礁石后面。确认没人能看到你们，你的手就马上就在她的身上游走起来。"

# game/BKcity_events.rpy:5022
translate chinese_simplified slave_beach_event_a56ec551:

    # "She turns away from you, blushing, hugging her knees and legs tightly pressed together. You know better than to make a scene here."
    "她转过身去，脸色通红，紧紧地抱着膝盖和腿。你不能发出太大的动静。"

# game/BKcity_events.rpy:5051
translate chinese_simplified slave_beach_event_e37339ef:

    # you "Open your mouth and look me in the eyes..."
    you "张开嘴伸出舌头，看着我的眼睛...."

# game/BKcity_events.rpy:5065
translate chinese_simplified slave_beach_event_d86d6012:

    # you "I've got my eyes on your titties for some time... Why don't you use them to pleasure me?"
    you "你的胸部令我着迷....为什么不试试用蜜瓜夹住香肠品尝一番呢？"

# game/BKcity_events.rpy:5079
translate chinese_simplified slave_beach_event_5e0b0700:

    # you "Spread your legs, babe. We're going to have some fun!"
    you "张开你的腿，宝贝。让我们来点更刺激的！"

# game/BKcity_events.rpy:5108
translate chinese_simplified slave_beach_event_888233e3:

    # "Obeying your order, she kneels down and opens her mouth, sticking her tongue out. You hold her chin as you push your cock inside."
    "服从你的命令，她跪下来张开嘴，伸出舌头。当你把你的鸡巴推进去时，你捏住她的下巴。"

# game/BKcity_events.rpy:5111
translate chinese_simplified slave_beach_event_f8b0a17f:

    # "Not waiting for her final answer, you move her swimsuit out of the way and insert your cock between her tits."
    "她还没来得及做出反应，你就已经把她的泳衣向两边拉开，泳衣勒住胸部让乳房挺翘起来，你把肉棒插在了山谷之中。"

# game/BKcity_events.rpy:5114
translate chinese_simplified slave_beach_event_04e79480:

    # "As you lay down on the sand, she moves on top of you and lowers herself on your dick."
    "你躺在沙滩上，她美腿一翻骑在了你的腰上，挪动你的肉棒对齐她的蜜缝。"

# game/BKcity_events.rpy:5117
translate chinese_simplified slave_beach_event_ff2351ec:

    # "Pushing her on all fours, you move her swimsuit out of the way and place your cock at the entrance of her pussy."
    "你把她的双手抓住，把她的泳衣移到一边，用手握住肉棒让龟头在穴口上下摩擦。"

# game/BKcity_events.rpy:5120
translate chinese_simplified slave_beach_event_e8b44802:

    # "Holding her ankles, you push her back in the sand and spread her legs. Your cock quickly finds the entrance of her womb, shoving aside her swimsuit."
    "抓着她的脚踝，你把她的背推进沙子里，然后掰开她的双腿。你的肉棒一直顶着她的小穴，挤开了她的泳衣。"

# game/BKcity_events.rpy:5123
translate chinese_simplified slave_beach_event_a8db2b45:

    # "Holding her from behind, you drag her panties down and push your cock inside her."
    "你从后面抱住她，顺手把她的内裤扒了下来，没有任何前戏就插了进去，她早就湿透了。"

# game/BKcity_events.rpy:5126
translate chinese_simplified slave_beach_event_8f2a8204:

    # "Falling down on top of her, you furiously get rid of her thin swimsuit. She moans as you enter her."
    "你趴在她身上，疯狂地撕扯掉她的薄泳衣，漏出了她美丽迷人的肉缝，她感觉到插入之后娇喘了一声。"

# game/BKcity_events.rpy:5130
translate chinese_simplified slave_beach_event_292c7391:

    # girl.char "Ahaaa!"
    girl.char "啊哈！"

# game/BKcity_events.rpy:5134
translate chinese_simplified slave_beach_event_0cecfc03:

    # "You feel some resistance inside, and then something tears. A trickle of blood running down your shaft confirms that she is no longer a virgin."
    "你感到一股阻力，然后有什么东西破开了。几滴鲜血顺着你的阴茎流下来，看来她不再是处女了。"

# game/BKcity_events.rpy:5145
translate chinese_simplified slave_beach_event_de6a410f:

    # "A crowd quickly gathers. [girl.name] blushes but makes no effort to stop you."
    "很快人们就开始围观，[girl.name]脸红了，但没有阻止你。"

# game/BKcity_events.rpy:5147
translate chinese_simplified slave_beach_event_5ff1ca2e:

    # passerby "Look at 'em go! I wish I had a recording crystal..."
    passerby "瞧瞧这对开放的情侣！真希望我有一个录像水晶...."

# game/BKcity_events.rpy:5151
translate chinese_simplified slave_beach_event_95a28545:

    # "You mercilessly use her for your own pleasure, coming and going until you feel close to bursting."
    "你为了发泄自己的兽语尽情地在她身上驰骋，进进出出，你感觉一股暖流传到下体。"

# game/BKcity_events.rpy:5153
translate chinese_simplified slave_beach_event_9ce6fdb2:

    # you "Here it comes... Get ready!"
    you "接好了！我的种子！"

# game/BKcity_events.rpy:5155
translate chinese_simplified slave_beach_event_71bc82a6:

    # girl.char "Nggh!"
    girl.char "呃！"

# game/BKcity_events.rpy:5170
translate chinese_simplified slave_beach_event_47645da3:

    # "Grabbing her hair, you push your cock deep inside her mouth before releasing a geyser of cum."
    "抓住她的头发，你把你的阴茎深深地插进她的喉咙里，然后射了出来。"

# game/BKcity_events.rpy:5174
translate chinese_simplified slave_beach_event_da2af5af:

    # girl.char "Ngggh!!! *cough* *cough*"
    girl.char "呃！！！*咳嗽* *咳嗽*"

# game/BKcity_events.rpy:5178
translate chinese_simplified slave_beach_event_4081e2d5:

    # "She chokes on your cock and starts spitting cum all over her body and swimsuit."
    "她被你的阴茎噎住了，把嘴巴里的精液吐在了身上和泳衣上。"

# game/BKcity_events.rpy:5180
translate chinese_simplified slave_beach_event_198e6936:

    # girl.char "So rough... Aaah..."
    girl.char "好粗糙……啊……"

# game/BKcity_events.rpy:5185
translate chinese_simplified slave_beach_event_795f8948:

    # "Lifting her chin, you look straight into her eyes as you bring yourself over the limit, exploding all over her face."
    "抬起她的下巴，你直视她的眼睛，你感觉自己快要坚持不住了，直接射在了她的脸上。"

# game/BKcity_events.rpy:5189
translate chinese_simplified slave_beach_event_831759f8:

    # girl.char "Aah!"
    girl.char "啊！"

# game/BKcity_events.rpy:5193
translate chinese_simplified slave_beach_event_5cf32175:

    # "Sticky white cum drips down her face as you make sure to wipe the last drops on her forehead. She looks dazed."
    "当你确定最后一滴也在她脸上流出来的时候，粘乎乎的白色粘液从她的脸上流了下来。她看起来很茫然。"

# game/BKcity_events.rpy:5195
translate chinese_simplified slave_beach_event_063eacfd:

    # girl.char "So... Much... Semen..."
    girl.char "好多……精液……"

# game/BKcity_events.rpy:5200
translate chinese_simplified slave_beach_event_00ef63b5:

    # "Wrapping your dick inside a lock of her hair, you rub against her silky soft curls until you are ready to cum."
    "你把肉棒包在她的一绺头发里，摩擦她柔滑的卷发，直到你准备好射精。"

# game/BKcity_events.rpy:5204
translate chinese_simplified slave_beach_event_ba1ded7a:

    # girl.char "Hey!!!"
    girl.char "嘿！！！"

# game/BKcity_events.rpy:5208
translate chinese_simplified slave_beach_event_4606b25e:

    # "Thick white cum bursts out of your cock, and you leisurely spread it all over her soft hair, until it is completely soaked."
    "厚厚的白色精液从你的阴茎上冒了出来，你悠闲地把它涂在她柔软的头发上，直到它完全湿透。"

# game/BKcity_events.rpy:5210
translate chinese_simplified slave_beach_event_5a2909b8:

    # girl.char "It will take hours to wash it away... Aw..."
    girl.char "洗掉它得花好几个小时……哎……"

# game/BKcity_events.rpy:5215
translate chinese_simplified slave_beach_event_c54783b0:

    # "Aiming your dick at her tits, you instruct her to squeeze them tight together before exploding all over her."
    "把你的老二对准她的乳沟，双手聚拢形成了一片山谷，这样精液就不会漏下去了。"

# game/BKcity_events.rpy:5219
translate chinese_simplified slave_beach_event_f5b6bc2f:

    # girl.char "Mmmh..."
    girl.char "嗯……"

# game/BKcity_events.rpy:5223
translate chinese_simplified slave_beach_event_67bf8a92:

    # "She looks at her soiled mounds strangely, absent-mindedly rubbing them together as the cum runs down her chest."
    "她面色奇怪地看着自己沾满精液的乳房，心不在焉地把它们揉在一起，让精液从胸口流下来。"

# game/BKcity_events.rpy:5225
translate chinese_simplified slave_beach_event_004f83c6:

    # girl.char "There's cum all over my swimsuit... And I've got no other clothes to wear! Aw..."
    girl.char "我的泳衣上全是精液……而且我还没有别的衣服可以穿！啊……"

# game/BKcity_events.rpy:5228
translate chinese_simplified slave_beach_event_5db0bde8:

    # "You cum hard all over her, covering her with sticky semen."
    "你毫无保留的射在了她的身上，用粘稠的精液覆盖她。"

# game/BKcity_events.rpy:5232
translate chinese_simplified slave_beach_event_a3d34740:

    # girl.char "Ooh..."
    girl.char "哦……"

# game/BKcity_events.rpy:5234
translate chinese_simplified slave_beach_event_c14666d3:

    # girl.char "My swimsuit is ruined... And I have no spare clothes!"
    girl.char "我的泳衣毁了……而且我没有备用衣服！"

# game/BKcity_events.rpy:5236
translate chinese_simplified slave_beach_event_6a7bf726:

    # you "Oh well, too bad..."
    you "哦，太糟糕了...."

# game/BKcity_events.rpy:5242
translate chinese_simplified slave_beach_event_4b744439:

    # "[girl.fullname]'s {b}service{/b} skill has increased."
    "[girl.fullname]的{b}侍奉{/b}技能提升了。"

# game/BKcity_events.rpy:5246
translate chinese_simplified slave_beach_event_5f0536be:

    # "You fuck her with abandon, and she moans louder and louder with every thrust."
    "你肆无忌惮地操着她，她在每次插入的间歇的呻吟声越来越大。"

# game/BKcity_events.rpy:5248
translate chinese_simplified slave_beach_event_107c173f:

    # girl.char "Aah, aaah, aaaaaah!!!"
    girl.char "啊啊啊啊啊！！！"

# game/BKcity_events.rpy:5250
translate chinese_simplified slave_beach_event_e5349557:

    # "You feel close to your limit and decide not to delay the inevitable any longer."
    "你觉得自己已经快射了，决定不再忍耐。"

# game/BKcity_events.rpy:5252
translate chinese_simplified slave_beach_event_c7922447:

    # you "Take this!"
    you "接招!"

# game/BKcity_events.rpy:5254
translate chinese_simplified slave_beach_event_0fbb58dc:

    # girl.char "AAAAAAH!"
    girl.char "啊啊啊啊！"

# game/BKcity_events.rpy:5270
translate chinese_simplified slave_beach_event_abc1e59b:

    # "Ramming your cock deeper inside her, you release a huge wad of cum as you hit her cervix. Her body arches back as she takes it all in."
    "你的巨龙冲进了她蜜穴的最深处，撞开她的子宫口，你射出了浓厚的精液。她感觉腹部一股暖流后身体向后拱起。"

# game/BKcity_events.rpy:5274
translate chinese_simplified slave_beach_event_6e03b3c5:

    # girl.char "Aaaaah!!!"
    girl.char "啊啊啊！！！"

# game/BKcity_events.rpy:5280
translate chinese_simplified slave_beach_event_51a34b98:

    # "She clings to you as your throbbing cock keeps pumping hot semen inside her."
    "她紧紧地抱住你，你跳动的肉棒一直在她体内喷着滚烫的精液。"

# game/BKcity_events.rpy:5282
translate chinese_simplified slave_beach_event_a4604e4c:

    # girl.char "You came inside... Aaah..."
    girl.char "你进来了……啊……"

# game/BKcity_events.rpy:5287
translate chinese_simplified slave_beach_event_2e26bc9b:

    # "You withdraw as you feel about to cum, spurting white semen all over her gaping pussy."
    "当你快要坚持不住的时候，你向外拔出，白色的精液灌满了她张开的小穴外缘。"

# game/BKcity_events.rpy:5293
translate chinese_simplified slave_beach_event_6e03b3c5_1:

    # girl.char "Aaaaah!!!"
    girl.char "啊啊啊！！！"

# game/BKcity_events.rpy:5297
translate chinese_simplified slave_beach_event_bc93318d:

    # "She watches in a daze as hot cum flows out of her pussy."
    "她茫然地看着滚烫的精液从她的小穴中流出。"

# game/BKcity_events.rpy:5301
translate chinese_simplified slave_beach_event_9d3f5943:

    # girl.char "I'm so dirty... Mmmh..."
    girl.char "我好脏啊……嗯……"

# game/BKcity_events.rpy:5306
translate chinese_simplified slave_beach_event_6d35652f:

    # "Popping your cock out of her, you explode all over her smooth belly."
    "将你的鸡巴从她体内抽出，然后射在她光滑的肚子上。"

# game/BKcity_events.rpy:5310
translate chinese_simplified slave_beach_event_3041fa8c:

    # girl.char "Hey!"
    girl.char "嘿！"

# game/BKcity_events.rpy:5314
translate chinese_simplified slave_beach_event_37e819ad:

    # "More cum spurts out, smearing her body and swimsuit."
    "更多的精液喷出，沾满了她的身体和泳衣。"

# game/BKcity_events.rpy:5316
translate chinese_simplified slave_beach_event_5d204b66:

    # girl.char "My swimsuit is all sticky now... Aw..."
    girl.char "我的泳衣现在黏糊糊的……哎……"

# game/BKcity_events.rpy:5323
translate chinese_simplified slave_beach_event_8a001466:

    # "[girl.fullname]'s {b}sex{/b} skill has increased."
    "[girl.fullname]的{b}性交{/b}技能提升了。"

# game/BKcity_events.rpy:5326
translate chinese_simplified slave_beach_event_01ea3072:

    # "Disappointed, you go back to your other business."
    "你大失所望，回去处理别的事情了。"

# game/BKcity_events.rpy:5330
translate chinese_simplified slave_beach_event_7b8f473b:

    # you "Well, I hope you enjoyed your time off - because it's over now."
    you "好吧，我希望你已经享受完你的假期了，因为你的假期即将结束。"

# game/BKcity_events.rpy:5332
translate chinese_simplified slave_beach_event_9ec50e45:

    # girl.char "Uh? What do you mean?"
    girl.char "嗯？你什么意思？"

# game/BKcity_events.rpy:5334
translate chinese_simplified slave_beach_event_51f4b6cf:

    # you "I mean, I can't let such a nice piece of booty go to waste even for a day. You look damn sexy in your swimsuit, and I'm sure customers are ready to pay some good money to use you right now!"
    you "我的意思是，我不能放任你如此浪费自己的美貌和天赋。你穿着泳衣的样子让男人们垂涎三尺，我敢肯定他们愿意排着队花钱来和你干上一炮！"

# game/BKcity_events.rpy:5347
translate chinese_simplified slave_beach_event_a5efeed9:

    # you "Listen, I'll give you... double your regular upkeep. How does that sound?"
    you "听着，我会给你....双倍的工资。听上去怎么样？"

# game/BKcity_events.rpy:5349
translate chinese_simplified slave_beach_event_534f02be:

    # girl.char "Double the money, uh..."
    girl.char "钱翻倍，呃……"

# game/BKcity_events.rpy:5359
translate chinese_simplified slave_beach_event_fc71f818:

    # you "I'll pay your normal upkeep, of course. It's only fair..."
    you "当然，我会支付给你正常的薪水。这是你应得的报酬...."

# game/BKcity_events.rpy:5361
translate chinese_simplified slave_beach_event_c1da3d17:

    # girl.char "..."
    girl.char "..."

# game/BKcity_events.rpy:5366
translate chinese_simplified slave_beach_event_dd50664b:

    # you "I own you, so why should I care about your opinion? Get your ass to work!"
    you "你不过是我的一个奴隶，我为什么要尊重你的意见？赶紧扭着你的屁股去工作！"

# game/BKcity_events.rpy:5368
translate chinese_simplified slave_beach_event_fdde360c:

    # girl.char "Hey! That's not fair..."
    girl.char "嘿！这不公平……"

# game/BKcity_events.rpy:5389
translate chinese_simplified slave_beach_event_30ebcc9d:

    # "She leaves with her head held up high before you have a chance to object."
    "在你打算呵斥她之前，她就昂着头离开了。"

# game/BKcity_events.rpy:5393
translate chinese_simplified slave_beach_event_4ee40f29:

    # you "Good. Let me get things set-up..."
    you "棒极了。让我来搭建一个临时的场地...."

# game/BKcity_events.rpy:5398
translate chinese_simplified slave_beach_event_5d3c6149:

    # you "Hear hear!"
    you "嘿！大家快来啊！"

# game/BKcity_events.rpy:5400
translate chinese_simplified slave_beach_event_9dfc76af:

    # you "Ladies and Gentlemen, I mean, mostly you Gentlemen, why don't you come and sample one of the best produce of [brothel.name], brought to you right here at the [loc]?"
    you "女士们先生们，我是说，主要是各位先生们，何不来品尝一下[brothel.name]最好的产品之一，就在[loc]这里为您奉上？"

# game/BKcity_events.rpy:5402
translate chinese_simplified slave_beach_event_c42686a9:

    # you "Lovely miss [girl.fullname] is waiting for you here, ready to serve at your whim..."
    you "你们有幸遇到了性感尤物 [girl.fullname] ，今天一整天都将为你们服务...."

# game/BKcity_events.rpy:5410
translate chinese_simplified slave_beach_event_0cd402a4:

    # girl.char "Who wants to try me? Come forward, don't be shy!"
    girl.char "谁想试试我？上前来，别害羞！"

# game/BKcity_events.rpy:5412
translate chinese_simplified slave_beach_event_c550eaaf:

    # "Dude" "Sweet! I want her to suck my dick... Is it cool?"
    "哥们" "太棒了！我想让她给我口交……可以吗？"

# game/BKcity_events.rpy:5414
translate chinese_simplified slave_beach_event_e836a702:

    # man "No, dude! I want to bang her!"
    man "不，兄弟！我想狠狠草她！"

# game/BKcity_events.rpy:5416
translate chinese_simplified slave_beach_event_230fb705:

    # passerby "Not fair! I wanted to fuck her ass..."
    passerby "不公平！我还想走她的屁眼呢。"

# game/BKcity_events.rpy:5428
translate chinese_simplified slave_beach_event_724bde1c:

    # you "You there, get yourself ready. [girl.name] will take good care of you..."
    you "都排好队。 [girl.name] 会让你物超所值的...."

# game/BKcity_events.rpy:5432
translate chinese_simplified slave_beach_event_ba9e6bd0:

    # you "One of you lucky bastards can get laid today... You! Step forward."
    you "你们这群色鬼中的一个，今天将有幸一亲美人芳泽....你！向前一步。"

# game/BKcity_events.rpy:5436
translate chinese_simplified slave_beach_event_562e2e91:

    # you "Did someone say 'anal sex'? You've come to the right place!"
    you "是不是有人说“肛交”？她紧致的菊穴会把你的灵魂都给吸出来！"

# game/BKcity_events.rpy:5440
translate chinese_simplified slave_beach_event_50a53d71:

    # you "Chill, guys, there's no need to fight: [girl.fullname] will serve all of you today!"
    you "冷静，伙计们，没有必要打架：[girl.fullname] 今天将为这里的每一个人服务！"

# game/BKcity_events.rpy:5444
translate chinese_simplified slave_beach_event_c64f1079:

    # "[girl.name] kneels in front of the man, lowering his trunks. His erect shaft pops out, almost hitting her in the face."
    "[girl.name] 来到男人面前跪下，低下头用嘴咬下他的内裤。他的挺立的阴茎突然弹出，打在了她的脸上。"

# game/BKcity_events.rpy:5445
translate chinese_simplified slave_beach_event_a9b8e8b4:

    # "Dude" "Oh, this is my lucky day..."
    "哥们" "哦，今天真是我的幸运日……"

# game/BKcity_events.rpy:5448
translate chinese_simplified slave_beach_event_841402fd:

    # "The customer moves on [girl.name]. She moans softly as his hands reach inside her swimsuit and fondle her privates."
    "男人朝[girl.name]凑了上去。把手伸进她的泳衣里抚摸她的私处，她轻声呻吟着。"

# game/BKcity_events.rpy:5449
translate chinese_simplified slave_beach_event_bac2af43:

    # man "Come on, babe! Let's fuck..."
    man "来吧宝贝儿！我会让你飞上云霄...."

# game/BKcity_events.rpy:5452
translate chinese_simplified slave_beach_event_061b0610:

    # "Grinning from ear to ear, the customer grabs her butt and bends her forward, pushing his hard cock against her butthole."
    "男人咬住她的耳朵，抓住她的臀部，让她像母狗一样趴着，将他坚硬的阴茎缓慢推进她的屁眼里。"

# game/BKcity_events.rpy:5454
translate chinese_simplified slave_beach_event_64a4be4e:

    # passerby "Let me just move this swimsuit out of the way... Nice."
    passerby "这件泳衣太碍事了..*嘶拉*..这下就方便多了。"

# game/BKcity_events.rpy:5457
translate chinese_simplified slave_beach_event_65ebfa4c:

    # "The customers" "Hurray!!!"
    "顾客" "Hurray!!!"

# game/BKcity_events.rpy:5459
translate chinese_simplified slave_beach_event_5c691dae:

    # "The three men eagerly jump on [girl.name], and she squeals as they start grabbing and rubbing her private parts."
    "这三个男人猴急地扑到 [girl.name] 身上，她发出尖叫，这三个男人直接按住她并开始挑逗她的私处。"

# game/BKcity_events.rpy:5474
translate chinese_simplified slave_beach_event_59a50666:

    # girl.char "Aaaaah!"
    girl.char "啊啊啊！"

# game/BKcity_events.rpy:5480
translate chinese_simplified slave_beach_event_0affd0ff:

    # "[girl.name] uses her mouth to pleasure the customer, taking her time to bring him to a perfect climax."
    "[girl.name]用嘴取悦顾客，舌头包裹住龟头，旋转盘绕，吞吐着肉虫，用力吮吸着马眼。"

# game/BKcity_events.rpy:5482
translate chinese_simplified slave_beach_event_45f08dcb:

    # "[girl.name] uses her hands to pleasure the customer, taking her time to bring him to a perfect climax."
    "[girl.name]用双手取悦顾客，一只手来回拨弄棒身，一只手轻柔地抚摸着睾丸，撸动的速度时快时慢，时紧时松。"

# game/BKcity_events.rpy:5484
translate chinese_simplified slave_beach_event_a7c8b3d0:

    # "[girl.name] uses her tits to pleasure the customer, taking her time to bring him to a perfect climax."
    "[girl.name]把她的双乳解放在男人面前，任凭他揉捏吮吸，男人把肉棒对着胸部使劲抽插，龟头绕着乳首转圈。"

# game/BKcity_events.rpy:5486
translate chinese_simplified slave_beach_event_2f8c7a7b:

    # "[girl.name] masturbates in front of the customer, looking him in the eye as he brings himself off."
    "[girl.name]在顾客面前手淫，抠弄自己的蜜穴，将渗出的淫水涂抹在唇间，用舌头一丝不剩地舔舐干净。"

# game/BKcity_events.rpy:5488
translate chinese_simplified slave_beach_event_a3debe29:

    # "[girl.name] uses her mouth and hands to pleasure the customer, taking her time to bring him to a perfect climax."
    "[girl.name]一边用嘴吞吐着肉龙，一边用手指深入刺激男人的前列腺。"

# game/BKcity_events.rpy:5490
translate chinese_simplified slave_beach_event_a7325ce9:

    # "Dude" "Oh, man... I will not stand... this aggression... Man..."
    "哥们" "哦，天哪……我绝不能容忍……这种侵略……天哪……"

# game/BKcity_events.rpy:5493
translate chinese_simplified slave_beach_event_97a15c09:

    # "The man starts fucking [girl.name] wildly and she does her best to give him a good time."
    "她两只绵软肉感的大腿完全夹紧，让细腻的肌肤上下搓揉起了肉棒，男人开始疯狂地操 [girl.name]。"

# game/BKcity_events.rpy:5496
translate chinese_simplified slave_beach_event_67a2ab6d:

    # "She yelps with pain as he enters her, forcing her hymen open. She is no longer a virgin."
    "当他插入她的小穴时，她痛苦地尖叫着，粗暴地捅穿了处女膜。她不再是处女了。"

# game/BKcity_events.rpy:5499
translate chinese_simplified slave_beach_event_d1de2594:

    # man "Oh, yes! I'm on fire..."
    man "哦...！老子从来没见过你这么骚的美人儿...."

# game/BKcity_events.rpy:5502
translate chinese_simplified slave_beach_event_fb7cd5b5:

    # "The guy fucks [girl.name]'s ass with abandon, spitting inside her crack to lubricate. She takes it all in stride."
    "这家伙肆无忌惮地操着 [girl.name] 的屁股，在她的裂缝里舔上口水来润滑。她从容应对。"

# game/BKcity_events.rpy:5504
translate chinese_simplified slave_beach_event_dff03954:

    # passerby "Oh, I can go all the way in... I'm balls deep... This is the best!"
    passerby "哦，还没插到花心....好深的小穴....就像是无底洞一样！"

# game/BKcity_events.rpy:5508
translate chinese_simplified slave_beach_event_1686205b:

    # "The men fuck her eagerly, and she does her best to make sure all of them are having fun."
    "男人们尽情地在她身上泄欲，喷涌出的精液将泳衣顶得隆起，在她那件可爱的衣服染上了一块块下流的湿痕。"

# game/BKcity_events.rpy:5511
translate chinese_simplified slave_beach_event_afefac09:

    # man "Oh, man, look at that! She was a virgin, hahaha..."
    man "嘿兄弟, 瞧!她还是个雏,哈哈哈..."

# game/BKcity_events.rpy:5513
translate chinese_simplified slave_beach_event_00417ccb:

    # passerby "Unbelievable!"
    passerby "难以置信!"

# game/BKcity_events.rpy:5518
translate chinese_simplified slave_beach_event_47f929f9:

    # "One of the men decides to fuck her ass while she uses the rest of her body to pleasure the others."
    "一名男子掰开她的屁股打算操她的菊穴，在她用双手提起泳衣的动作当中，龟头与升起的紧致布料之间也拉出了几条淫靡的水丝。"

# game/BKcity_events.rpy:5520
translate chinese_simplified slave_beach_event_7ff63124:

    # "She uses her whore body and her skills to the max to help the customers reach climax."
    "她用自己妖娆的身姿和熟练地技术让在场所有的男人都缴械投降了。"

# game/BKcity_events.rpy:5522
translate chinese_simplified slave_beach_event_323a8633:

    # "Dude" "Awesome..."
    "哥们" "惊人的..."

# game/BKcity_events.rpy:5524
translate chinese_simplified slave_beach_event_85bafe9e:

    # man "This is too good.."
    man "这里也好爽，她要是我老婆我能和她换着花样连续玩一个星期。"

# game/BKcity_events.rpy:5526
translate chinese_simplified slave_beach_event_19af7a9d:

    # passerby "That brothel guy wasn't joking... I've never had so much fun!"
    passerby "那个皮条客真不是在开玩笑....我从来没操过这么紧的小穴！"

# game/BKcity_events.rpy:5530
translate chinese_simplified slave_beach_event_b3717b26:

    # girl.char "Hmm... Oooh..."
    girl.char "嗯……哦……"

# game/BKcity_events.rpy:5540
translate chinese_simplified slave_beach_event_f33411fa:

    # "After having their way with her, the group looks like they're about to cum already."
    "在被她服务之后，这群人看起来已经快要坚持不住了。"

# game/BKcity_events.rpy:5544
translate chinese_simplified slave_beach_event_f99320fc:

    # "After a while, the man looks like he's about to reach his limit."
    "片刻之后，这个男人似乎快要达到极限了。"

# game/BKcity_events.rpy:5553
translate chinese_simplified slave_beach_event_5efb3fe9:

    # "The [cust] decide[ending] to unload all over her face. She gives out a squeal as she receives it."
    "[cust] 决定给她敷一个免费的面膜[ending]，浓厚的精液覆盖住了她的美丽面容，她的眼睛、口鼻都被遮住了。"

# game/BKcity_events.rpy:5558
translate chinese_simplified slave_beach_event_add1c94c:

    # "The [cust] want[ending] to cum in her mouth, and she complies. Doing her best not to gag, she swallows a flow of smelly cum."
    "[cust] 想要让她饱餐一顿[ending]，她欣然应允。她用力吞咽，最后张开小嘴吐出沾满精液的香舌证明全部都吞咽下去。"

# game/BKcity_events.rpy:5563
translate chinese_simplified slave_beach_event_a7da3295:

    # "The [cust] end[ending] up jerking off in her hair, covering it with sticky cum."
    "[cust] 用她的发丝包裹自己的肉棒来清洁[ending]，头发上到处都是粘稠的精液，一滴一滴地往下滴落。"

# game/BKcity_events.rpy:5568
translate chinese_simplified slave_beach_event_bf1f422c:

    # "The [cust] get[ending] ready to cover her face and hair with cum, and she squeals as it shoots all over her face."
    "[cust] 准备把精液射在她的脸颊和头发上[ending]，当精液射在她的脸上的时候，她发出了尖叫声。"

# game/BKcity_events.rpy:5573
translate chinese_simplified slave_beach_event_aafba0af:

    # "The [cust] cum[ending] all over her body and swimsuit, until she is covered with sticky semen."
    "[cust] 把肉棒插入到泳衣和身体间一顿爆射[ending]，泳衣里到处都是粘稠的精液，包裹着她的身体。"

# game/BKcity_events.rpy:5578
translate chinese_simplified slave_beach_event_f23d0bd3:

    # "The [cust] want[ending] to cum inside, and she moans as her pussy fills up to the brim with hot white semen."
    "[cust] 想要在她的子宫里内射[ending]，当她的子宫充满炙热的精液时，她的小腹微微隆起。"

# game/BKcity_events.rpy:5583
translate chinese_simplified slave_beach_event_16504c6e:

    # "It's creampie time! The [cust] jerk[ending] off and cum[ending] all over her gaping hole."
    "泡芙时间到！[cust]猛地一顶[ending]，把她那张大的小穴射的满满的[ending]。"

# game/BKcity_events.rpy:5588
translate chinese_simplified slave_beach_event_d10ce105:

    # "Overwhelmed, the [cust] cannot endure anymore and cums hard."
    "实在顶不住了，[cust]再也忍不住了，用力射了出来。"

# game/BKcity_events.rpy:5592
translate chinese_simplified slave_beach_event_33fb437a:

    # "While they were going at it, a line of would-be customers has formed by your improvised stall."
    "当他们在水床上干的天昏地暗时，你临时搭建的摊位外已经排起了长龙。"

# game/BKcity_events.rpy:5611
translate chinese_simplified slave_beach_event_19352ab1:

    # "She ends up serving a total of [nb] customers."
    "她最终为[nb]位顾客提供了服务。"

# game/BKcity_events.rpy:5623
translate chinese_simplified slave_beach_event_7335c319:

    # "The customers brought in [tip] gold. You gave [girl.name] [upk] gold for upkeep."
    "顾客消费了 [tip] 金币。你给了 [girl.name] [upk] 金币作为酬劳。"

# game/BKcity_events.rpy:5625
translate chinese_simplified slave_beach_event_3db5f193:

    # "The customers brought in [tip] gold. It was all profit, since you deprived [girl.name] of her rightful upkeep."
    "顾客消费了 [tip] 金币。这些钱全都进了你的口袋，因为你一分钱也没有付给 [girl.name] 。"

# game/BKcity_events.rpy:5633
translate chinese_simplified slave_beach_event_5d996cc8:

    # girl.char "What? No, I'm not going to do that with this guy!"
    girl.char "什么？不，我不会对这个人做那种事！"

# game/BKcity_events.rpy:5643
translate chinese_simplified slave_beach_event_2a52d73e:

    # you "Damn you! Just you wait until I catch you again..."
    you "他奶奶的!别给老子逮到你..."

# game/BKcity_events.rpy:5645
translate chinese_simplified slave_beach_event_41dee3d2:

    # you "Hmph, you're not even worth the trouble... You'd better do what I tell you next time!"
    you "呼,你根本不值得我操心...下次你最好照我说的做!"

# game/BKcity_events.rpy:5647
translate chinese_simplified slave_beach_event_7bafea4a:

    # "You realize she is in no shape to serve customers now. Frustrated, you tell her to pack her things and go back to the brothel."
    "你知道她现在的状态不适合接待顾客。你很失望，让她收拾东西回青楼去。"

# game/BKcity_events.rpy:5658
translate chinese_simplified slave_beach_event_dc0306da:

    # "Let down, the men scatter and grumble something about false advertising... Your brothel has lost reputation."
    "客人们十分扫兴，这些都是虚假宣传，他们转身离开....你的青楼名誉扫地了。"

# game/BKcity_events.rpy:5663
translate chinese_simplified slave_beach_event_2666d939:

    # you "All right, I'll see you at [brothel.name]."
    you "好的，让我们在 [brothel.name] 碰面。"

# TODO: Translation updated at 2026-06-12 05:03

translate chinese_simplified strings:
    old "Let me..."
    new "让我..."
    old "What will you do?"
    new "你会怎么做？"
    old "What do you do?"
    new "你做什么工作？"
    old "Have sex with me"
    new "和我做爱"
    old "Get ready to serve some customers"
    new "准备好接待顾客吧"
    old "Nothing (Leave)"
    new "无（离开）"
    old "What will you have her do?"
    new "你想让她做什么？"
    old "Choose what [girl.name] will do."
    new "选择[girl.name]将要做什么。"
    old "You remember [girl.name] dislikes %s, but you decide to go for it anyway."
    new "你记得[girl.name]不喜欢%s，但你还是决定试一试。"
translate chinese_simplified strings:

    # game/core/content/city_events/city_events.rpy:336
    old "Sell it to Willow"
    new "卖给 Willow"

    # game/core/content/city_events/city_events.rpy:4802
    old "You remember [girl.name] likes %s, and decide to exploit her weakness."
    new "你记得 [girl.name] 喜欢 %s，于是决定利用她的弱点。"

# TODO: Translation updated at 2026-09-25 05:26

# game/core/content/city_events/city_events.rpy:4935
translate chinese_simplified slave_beach_event_bbacacf0:

    # "[girl.fullname]'s {b}[stat1_text]{/b} and {b}[stat2_text]{/b} have increased."
    "[girl.fullname] 的 {b}[stat1_text]{/b} 和 {b}[stat2_text]{/b} 提升了。"

translate chinese_simplified strings:
    old "Fight it"
    new "战斗"
    old "Banish it"
    new "驱逐它"
    old "Run away"
    new "逃跑"
    old "Show off"
    new "炫耀一番"
    old "Flirt with the girls"
    new "与女孩们调情"
    old "Talk to her"
    new "与她攀谈"
    old "Hypnotize her"
    new "催眠她"
    old "Not my concern"
    new "与我无关"
    old "Call her out"
    new "当面揭穿她"
    old "Cut your losses"
    new "及时止损"
    old "Run after the thief"
    new "追赶小偷"
    old "Ensnare the thief"
    new "用法术困住小偷"
    old "Face him"
    new "直面他"
    old "Ignore him"
    new "别理他"
    old "Look for her cat"
    new "帮她找猫"
    old "Track it with a spell"
    new "用法术追踪它"
    old "Force the door open"
    new "强行破门"
    old "Reveal with magic"
    new "用法术显形"
    old "Walk away"
    new "走开"
    old "Big man"
    new "壮汉"
    old "big man"
    new "壮汉"
    old "Weird man"
    new "怪人"
    old "weird man"
    new "怪人"
    old "Funny man"
    new "滑稽的人"
    old "funny man"
    new "滑稽的人"
    old "Fix the wagon"
    new "修理货车"
    old "Bring some customers"
    new "带些客人来"
    old "Decline to help"
    new "拒绝帮忙"
    old "Charm her"
    new "魅惑她"
    old "Give her some gold"
    new "给她一些金币"
    old "Fight back"
    new "反击"
    old "Run for your life"
    new "逃命"
    old "Cast a control spell"
    new "施放控制法术"
    old "Make a speech"
    new "发表演说"
    old "Divert attention with gold"
    new "用金币转移注意力"
    old "You take a stroll through the %s. Nothing meaningful happens."
    new "你在%s闲逛了一圈，没发生什么有意义的事。"
    old "After a pleasant walk around the %s, you decide it's time to head home."
    new "在%s愉快地散步之后，你决定该回家了。"
    old "The %s is peaceful today. You have a pleasant walk."
    new "今天的%s很宁静，你散步散得很惬意。"
    old "You spend time browsing the wares of some passing merchants near the %s. In the end, you don't find anything you like."
    new "你在%s附近逛了逛路过的商贩，到头来没有找到喜欢的东西。"
    old "You have a nice time sitting on a bench near the %s, watching people come and go and whistling at pretty girls. None pay you any attention, however."
    new "你坐在%s附近的长椅上，看着人来人往，对漂亮的女孩们吹口哨。然而没人理睬你。"
    old "Walking around the %s, you see nothing special. A waste of time."
    new "你在%s转了转，没什么特别的。浪费时间。"
    old "You walk among the good people of the %s. Everyone looks friendly on such a beautiful day."
    new "你走在%s的好市民中间。在这么美好的一天里，每个人看起来都很友善。"
    old "Watching people come and go through the %s, you wonder how you could profit from their endeavors."
    new "看着人们穿梭往来于%s，你想知道如何从他们的营生中分一杯羹。"
    old "The people of the %s are a wretched and vicious bunch, just like anywhere else. Elbowing your way through this crowd of losers, you despise them all."
    new "%s的居民是一群卑劣恶毒的家伙，和别处一样。你挤过这群废物，打心底鄙视他们。"
    old "In the nooks and shadows of the %s, you see discreet but unmistakable signs of Shalia worship. You nod approvingly."
    new "在%s的角落和阴影中，你看到了隐秘但确凿无误的莎莉亚崇拜迹象，你赞许地点了点头。"
    old "Looking at a group of pilgrims crossing the %s, you see some flamboyant, outspoken worshippers of Arios, wearing their faith on their sleeves. Others are more humble, and follow the idols in silent prayer. You wonder which ones are the true faithful."
    new "看着一群朝圣者穿过%s，你看到了一些张扬外露的阿里奥斯信徒，他们把信仰穿在身上；另一些则更为谦逊，默默祈祷。你想知道谁才是真正的信徒。"
    old "All sorts of religious nuts are travelling through the %s with feverish eyes, claiming to do the bidding of one god or another. 'Spice of the people', you tell yourself, shaking your head in disbelief."
    new "各种宗教狂热分子正狂热地穿过%s，声称在为某个神明效劳。“人民的调味品”，你摇着头对自己说，难以置信。"
    old "some juicy gossip"
    new "一些劲爆的八卦"
    old "a disturbing rumor"
    new "一则令人不安的传闻"
    old "a curious story"
    new "一个稀奇的故事"
    old "an interesting tale"
    new "一个有趣的传说"
    old "a word of warning"
    new "一句警告"
    old "Young man"
    new "年轻男子"
    old "Young woman"
    new "年轻女子"
    old "Old man"
    new "老头"
    old "Old woman"
    new "老妇"

