# TODO: Translation updated at 2026-06-12 08:13

translate chinese_simplified strings:
    old "You cannot train a girl more than once per day."
    new "一天之内不能训练同一个女孩超过一次。"
    old "You cannot do the same things more than 3 times a day with a girl."
    new "一天之内，你不能和同一个女孩做同样的事情超过三次。"
    old "Receive a %s Potion of Seduction."
    new "获得 %s 瓶魅惑药水。"
    old "Receive a %s Balm of Restoration."
    new "获得 %s 修复膏。"
    old "Receive a %s Incense of Bliss."
    new "获得 %s 份极乐香。"
    old "Receive a %s Magic Powder."
    new "获得 %s 魔法粉末。"
    old "Receive a %s Wyvern egg."
    new "获得 %s 枚飞龙蛋。"
    old "You have received %s as a starting slave!"
    new "你已获得 %s 作为初始奴隶！"
    old "Removing %s from game mixes"
    new "从游戏混合池中移除%s"
    old "- Dancer: "
    new "- 舞女："
    old "- Masseuse: "
    new "- 按摩师："
    old "- Geisha: "
    new "- 艺伎："
    old "- Whore: "
    new "- 娼妓："
    old "%s has been updated."
    new "%s 已更新。"
translate chinese_simplified strings:

    # game/core/framework/game_systems.rpy:167
    old "Mod: %s has been removed."
    new "模组：%s 已被移除。"

    # game/core/framework/game_systems.rpy:178
    old "%s has been added."
    new "%s 已添加。"

# TODO: Translation updated at 2026-09-11 06:39

translate chinese_simplified strings:
    old "You made %s last night.\n"
    new """你昨晚赚了 %s。
"""
    old "You lost %s last night.\n"
    new """你昨晚亏损了 %s。
"""
    old "- Gold made: + %s\n"
    new """- 赚取金币：+ %s
"""
    old "- Girls upkeep: - %s\n"
    new """- 女孩的供养费：- %s
"""
    old "- Brothel costs: - %s\n{/size}\n"
    new """- 青楼开销：- %s
{/size}
"""
    old "%s customer%s came to the brothel.\n"
    new """%s 位客人%s来到了青楼。
"""
    old "- Customer%s served (job): %s/%s\n"
    new """- 接待的客人%s（工作）：%s/%s
"""
    old "- Customer%s entertained (job): %s/%s\n"
    new """- 招待的客人%s（工作）：%s/%s
"""
    old "- Customer%s served (sex): %s/%s\n"
    new """- 接待的客人%s（性服务）：%s/%s
"""
    old "- Customer%s satisfied (sex): %s/%s\n{/size}\n"
    new """- 满意的客人%s（性服务）：%s/%s
{/size}
"""
    old "%s girl%s worked in the brothel. "
    new "%s 位姑娘%s在青楼工作。 "
    old "%s girl%s went on strike. "
    new "%s 位姑娘%s罢工了。 "
    old "%s girl%s ran away from the brothel."
    new "%s 位姑娘%s从青楼逃跑了。"
    old "%s girls were resting at the brothel. "
    new "%s 位姑娘在青楼休息。 "
    old "%s girl was resting at the brothel. "
    new "%s 位姑娘在青楼休息。 "
    old "%s girls were hurt while working. "
    new "%s 位姑娘在工作中受伤了。 "
    old "%s girl was hurt while working. "
    new "%s 位姑娘在工作中受伤了。 "
    old "%s girl%s became exhausted while working. "
    new "%s 位姑娘%s在工作中精疲力竭了。 "
    old "%s girls were at the farm last night. "
    new "%s 位姑娘昨晚在农场。 "
    old "%s girl%s resisted training. "
    new "%s 位姑娘%s抗拒训练。 "
    old "%s girl%s ran away from the farm."
    new "%s 位姑娘%s从农场逃跑了。"
    old "%s girl%s got hurt while resisting."
    new "%s 位姑娘%s在抗拒时受伤了。"
    old "%s minion%s got hurt in the fighting."
    new "%s 名手下%s在战斗中受伤了。"
    old "- In training: %s\n"
    new """- 训练中：%s
"""
    old "- In holding: %s{/size}\n"
    new """- 关押中：%s{/size}
"""
    old "%s girls were resting at the farm. "
    new "%s 位姑娘在农场休息。 "
    old "%s girl was resting at the farm. "
    new "%s 位姑娘在农场休息。 "

# TODO: Translation updated at 2026-09-25 05:26

translate chinese_simplified strings:
    old " was set to rest."
    new " 被安排休息。"
    old "Updating mods... "
    new "正在更新 Mod…… "
    old "Mod: %s has been added."
    new "Mod：%s 已添加。"
    old "Mod: %s has been updated."
    new "Mod：%s 已更新。"
    old "Mod: %s has been activated."
    new "Mod：%s 已激活。"
    old " (%i/%i)"
    new "（%i/%i）"
    old "{i}Previous{/i}"
    new "{i}上一页{/i}"
    old "{color=#AAA}{i}Previous{/i}"
    new "{color=#AAA}{i}上一页{/i}"
    old "{i}Next{/i}"
    new "{i}下一页{/i}"
    old "{color=#AAA}{i}Next{/i}"
    new "{color=#AAA}{i}下一页{/i}"
    old " Allows autorest to be set up from the Schedule screen."
    new " 允许在日程界面设置自动休息。"
    old "She must be open to at least one sex act before choosing 'Bisexual'."
    new "在选择“双性恋”之前，她至少要接受一种性行为。"
    old "She must be open to at least two sex acts before choosing 'Group'."
    new "在选择“群交”之前，她至少要接受两种性行为。"
    old "Requires 2 or 3 free minions in the farm."
    new "需要农场里有 2 或 3 个空闲仆从。"

