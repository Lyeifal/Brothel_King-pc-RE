...
############ Jman - Headhunter Mod End ########
            ...
            if slavemarket_firstvisit:
                ...
                $ _context = "slavemarket"

            call acquire_girl(girl, price, context = _context, can_follow=False) from _call_acquire_girl_3 # Checks that MC has the necessary money and room in the brothel or farm.
...
label farm_loop():
    ...
    while True:
        ...
        if res == "show":
            call farm_show(obj) # obj is the girl in this case
...
label main():
    ...
    if brothel.get_common_rooms():
        $ _room = brothel.get_random_room_pic()
    else:
        $ _room = "black"
        ...
        if result == "assign":
            ...
            if girl.job:
                ...
                if game.has_active_mod("chrisjobmod"):
                    ...
                    jump girls
                #<End of Chris Job Mod: edit from JMan>
                ...
                if MC.has_gold(price):
                    if isinstance(it, Minion): # Minions
                        ...
                        if res:
                            ...
                            $ notify("[text1]", it.pic, col=c_white)
                        ...
                        if right_focus == NPC_twins:
                            ...
                            $ notify(merchant_greetings["Yesterday bought something"], "side yesterday", col=c_turquoise)
...
label postings:
    ...
    if NPC_mizuki.flags["quest W"] == "available" and mizuki_questW not in quest_board.quests:
        $ quest_board.quests.insert(0, mizuki_questW)
    if NPC_mizuki.flags["quest K"] == "available" and mizuki_questK not in quest_board.quests:
        $ quest_board.quests.insert(0, mizuki_questK)
    ...
    while True:
        ...
        if result == "classes":
            ...
            if selected_girl not in available_girls and available_girls:
            ...
            if renpy.call_screen("yes_no", t):
                ...
                if commited_quest.commit_label:
                    call expression commited_quest.commit_label pass (selected_girl) from _call_expression_10
                else:
                    call dialogue(selected_girl, "leave for " + commited_quest.type) from _call_dialogue_101

                if commited_quest.type == "quest":
                    $ quest_board.quests.remove(commited_quest)
                    if quest_board.quests:
                        $ selected_quest = quest_board.quests[0]
                    else:
                        $ selected_quest = None
                ...
                elif commited_quest.type == "class":
                    if story_flags["postings free class"]:
                        $ story_flags["postings free class"] = False
                    else:
                        $ MC.change_gold(-commited_quest.get_gold())

                    if len(commited_quest.enrolled) >= commited_quest.capacity:
                        $ quest_board.classes.remove(commited_quest)
                        if quest_board.quests:
                            $ selected_quest = quest_board.quests[0]
                        else:
                            $ selected_quest = None
