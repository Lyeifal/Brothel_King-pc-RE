...
image img_dice1 = ProportionalScale("resources/ui/dice1.webp", *res_tb(20))
image img_dice2 = ProportionalScale("resources/ui/dice2.webp", *res_tb(20))
image img_dice3 = ProportionalScale("resources/ui/dice3.webp", *res_tb(20))
image img_dice4 = ProportionalScale("resources/ui/dice4.webp", *res_tb(20))
image img_dice5 = ProportionalScale("resources/ui/dice5.webp", *res_tb(20))
image img_dice6 = ProportionalScale("resources/ui/dice6.webp", *res_tb(20))
...
label end_day:
    ...
    # Phase 6: Settlement context for Mod hooks
...
#### PLAYING CONDITIONAL EVENTS ####
    ...
    $ print("Preparing %i conditional events took %.2f seconds" % (len(check_events), t1-t0)) # Debugging to spot possible 'infinite' loop
    ...
    $ print("Resolving %i conditional events took %.2f seconds" % (len(check_events), t2-t1)) # Debugging to spot possible 'infinite' loop
...
#### PREPARING GIRLS ####
    ...
    ## Autocast spells (NIGHT)
        ...
        print("Early night set-up took %.2f seconds" % (t3-t2)) # Debugging to spot possible 'infinite' loop
    ...
    ## Get working girls
        ...
        # Phase 6: Settlement hooks
        settlement_ctx.working_girls = working_girls
        settlement_ctx.striking_girls = striking_girls
        settlement_ctx.resting_girls = resting_girls
        settlement_ctx.sick_girls = sick_girls
        settlement_ctx.away_girls = away_girls
        settlement_ctx.job_girls = job_girls
        settlement_ctx.whores = whores
        hook_manager.invoke("on_settlement_girls_ready", settlement_ctx)
    ...
    ## Apply advertisement
        ...
        if working_girls: # Open
            ...
            if round_int(brothel.rep) != old_rep:
                ...
                rep_text = __("Your brothel's reputation remains stable (%s).") % '{:,}'.format(round_int(brothel.rep))

            night_early.add(_("Brothel reputation:\n%s (%s)") % (str_int(brothel.rep) + "/" + str_int(brothel.max_rep), plus_text(chg, color_scheme="rep")), "header", ttip_title = _("Brothel report"), ttip = rep_text)

            night_early.add(_("Advertising: +%s") % str_dec(raw_rep))
            if bonus_rep >= 1:
                night_early.add(_("Perks: +%s") % str_dec(bonus_rep))
            if decay_rep <= -1:
                night_early.add(_("Chapter modifier: %s") % plus_text(decay_rep))
            ...
            if not ad_pic:
                if brothel.advertising > 0:
                    ...
                    ad_pic = "resources/events/" + rand_choice(advertising_pics[adv_index])
                else:
                    ad_pic = "resources/events/" + rand_choice(night_pics)
        ...
        if farm.girls:
            night_early.add(_("Girls: %i") % len(MC.girls + farm.girls), "header", ttip = _("You have %i girls in your brothel and farm.") % len(MC.girls + farm.girls), ttip_title=_("Girl report"))
        else:
            night_early.add(_("Girls: %i") % len(MC.girls), "header", ttip = _("You have %i girls in your brothel.") % len(MC.girls), ttip_title=_("Girl report"))


        if working_girls:
            line = event_color["a little good"] % (_("Working: %s") % len(working_girls))
        ...
        if sick_girls:
            ...
            night_early.add(line, ttip=ttip, ttip_title = event_color["bad"] % _("Got sick"))

        if striking_girls:
            line = event_color["bad"] % (_("Refused to work: %s") % len(striking_girls))
            ttip = list_text([g.fullname for g in striking_girls])
            night_early.add(line, ttip=ttip, ttip_title = event_color["bad"] % _("Refused to work"))
        ...
        if resting_girls:
            line = event_color["normal"] % (_("Resting: %s") % len(resting_girls))
            ttip = list_text([g.fullname for g in resting_girls])
            night_early.add(line, ttip=ttip, ttip_title = _("Resting"))

        if away_girls:
            line = event_color["normal"] % (_("Away: %s") % len(away_girls))
            ttip = list_text([g.fullname for g in away_girls])
            night_early.add(line, ttip=ttip, ttip_title = _("Away"))

        if farm.girls:
            ttip = list_text([g.fullname for g in farm.girls])
            night_early.add(_("Farm: %s") % len(farm.girls), ttip=ttip, ttip_title = event_color["fear"] % _("At the Farm"), col=c_purple)
    ...
    ## Close brothel if no girls are available
        ...
        if not working_girls:
            night_early.add(_("Brothel closed"), "header", col="bad")
            night_early.add(_("Customers turned away: %s") % len(customers), ttip = get_customer_population_count(customers))
            ...
            if len(customers) > 5: # Lose rep for each customer that comes in vain
                ...
                if rep_loss:
                    night_early.add(_("Reputation lost: %s") % plus_text(rep_loss))
    ...
    ## Brothel maintenance (customers might turn away if the brothel is dirty)
        ...
        _penalty = cleanliness_penalties.get(cleanliness)
        if _penalty:
            turned_away = dice(_penalty["dice_count"], district.rank) + _penalty["dice_bonus"]
            maint_text = __(_penalty["text_i18n"])
        ...
        if working_girls:
            ...
            if turned_away > 0:
                ...
                if not customers:
                    text1 = __("Your brothel was so dirty that all %s customers ran away (%s reputation)") % (str(len(lost_customers)), str_int(rep_loss))
                else:
                    text1 = __("%s customers turned away because the brothel looked filthy (%s reputation)") % (str(len(lost_customers)), str_int(rep_loss))
                maint_text += "\n" + text1
            ...
            night_early.add(_("Customers: %s") % len(customers), "header", ttip = cust_text, ttip_title = _("Customer report"))
        ...
        #     ad_pic = Picture(ad_pic, "resources/events/" + ad_pic)

        first_event = Event(pic = ad_pic, char = "", text = night_text, changes = night_early, type ="UI") #!

        t5 = time.perf_counter()
        print("Brothel preparation took %.2f seconds" % (t5-t4)) # Debugging to spot possible 'infinite' loop

    #### SECURITY EVENTS ####

    if MC.get_effect("special", "defender") == 1: # Recover one interaction to deal with defense (Defender spell)
        $ MC.interactions += 1
    ...
    $ print("Security took %.2f seconds (delay only meaningful if no security event happened)" % (t6-t5)) # Debugging to spot possible 'infinite' loop
...
#### CUSTOMERS AND GIRLS INTERACT ####
    ...
    python:
        ...
        print("Jobgirls matchmaking took %.2f seconds" % (t7-t6)) # Debugging to spot possible 'infinite' loop
...
#                         game.mm_log += "\n%s removed (hurt)" % girl
        ...
        print("Whores matchmaking took %.2f seconds" % (t8-t7)) # Debugging to spot possible 'infinite' loop
    ...
    python:
        ...
        if not leftover_customers:
            ...
            ev = Event(Picture(pic, "resources/events/" + pic), text = text1 + _("girls were available for sex and %s customers left disappointed.") % str_int(len(leftover_customers)), type ="UI", changes=night_no_girls, debug_id=1)
    ...
    ### Update girl relationships
        ...
        print("Predicting events and updating relationships took %.2f seconds" % (t9-t8)) # Debugging to spot possible 'infinite' loop
    ...
    python:
        ...
        print("Farm preparation took %.2f seconds" % (t10-t9)) # Debugging to spot possible 'infinite' loop
    ...
    ### Rest your girls
        ...
        for girl in striking_girls + resting_girls:
            ...
            if used:
                ...
                if not pic:
                    pic = girl.get_pic("rest", "profile", and_tags = "naked", soft=True, horizontal=True)
            else:
                pic = girl.get_pic("rest", "profile", not_tags=["mass", "waitress", "geisha", "swim"], naked_filter=True, soft=True, horizontal=True)

            resting_events.append(Event(pic, char = girl.char, text = resting_text, changes = resting_changes, sound = extra_sound, type ="Rest"))

        t11 = time.perf_counter()
        print("Rest preparation took %.2f seconds" % (t11-t10)) # Debugging to spot possible 'infinite' loop
...
#### SHOW INTERACTIONS ####
    ...
    $ print("Running %i brothel events" % len(perform_events)) # Debugging to spot possible 'infinite' loop
    $ show_mm_debug_msg = True
    while perform_events:
        ...
        if ev == "job_matchmaking":
            ...
            if show_mm_debug_msg:
                $ show_mm_debug_msg = False
                $ print("End of matchmaking screens. %i perform events left." % len(perform_events)) # Debugging to spot possible 'infinite' loop
            call show_night_event(ev) from _call_show_night_event


    $ t12 = time.perf_counter()
    $ print("Calling events took %.2f seconds (delay only meaningful if event display is turned off)" % (t12-t11)) # Debugging to spot possible 'infinite' loop

    ## Change Brothel Rep

    python:
        night_late = NightChangeLog("Night recap", col=c_lightorange)
        night_late.add(__("Y%i M%i D%i %s") % (calendar.year, calendar.month, calendar.day, __(calendar.get_weekday())), "header")
        old_rep = brothel.rep
        ...
        night_late.add(_("Total customers: %i") % len(customers), "header")
        night_late.add(_("Entertainment (served): %s") % (event_color["average"] % served), ttip=_("%i customers were entertained out of %i waiting customers") % (served, len(customers)))
        night_late.add(_("Whoring (served): %s") % (event_color["average"] % laid), ttip=_("%i customers were tended to by a whore out of %i waiting customers") % (laid, len(customers)))
        night_late.add(_("Entertainment (satisfied): %s") % (event_color["a little good"] % entertained), ttip=_("%i customers were satisfied out of %i entertained customers") % (entertained, served))
        night_late.add(_("Whoring (satisfied): %s") % (event_color["a little good"] % satisfied), ttip=_("%i customers were satisfied out of %i whoring customers") % (satisfied, laid))

        log.add_report(event_color["good"] % (_("%s customer(s) came to your brothel. %s got attended (%s entertained), %s got laid (%s satisfied).") % (str(len(customers)), str(served), str(entertained), str(laid), str(satisfied))))

        rep_chg = sum(c.get_reputation_change() for c in customers)
        rep_chg = brothel.change_rep(rep_chg)

        night_late.add(_("Reputation: %s (%s)") % (str_int(brothel.rep) + "/" + str_int(brothel.max_rep), event_color["rep"] % (plus_text(rep_chg))), "header", ttip=plus_text(rep_chg) + _(" reputation increase from satisfied customers."))

        log.add_report(event_color["rep"] % (_("Brothel reputation changed from %i to %i (%s)") % (old_rep, brothel.rep, plus_text(round_int(rep_chg)))))
    ...
    $ renpy.say("", __("End of whoring phase"), interact=False)
    ...
    $ print("End of whoring phase took %.2f seconds (delay only meaningful if event display is turned off)" % (t13-t12)) # Debugging to spot possible 'infinite' loop
    $ print("Total time elapsed for brothel events: %.2f seconds" % (t13-t0)) # Debugging to spot possible 'infinite' loop

### SHOW FARM EVENTS ####

    $ print("Running %i farm events" % len(farm_events)) # Debugging to spot possible 'infinite' loop
    while farm_events:
...
#### SHOW RESTING GIRLS ####
    ...
    $ print("Running %i resting events" % len(resting_events)) # Debugging to spot possible 'infinite' loop
    ...
    if rel_changes:
        while rel_changes:
            ...
            if new_status == "friend":
                $ log.add_report(event_color["good"] % (__("%s and %s have become friends.") % (girl1.fullname, girl2.fullname)))
            elif new_status == "rival":
                $ log.add_report(event_color["bad"] % (__("%s and %s have become rivals.") % (girl1.fullname, girl2.fullname)))
            elif new_status == "normal":
                $ log.add_report(_("%s and %s are no longer %ss.") % (girl1.fullname, girl2.fullname, old_status))
...
#### END NIGHT ####
    ...
    ## Update mood
        ...
        for girl in (MC.girls + farm.girls):
            ...
            if girl not in (away_girls):
                girl.update_mood(resting = girl in [striking_girls + resting_girls])
    ...
    ## Clean up the brothel
        ...
        night_late.add(_("Cleanliness: %i (%s)") % (int(brothel.dirt), plus_text(log.dirt + chg)), "header", ttip=_("Your brothel is %s") % brothel.get_cleanliness())
        night_late.add(_("Dirt: %s") % plus_text(log.dirt))
        night_late.add(_("Maintenance: %s") % plus_text(chg))
    ...
    ## Pay the rent
        ...
        if loan_payment:
            ...
            _gold_text += __("\nYou paid {image=img_gold} %s to the bank for your loan.") % '{:,}'.format(round_int(loan_payment))

        _gold_recap = __("You paid {image=img_gold_24} %s in brothel costs.") % (event_color["a little bad"] % '{:,}'.format(round_int(log.upkeep+log.costs)))
    ...
    ## Katchiiing
        ...
        if bonus > 0:
            _gold_recap += " (" + (__("income bonus: {image=img_gold_24} ") + event_color["a little good"] % '{:,}'.format(round_int(bonus))) + ")"
        elif bonus < 0:
            _gold_recap += " (" + (__("income penalty: {image=img_gold_24} ") + event_color["a little bad"] % '{:,}'.format(round_int(bonus))) + ")"
        ...
        if brothel.get_effect("special", "resources as income"): # Taking into account the lost revenue, this is equivalent to a 50% discount on the cost of buying resources from the market
            ...
            nb = int(resource_value//resource_gold_value[resource_dict[resource].rank])

            if nb > 0:
                MC.gain_resource(resource, nb, message=False)
                _gold_recap += __(" and ") + str(nb) + " " + resource
                night_late.add(_("Resources"), "header")
                night_late.add(_("+%i {image=tb %s}") % (nb, resource), col="good")
        ...
        night_late.add(_("Income: {image=img_gold_20} %s") % plus_text(log.net, "gold"), "header")

        night_late.add(_("Girls income: ") + event_color["a little good"] % plus_text(log.gold_made-street_gold), ttip_title=_("Girl income"), ttip=list_text([_("%s: %s") % (capitalize(g.fullname), plus_text(g.get_log("total_gold", "today"))) for g in working_girls]))
        if street_gold:
            night_late.add(_("Street whores: ") + event_color["a little good"] %  plus_text(street_gold), ttip_title=_("Street whores income"), ttip=list_text([_("%s: %s") % (capitalize(g.fullname), plus_text(g.today_street_tip)) for g in MC.street_girls]))

        night_late.add(_("Costs: ")  + event_color["bad"] % str_int(-log.upkeep - log.costs), ttip = _gold_text)

        night_late.add(_("* Upkeep: %i") % -log.upkeep, col="bad", ttip = list_text([_("%s: %s") % (capitalize(g.fullname), plus_text(g.get_log("upkeep", "today"))) for g in (MC.girls+farm.girls)]))

        if working_girls:
            night_late.add(_("* Advertising: %i") % -brothel.get_adv_cost(), col="bad", ttip=_("%i advertising girls x %i = {image=img_gold} %i") % (brothel.advertising, helper_cost[district.rank], brothel.get_adv_cost()))

            night_late.add(_("* Security: %i") % -brothel.get_sec_cost(), col="bad", ttip=_("%i goons x %i = {image=img_gold} %i") % (brothel.security, helper_cost[district.rank], brothel.get_sec_cost()))

        night_late.add(_("* Maintenance: %i") % -brothel.get_maintenance_cost(), col="bad", ttip=_("%i maids x {image=img_gold} %i = {image=img_gold} %i") % (brothel.maintenance, helper_cost[district.rank], brothel.get_maintenance_cost()))

        if loan_payment:
            night_late.add(_("* Loan payment: %i") % loan_payment, col="bad")

        if bonus:
            night_late.add(_("Perks and special effects: %s") % plus_text(bonus, "standard"))

        MC.gold += log.net
        NPC_taxgirl.MC_income += (log.net * brothel.get_effect("boost", "taxable net income"))

        if log.net > 0:
            _gold_recap += __(", netting you a profit of {image=img_gold_24} {b}") + event_color["a little good"] % '{:,}'.format(log.net) + "{/b}."
        elif log.net < 0:
            _gold_recap += __(", failing to make a profit. You lost {image=img_gold_24} {b}") + event_color["a little bad"] % '{:,}'.format(-1*log.net) + "{/b}."
        else:
            _gold_recap += __(", breaking even. You made {image=img_gold_24} {b}0{/b}.")

        pic = rand_choice(treasure_pics["-"])
        for _thr in treasure_thresholds:
            if _thr["min"] is not None and log.net > _thr["min"]:
                pic = rand_choice(treasure_pics[_thr["category"]])
                break
        if not nsfw:
            pic = rand_choice(night_pics)

        ev = Event(Picture(pic, "resources/events/" + pic), text = _gold_recap, changes = night_late, sound = s_cash, type ="UI")
...
#        calendar.set_alarm(calendar.time, weighted_choice(rand_events)) # Chooses a random event for today (may not happen if prerequisites are not met)
    ...
    while exit_girls:
        ...
        call exit_farm(girl, farm.test_exit_conditions(girl))
...
#### PLAYING CONDITIONAL EVENTS
    ...
    $ hook_manager.invoke("on_settlement_end", settlement_ctx)
    $ hook_manager.invoke("on_day_start", settlement_ctx)
