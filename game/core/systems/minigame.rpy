...
image bg pavement = Tile("resources/minigame/bg pavement.webp")
...
init -2 python:
    ...
    def change_cursor(type="default"):
        if type == "default":
            ...
            setattr(config, "mouse", {"default": [("resources/minigame/toy hammer 1.webp", 0, 0)]})
        elif type == "2":
            setattr(config, "mouse", {"default": [("resources/minigame/toy hammer 2.webp", 0, 0)]})
...
screen score():
    frame xalign 0.0 yalign 1.0 xpadding 40 ypadding 40 xmargin 20 ymargin 20 background c_ui_darker:
        ...
        text _("{b}Hits{/b} %s") % ("X"*njgame.hits + " "*(3-njgame.hits)) color c_main

        text _("{b}Misses{/b} %s") % ("X"*njgame.misses + " "*(3-njgame.misses)) color c_lightred
...
label ninja_game(ninja): # Where ninja is an NPC object
    ...
    if game.chapter >= 3:
        call expression "c3_" + ninja.name.lower() + "_final_intercept" from _call_expression_11
