...
label security(working_girls, ev_type=None): # Happens when the threat level overcomes the event threshhold. ev_type may be provided for debugging.
    ...
    if ev_type == "raid": # A raiding party blitzes the brothel, trying to kidnap one of your girls. 2-4 girls are targeted, you can only defend one yourself.
        ...
        python:
            ...
            pic1 = "resources/events/" + rand_choice(security_pics["brothel defense"])
            pic2 = "resources/events/" + rand_choice(security_pics[attackers])
...
#        show screen show_event(girl.profile, x=config.screen_width, y=int(config.screen_height*0.8))
        ...
        if r:
            ...
            if girl.test_shield():
                play sound s_spell
                $ sec_pic = "resources/events/" + rand_choice(security_pics["girl shield"])
                ...
                python:
                    ...
                    if not sec_pic:
                        ...
                        if not sec_pic:
                            sec_pic = "resources/events/" + rand_choice(security_pics["default girl fight"])
                ...
                python:
                    ...
                    if not sec_pic:
                        ...
                        if not sec_pic:
                            sec_pic = "resources/events/" + rand_choice(security_pics["assassin"])
...
#                    text2 += "\n{color=[c_red]}" + girl.fullname + " has been kidnapped!{/color}"
        ...
        python:
            ...
            pic = "resources/events/" + rand_choice(security_pics["siege"])
        ...
        if not g_pic:
            $ g_pic = "resources/events/" + rand_choice(security_pics["default girl fight"])
                ...
                label enemy_general_captured():
                    ...
                    menu:
                        ...
                        "Keep her as a slave":
                            ...
                            $ enemy_general = None # Sanity check in case acquire_girl doesn't return
                            call acquire_girl(girl, can_follow=False) from _call_acquire_girl_5
    ...
    def get_security_event(ev_type, target_girls): # Returns a list of events to add to initial event list (alert levels 1 and 2 only)
        ...
        if ev_type == "thief":
            ...
            if guard_defense >= _max:
                sec_pic = "resources/events/" + rand_choice(security_pics["thief defense"])
                ...
                sec_pic = "resources/events/" + rand_choice(security_pics["thief"])
            ...
            if guard_defense >= _max:
                sec_pic = "resources/events/" + rand_choice(security_pics["monster defense"])
                sec_sound = s_clash
                ...
                sec_pic = "resources/events/" + rand_choice(security_pics["sword defense"])
                sec_sound = s_clash
                for girl in working_girls:
                    girl.change_love(1)
                ...
                if girl.test_shield():
                    ...
                    if not sec_pic:
                        sec_pic = "resources/events/" + rand_choice(security_pics["monster rape"])
                    sec_sound = s_roar
                    sec_with = vpunch
                    ...
                    if girl.has_trait("Virgin"):
                        if dice(6) >= 4:
                            girl.pop_virginity(origin="rape")
                            sec_text += __(event_color["bad"] % " She has lost her virginity.")
                        else:
                            sec_text += __(" Fortunately, her virginity was left intact.")
        ...
        ## ALERT LEVEL 2 ##
            ...
            if guard_defense >= _max:
                sec_pic = "resources/events/" + rand_choice(security_pics["assassin defense"])
                sec_sound = s_clash
                ...
                if MC.playerclass == "Warrior":
                    ...
                    sec_pic = "resources/events/" + rand_choice(security_pics["magic defense"])
                    sec_sound = s_spell
                elif MC.playerclass == "Trader":
                    sec_pic = "resources/events/" + rand_choice(security_pics["dragon defense"])
                    sec_sound = s_roar
                ...
                if girl.test_shield():
                    ...
                    if not sec_pic:
                        sec_pic = girl.get_pic("hurt", not_tags=["rest"], strict=True)
                        if not sec_pic:
                            sec_pic = Picture(path="resources/events/" + rand_choice(security_pics["assassin"]))
        ...
        elif ev_type == "brawl": # A brawl erupts in the tavern, causing some girls to get hurt in the fighting and some damage (dirt) to the brothel. No upside.

            _min, _max = alert_limits2[game.chapter]

            sec_pic = "resources/events/" + rand_choice(security_pics["brawl"])
            ...
            if guard_defense >= _max:
                ...
                if not story_flags["first security event"]:
                    NPC_sill.flags["first security heal"] = hurt_girls
        ...
        ## QUIET ##
            ...
            sec_sound = s_chimes
            sec_pic = "resources/backgrounds/castle night.webp"
        ...
        ## ESCALATION ##
            ...
            game.track("security event", -1)
            sec_sound = s_mystery
            sec_pic = "resources/events/" + rand_choice(security_pics["alert"])
            ...
            if brothel.alert_level == 1:
                sec_pic = "resources/events/" + rand_choice(security_pics["hood"])
                sec_sound = s_wolf
                ...
                sec_pic = "resources/events/" + rand_choice(security_pics["dark street"])
                sec_sound = s_wolf
        ...
        if lost_gold:
            ...
            if not story_flags["first security event"]:
                NPC_sill.flags["first security refund"] = lost_gold
        if bonus_gold:
            MC.gold += bonus_gold
...
#    stop music fadeout 2.0
    ...
    $ renpy.show_screen("show_event", Picture(path="resources/characters/npc/encounters/" + rand_choice(encounter_pics["secret_empty"])), _layer="master")
