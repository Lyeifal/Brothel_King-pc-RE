...
label before_main_menu(): # Will show before main menu (standard Ren'py label)
    ...
    ## EN: Load JSON-driven traits and perks first (BK Evolution).
    ## ZH: 先加载 JSON 驱动的特质和天赋（BK Evolution）。
    $ DataLoader.load_traits()
    $ DataLoader.load_perks()
    $ DataLoader.load_meta_progression()
    ...
    if new_gp:
        if len(new_gp) == 1:
            $ text1 = __("A new girl pack: {b}{color=%s}%s{/color}{/b} has been found! What would you like to do?") % (c_green, new_gp[0])
        else:
            $ text1 = __("%i new girl packs have been found:{b}{color=%s}%s{/color}{/b}. What would you like to do?") % (len(new_gp), c_green, and_text(new_gp))

        if renpy.call_screen("yes_no", text1, __("Update all girl mixes"), __("Don't update (update manually)")):
            python:
                for mix in persistent.girl_mix.values():
                    for gp in new_gp:
                        if gp not in mix:
    ...
    python:
        ...
        for gp in gpacks:
            gpinfo_dict[gp] = {"creator" : read_init_file_field(GirlFilesDict.get_ini(gp), "identity", "creator", _default="Unknown", skip_checks=True),
                                ...
                                "unique" : read_init_file_field(GirlFilesDict.get_ini(gp), "identity", "unique", _default=False, skip_checks=True) or read_init_file_field(GirlFilesDict.get_ini(gp), "cloning options", "unique", _default=False, skip_checks=True),
...
label after_load: # Happens after a game state is loaded
    ...
    python:
        ...
        for mod_name in game.active_mods.keys():
            ...
            if mod_name in detected_mods.keys():
                new_mod = detected_mods[mod_name]
                if game.active_mods[mod_name] != new_mod and game.active_mods[mod_name].version == new_mod.version: # A different instance of this mod object exists
                    game.active_mods[mod_name] = new_mod
            else:
                debug_notify("Couldn't detect '%s' mod." % mod.name)

            # Record load_label
            if hasattr(mod, "load_label") and mod.load_label:
                load_labels.append(mod.load_label)

    # Calls load_labels (load_label cannot take arguments)
    while load_labels:
        $ lbl = load_labels.pop()
        call expression lbl
...
label generic_event(title = __("An Event Most Generic"), pic = "bg farmland", desc = "One day as he was strolling by a surprisingly unremarkable place, {b}[MC.name]{/b} was struck to witness an event that was almost unfathomably generic.\n\nSo generic was that event, in fact, that within a few moments, he had forgotten all about it.\n\nIt left a strong impression in his mind that sometimes, things happen, and that sometimes, one simply does not give a flying fuck.\n\n{b}Boredom{/b} {color=[c_darkred]}+5{/color}"):
    ...
    call screen generic_event_screen(title, pic, desc) with dissolve
...
label chapter(chapter = None, silent=False): ## Shows the chapter intro with text1 as chapter number and text2 as chapter title. Keep game logic out of this label (use advance_to_chapter instead)
    ...
    python:
        ## EN: Load chapter titles from JSON if available (BK Evolution), else fallback to hardcoded.
        ## ZH: 优先从 JSON 加载章节标题（BK Evolution），否则回退到硬编码。
        try:
            import json, os
            _ch_titles_path = os.path.join(config.gamedir, "core", "data", "chapters", "chapter_titles.json")
            if os.path.exists(_ch_titles_path):
                with open(_ch_titles_path, "r", encoding="utf-8") as _f:
                    _chapter_titles = json.load(_f)
                del _f
                _ch_data = _chapter_titles.get(str(game.chapter), _chapter_titles.get("7"))
                text1 = __(_ch_data.get("title_i18n", _ch_data.get("title", "Epilogue")))
                text2 = __(_ch_data.get("subtitle_i18n", _ch_data.get("subtitle", "Work in progress")))
            else:
                raise IOError("chapter_titles.json not found")
        except Exception:
            if game.chapter == 0:
                text1 = __("Prelude")
                text2 = __("Dark streets, dark deeds")
            elif game.chapter == 1:
                text1 = __("Chapter One")
                text2 = __("Small Beginnings")
            elif game.chapter == 2:
                text1 = __("Chapter Two")
                text2 = __("Blades in the Night")
            elif game.chapter == 3:
                text1 = __("Chapter Three")
                text2 = __("A Dance of Fire and Steel")
            elif game.chapter == 4:
                text1 = __("Chapter Four")
                text2 = __("Drums of War")
            elif game.chapter == 5:
                text1 = __("Chapter Five")
                text2 = __("Work in progress")
            elif game.chapter == 6:
                text1 = __("Chapter Six")
                text2 = __("Work in progress")
            else:
                text1 = __("Epilogue")
                text2 = __("Work in progress")
    ...
    if not silent:
        ...
        $ text2 = Text((text2), size=64, yalign=0.6, xpos=0.5, drop_shadow=(2,2), font = "resources/fonts/VIVALDII.TTF")
...
label reached_goal():
    ...
    if game.seen_goal_message == False:
        if game.chapter == 1 and game.is_story_mode() and not debug_mode:
            ...
            call screen OK_screen(__("Goal reached!"), text1, pic = Picture(path="resources/ui/goal.webp"))
...
## 1. Basic chapter Set-up
    ...
    python:
        ...
        if start:
            print("Starting game at chapter %i" % chapter)
...
## 2. District choice
    ...
    python:
        for _unlock_ch, _district_keys in chapter_district_unlocks.items():
            if game.chapter >= _unlock_ch:
                for _dk in _district_keys:
                    game.blocked_districts.append(district_dict[_dk])

    # Choose target district
    if chapter <= 1:
        $ change_district(district_dict["slum"], free, start)

    elif 1 < chapter < MAX_CHAPTER:
        ...
        while loop:
            ...
            if isinstance(chosen_district, District):
                if district and chosen_district.name == district.name:
                    $ text1 = __("%s is the district where you are currently established. Please choose a different district.") % __(chosen_district.name)
                    "[text1]"
                else:
                    ...
                    if renpy.call_screen("yes_no", __("%s%s") % (__("{i}[chosen_district.description]{/i}\n\nDo you really want to move your brothel to {b}%s{/b}?\n\n{size=-2}This will reset all your room improvements, but you will keep your furniture and decorations.") % __(chosen_district.name), free_room_text)):
...
## 3. Brothel relocation
    ...
    # CONTINUE - Auction off old brothel
        if game.chapter != 2 or not game.is_story_mode():
    ...
    # Refresh max level, NewGame+ settings and calendar
    ...
    if start: # For debugging: deactivate MC spells when advancing
        ...
        $ game.update_achievements() # Enable or disables achievements depending on difficulty settings

    $ calendar.updates(change_district=not start) # change_district disables quest and resource market updates

    $ t3 = time.perf_counter()
    if debug:
        $ print("step3: %.2f seconds" % (t3 - t2))
...
## 5. Story events/unlocks
    ...
    if start:
        if not game.is_story_mode() or (chapter > 1 and debug_mode == "quick"):
        ...
        if chapter > 2:
            ...
            $ NPC_mizuki.love = 5
            if chapter == 3:
                $ game.set_task("The Water Kunoichi: Gather hints from your contacts", "story2")
    ...
    # Chapter 2 intro
    if game.is_story_mode() and game.chapter == 2: # Includes the auction_brothel event as part of the story
        call c2_intro() from _call_c2_intro
        call chapter2 from _call_chapter2

    if game.is_story_mode() and game.chapter == 3:
        if debug_mode:
            menu:
...
## 6. License, Rank and Goals updates
    ...
    if not silent:
        call screen OK_screen(__("You have received a new goal"), __(game.get_goal_description(channel="advance")), pic = Picture(path="resources/ui/goal.webp"))

    $ t6 = time.perf_counter()
    if debug:
        $ print("step6: %.2f seconds" % (t6 - t5))

## 7. End game

    if game.chapter == MAX_CHAPTER:
...
label got_license(level):
    ...
    call screen OK_screen(__("New license available!"), __("You have received a brand new %s. Good work!") % lic_name, pic = Picture(lic_pic, "resources/ui/" + lic_pic))
    ...
    if MC.interactions > 0:
        ...
        if len(MC.girls) >= brothel.bedrooms:
            ...
            menu:
                ...
                "Capture her and send her to the farm" if farm.active and farm.has_room():
                    $ capture_option = "farm"

                "Capture her and sell her":
                    $ capture_option = "sell"
...
label return_from_quest(girl, quest):
    ...
    if quest.return_label:
        call expression quest.return_label pass (girl) from _call_expression_9
...
label visit_exchange():
    ...
    while True:
        ...
        if result == "quit":
            ...
            elif renpy.call_screen("yes_no", __("Are you sure you want to exchange [source_nb] [source] for [target_nb] [target]?")):
...
label challenge(name, diff, score=False, raw=False, bonus=0, opponent_bonus=0, _sound=True, score_limit=0, forced=False, bonus_text="", opponent_bonus_text=""):
    ...
    if MC.challenges[name].estimate_diff(diff=diff+score_limit, score=score, raw=raw, bonus=bonus, opponent_bonus=opponent_bonus, forced=forced) != "Safe":
        call screen challenge(name, diff+score_limit, raw=raw, bonus=bonus, opponent_bonus=opponent_bonus, bonus_text=bonus_text, opponent_bonus_text=opponent_bonus_text)

    if (result and not score) or (result >= score_limit and score) or forced:
        if score:
            $ notify(__("Challenge result: %s") % plus_text(result, decimals=False))

        call success(_sound=_sound) from _call_success_15

    else:
        call failure(_sound=_sound) from _call_failure_8
        ...
        while True:
            ...
            if result[1] == "bargain":
                ...
                if MC.has_gold(price):
                    ...
                    $ result = renpy.call_screen("yes_no", __("Do you really want to buy this [it.name] for [price] gold?"))
                    ...
                    if result == True:
                        ...
                        if dice(6) == 1:
                            ...
                            if it.rank == it.min_rank:
                                ...
                                $ MC.items.append(it)
                                $ renpy.say("", __("You have been ripped off. This item is just %s.") % article(it.name))
...
label vital_scanners_built():
    ...
    $ _girls = [g for g in MC.girls if not g.away]

    if len(_girls) > 1:
        $ girl1, girl2 = rand_choice(_girls, 2)
    elif len(_girls) == 1:
        $ girl1 = girl2 = _girls[0]
    else:
...
label farm_shows_intro(girl):

    if story_flags["farm shows"]:
        return # Sanity check in case several girls activate the threshold at once

    $ story_flags["farm shows"] = True
...
label farm_show(girl=None):

    if story_flags["last farm event"] == calendar.time and not debug_mode:
        "You can only hold one farm show every day."
        return

    if not girl and debug_mode:
        $ girl = MC.girls[0] # For testing only

    hide screen farm_tab with dissolve

    camera:
        perspective False # This is needed so that the buttons don't break down. Don't ask me why.

## Step 1 (Init): Choose min_type, act and mode (cancel option) ##
    # 'min_type' can be random (and involve minions of various types)
    # 'act' is one of the 7 extended acts
    # "mode" is considered gentle, tough or hardcore (depending on girl reaction)
        # Gentle carries no penalties
        # Tough carries mood and fear penalties, but added preference impact.
        # Hardcore carries mood, fear and sanity penalties. Preferences may be altered in a large way, good or bad.

    # 1.1 Initialize variables
    $ chosen_minions = []
    $ chosen_act = "naked"

    # Pick initial act: Will automatically choose the most advanced act that is also gentle, or else 'naked'.
    python:
        for act in extended_sex_acts:
            if girl.will_do_farm_act(act) == "accepted":
                if act != "group" or farm.count_minions() >= 2:
                    chosen_act = act

    # 1.2 Show init screen

    show screen fshow_init(girl, chosen_act) with dissolve

    $ r = ui.interact()

    hide screen fshow_init with dissolve

    if r == "cancel":
        show screen farm_tab with dissolve
        return

    elif r == "commit":
        $ story_flags["last farm event"] = calendar.time

    # 1.3 Adjust minions

    if not chosen_minions:
        if chosen_act == "group": # Group can only be selected if there 2+ available minions
            if farm.count_minions() >= 3:
                $ chosen_minions = rand_choice(farm.get_healthy_minions(), 3)
            else: # Group cannot be chosen if there aren't at least two minions
                $ chosen_minions = rand_choice(farm.get_healthy_minions(), 2)
        else:
            $ chosen_minions = [rand_choice(farm.get_healthy_minions())]
    else:
        if chosen_act == "group":
            if len(chosen_minions) == 1: # Only one minion selected for group
                if farm.count_minions() >= 3: # Increases list to 3 minions
                    $ chosen_minions += rand_choice(farm.get_healthy_minions(), 2)
                else: # Increases list to 2 minions
                    $ chosen_minions.append(rand_choice(farm.get_healthy_minions()))
            elif len(chosen_minions) > 3: # Too many minions selected for group
                $ chosen_minions = rand_choice(chosen_minions, 3) # Trims list to 3 minions
        else:
            if len(chosen_minions) > 1: # Too many minions selected for single act
                $ chosen_minions = [rand_choice(chosen_minions)] # Trims list to 1 minion

    # 1.4 Find out minion type

    python:
        base_type = chosen_minions[0].type

        if chosen_act == "group":
            if are_different([mn.type for mn in chosen_minions]):
                min_descript = "various"
            else:
                min_descript = base_type + "s"
        else:
            min_descript = base_type


## Step 2: Bring in the customers (affected by % charge) ##

    # 2.1 Calculate rep for farm show
    $ rep = brothel.rep//len(MC.girls+farm.girls) + girl.rep*2 # Customers = bro rep / total nb of girls + (girl rep * 2)

    # 2.2 Generate customers according to rep
    $ customers, t, pop_dict = generate_customers(rep, False, False)

    # 2.3 Gather customer stats
    if customers:
        python:
            # Population count
            cust_details = ""
            for pop in all_populations:
                if pop_dict[pop.name]:
                    cust_details += __("%i %s\n") % (pop_dict[pop.name], pop.name)
            ...
            for c in customers:
                if c.wants_sex_act == chosen_act:
                    total_cust_budget += c.ent_budget * 1.25
                else:
                    total_cust_budget += c.ent_budget
                total_diff += c.diff

            if chosen_act == "bisexual":
                total_cust_budget *= tip_act_modifier["bisexual bonus"]*2
            elif chosen_act == "group":
                total_cust_budget *= tip_act_modifier["group bonus"]*len(chosen_minions)
            elif chosen_act != "naked": # Naked carries a base 1.0 modifier
                total_cust_budget *= tip_act_modifier[chosen_act]

            total_cust_budget = int(total_cust_budget * brothel.get_effect("boost", "farm tips"))
            average_diff = total_diff // len(customers)

    else: # Shouldn't happen
        "No customers available."
        return

## Step 3: Determine success of the show (depends on girl stats, min level and x perf)

    # 3.1 - Girl presents herself (BBCR test) - customers comment on her good/bad points. If naked, stops there.

    python:
        cust_score = 0
        intro_title = __("%s's introduction") % girl.fullname

        # Returns customer score for each stat between -8 and +8
        for cust in customers:
            cust_score += girl.test_stats(advanced_stats=farm_perform_dict["naked_stats"], diff=cust.diff)

        cust_score = int(cust_score / len(customers))

        # Describe best/worst stat
        _bbcr = ["beauty", "body", "charm", "refinement"]
        renpy.random.shuffle(_bbcr)
        _bbcr.sort(key = lambda x: girl.get_stat(x), reverse=True) # sorts stats by skill value
        top_stat = _bbcr[0]
        low_stat = _bbcr[-1]

        intro_desc = __("The customers gather to see %s perform %s.\n\n") % (and_text([event_color["love +"] % btext % girl.name, *[event_color["fear +"] % btext % mn.name for mn in chosen_minions]]), rand_choice(farm_perform_dict["location"])) # M1. Intro message

        if cust_score > 6: # Top score
            intro_desc += farm_perform_dict["intro perfect"][top_stat] % farm_perform_dict["intro minion"][min_descript]
        elif cust_score > 3: # Good score
            intro_desc += farm_perform_dict["intro good"][top_stat] % farm_perform_dict["intro minion"][min_descript]
        elif cust_score > 0: # Average score
            intro_desc += (farm_perform_dict["intro average+"][top_stat] + farm_perform_dict["intro average-"][low_stat]) % farm_perform_dict["intro minion"][min_descript]
        elif cust_score > -3: # Bad score
            intro_desc += farm_perform_dict["intro bad"][low_stat]
        ...
        if debug_mode:
            intro_title += __(" (score: %i)") % cust_score

        # Select intro pic (naked)
        intro_pic = girl.get_pic("naked", and_tags=farm_pics[base_type], soft=True)

## Step 4: Show performance - test skills depending on act type ##
        # Service: Service, Charm, Ob
        # Sex: Sex, Beauty, Libido
        # Anal: Anal, Body, Constitution
        # Fetish: Fetish, Refinement, Sensitivity
        # Bisexual: Service, Fetish, Libido, Sensitivity
        # Group: Sex, Anal, Ob, Con

    python:
        g_name = event_color["love +"] % btext % girl.name
        change_dict = {}

        # 4.1 Describe set-up
        perf_title = farm_perform_dict[chosen_act + " story " + base_type]["title"]
        perf_desc = farm_perform_dict[chosen_act + " story " + base_type]["desc"] # M2. Set-up
        ...
        if chosen_act  == "naked": # Naked act
            cum_pic = perf_pic = intro_pic # One pic only for naked show
            perf_desc = perf_desc % (g_name, event_color["fear +"] % btext % chosen_minions[0].name) # Custom set-up text

        # 4.4 (Naked) End the performance
            cum_desc = __("After giving the customers time to look at %s from every angle, Gizel tells the expectant crowd that the show is now over.") % g_name
            ...
            if chosen_act != "group":
                partner = event_color["fear +"] % btext % chosen_minions[0].name
                perf_pic = girl.get_pic(chosen_act, and_tags=farm_pics[base_type], not_tags=["group", "cumshot"])
            else:
                partner = and_text([event_color["fear +"] % btext % mn.name for mn in chosen_minions]) # Unused
                perf_pic = girl.get_pic(chosen_act, and_tags=farm_pics[base_type], not_tags=["cumshot"])

            # Custom set-up text
            if chosen_act in ("service", "fetish"):
                perf_desc = perf_desc % (g_name, partner)
            elif chosen_act in ("sex", "anal"):
                perf_desc = perf_desc % (partner, g_name)
            elif chosen_act == "bisexual":
                perf_desc = perf_desc % (g_name, partner, g_name)
            elif chosen_act == "group":
                perf_desc = perf_desc % g_name

            # Performance score - Returns customer score for each stat between -8 and +8
            cust_score_perf = 0
            for cust in customers:
                cust_score_perf += girl.test_stats(advanced_stats=farm_perform_dict[chosen_act + "_stats"], diff=cust.diff)

            cust_score_perf = int(cust_score_perf / len(customers))

            if cust_score > 3: # +1 for good result
                cust_score_perf += 1
            elif cust_score <= 0: # -1 for bad result
                cust_score_perf -= 1

            # Checks best/worst performing stat
            _stats = [s[0] for s in farm_perform_dict[chosen_act + "_stats"]]
            renpy.random.shuffle(_stats)
            _stats.sort(key = lambda x: girl.get_stat(x), reverse=True) # sorts stats by skill value
            top_stat = _stats[0]
            low_stat = _stats[-1]

            # M3, M4, M5. Crowd reaction, set-up second part, results
            if cust_score_perf > 6: # Top score
                perf_desc += "\n\n" + farm_perform_dict["perf good"][top_stat]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " story " + base_type]["desc2"]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " perfect"]
                change_dict["rep"] = district.rank
            elif cust_score_perf > 3: # Good score
                perf_desc += "\n\n" + farm_perform_dict["perf good"][top_stat]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " story " + base_type]["desc2"]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " good"]
            elif cust_score_perf > 0: # Average score
                perf_desc += "\n\n" + farm_perform_dict["perf good"][top_stat] + " " + farm_perform_dict["perf bad"][low_stat]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " story " + base_type]["desc2"]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " average"]
            elif cust_score_perf > -3: # Bad score
                perf_desc += "\n\n" + farm_perform_dict["perf bad"][low_stat]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " story " + base_type]["desc2"]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " bad"]
            else: # Terrible score
                perf_desc += "\n\n" + farm_perform_dict["perf bad"][low_stat]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " story " + base_type]["desc2"]
                perf_desc += "\n\n" + farm_perform_dict[chosen_act + " very bad"]
            ...
            if debug_mode: # Debugging only
                perf_title += __(" (score: %i)") % cust_score_perf

        # 4.4 (Other sex acts) End the performance
            # Big finish - customer(s) join in if perf is good. Bonus tip

            # Pop virginity
            if chosen_act in ("sex", "group"):
                if girl.pop_virginity("farm"):
                    cum_desc += __("The customers gasp as they notice %s was a virgin. They go wild realizing she just lost her virginity to a mindless minion in a filthy farm exhibit (%s).") % (g_name, event_color['good'] % "large bonus tip") # M6. Finish
                    cum_bonus += 1.0

            # Cumshot if score is at least average
            if cust_score_perf > 0:
                if chosen_act != "group":
                    cum_pic = girl.get_pic(chosen_act, and_tags=["cumshot"] + farm_pics[base_type], not_tags=["group"])
                    cum_desc += farm_perform_dict[min_descript + " cum"] % (partner, g_name) # M6. Finish
                else:
                    cum_pic = girl.get_pic(chosen_act, and_tags=["cumshot"] + farm_pics[base_type])
                    cum_desc += farm_perform_dict[min_descript + " cum group"] % g_name # M6. Finish

                if dice(10) <= cust_score_perf:
                    cum_desc += __("\n\nSome customers couldn't help but whip out their dicks and masturbate, the most daring of which came close enough to cum on %s's exposed body (%s).") % (g_name, event_color['good'] % "bonus tip")
                    cum_bonus += 0.15
            else:
                cum_pic = perf_pic
                cum_desc += __("After a while, realizing that %s is not going to carry the show much further, Gizel signals the end of the show. The customers leave, disappointed that it was cut short before %s could cum.") % (g_name, partner)


## Step 5: Determine tip (from 0.5 to 2.5 times the build-up tip) ##
    # Base tip = sum of ent_budget for all customers * rank
    python:
        if chosen_act == "naked":
            _score = cust_score
        else:
            _score = cust_score_perf

        # Calculate bonus. Score bonus is 10% per point
        _bonus = girl.get_build_up() / 100 + _score * 0.1 + cum_bonus

        # Calculate income
        _income = int(total_cust_budget * _bonus)


## Step 6: Results - Girl stats, preferences, mood and minion xp ##

    python:
        # 6.1 Energy loss
        change_dict["energy"] = -5 - 10*len(chosen_minions) # 15 EN spent + 10 per additional minion

        # 6.2 Farm shows will raise stats according to the chosen act
        _pos_stats = [s[0] for s in farm_perform_dict[chosen_act + "_stats"]]

        for stat in _pos_stats:
            if dice(250) > girl.get_stat(stat, raw=True):
                try:
                    change_dict[stat] += dice(1 + average_diff // 25)
                except:
                    change_dict[stat] = dice(1 + average_diff // 25)

        # 6.3 Farm shows may lower some stats

        _neg_stats = ["beauty", "refinement", "sensitivity"]

        for stat in _neg_stats:
            if dice(3) == 1:
                try:
                    change_dict[stat] -= dice(1 + average_diff // 50)
                except:
                    change_dict[stat] = -dice(1 + average_diff // 50)

        # 6.4 Apply stat changes
        for stat in change_dict.keys():
            c = girl.change_stat(stat, change_dict[stat])
            change_dict[stat] = c

        # 6.5 Preference changes
        brk = 0

        if girl.will_do_farm_act(chosen_act) == "accepted": # Moderate preference gain
            brk = girl.raise_preference(chosen_act, type=None, bonus=1.0, status_change=False, context="farm")
            if girl.get_stat("libido") > 150:
                girl.change_mood(2)

        elif girl.will_do_farm_act(chosen_act) == "resisted": # Large preference gain
            brk = girl.raise_preference(chosen_act, type=None, bonus=2.0, status_change=False, context="farm")
            girl.change_mood(-2)
            girl.change_fear(2)

        elif girl.will_do_farm_act(chosen_act) == "refused": # Chance of large preference gain or loss
            if girl.is_("very dom"):
                d = 10
            elif girl.is_("dom"):
                d = 8
            ...
            if dice(10) >= d:
                brk = girl.raise_preference(chosen_act, type=None, bonus=2.5, status_change=False, context="farm")
                cum_desc += __("\n\nEven though she was forced to go through the ordeal, %s was left strangely confused and aroused by the experience.") % g_name
            else:
                c = 375 - (girl.get_stat("obedience")//2 + girl.get_stat("libido"))
                brk = girl.change_preference(act, c)

            girl.change_mood(-5)
            girl.change_fear(5)

        else:
            debug_notify("NO PREFERENCE FOUND: %s" % chosen_act)

        if brk:
            change_dict[chosen_act + " preference"] = brk

        #! To Do: unlock weaknesses (act / farm)

        # 6.6 List results
        results_desc = btext % "Farm Show Results:\n" + list_stat_changes(change_dict)

        # 6.7 Minions experience
        for mn in chosen_minions:
            mn.xp += girl.rank*2
            if mn.level_up():
                notify(__("MINION LEVEL UP (level %s)") % mn.level, col="special")
...
## Step 7: Display events ##
    ...
    $ desc = (__("{b}%i customers came to watch the show{/b}:\n%s") % (len(customers), cust_details)) + (__("\n\n{b}Result forecast{/b}:\nFarm bonus: %i") % girl.get_build_up() + "%%\n{image=img_gold_20} " + event_color["average contrast"] % "{b}%s{/b}" + __("{size=-4} (will vary according to %s's performance)")) % ('{:,}'.format(total_cust_budget), girl.name)

    call screen fshow_screen(customers, title, pic, desc) with dissolve

    play sound _sound

    # 7.1 Intro screen
    call generic_event(intro_title, intro_pic, desc = intro_desc)

    # 7.2 Perf screen
    call generic_event(perf_title, perf_pic, desc = perf_desc)

    # 7.3 End screen
    call generic_event(perf_title, cum_pic, desc = cum_desc + "\n\n" + results_desc)

    # 7.4 Gold counter
    call screen farm_show_gold(girl, total_cust_budget, _income, _bonus)
    $ MC.change_gold(_income, silent=True)

## Step 8: Reset build-up ##
    $ girl.reset_build_up()
...
label send_to_farm(girl, duration=None, can_beg=True, can_cancel=True, can_follow=True, silent=False):
...
label farm_max_skill(girl, skill):
    ...
    # if girl in farm.girls and farm.programs[girl].act == skill: # Will not ask if program was changed
...
label farm_resisted(girl, context):
    ...
    $ prog = farm.programs[girl] # Notification already sent
    ...
    if NPC_gizel.flags[("no events", "all")] or NPC_gizel.flags[("no events", girl)]:
        python:
            text1 = ""
            if context.startswith("rebel"):
                if context.endswith("runaway"):
                    ...
                    text1 = __("%s fought Gizel at the Farm and got hurt.") % girl.fullname
                    _col = "a little bad"

                elif context.endswith("subdued"):
                    text1 = __("%s fought Gizel at the Farm and got subdued.") % girl.fullname
                    _col = "a little bad"

            elif context == "refused":
                text1 = __("%s refused training (visit Farm to change training mode).") % girl.fullname
                _col = "a little bad"

            elif context == "resisted":
                text1 = __("%s resisted training (visit Farm to change training mode).") % girl.fullname
                _col = "a little bad"

        if text1:
            $ notify(text1, "side gizel", _col)
        return
...
label farm_resisted_menu:
    if girl in farm.girls:
        menu:
            ...
            "Do not warn me about what happens to [girl.fullname] anymore (switch to notifications)":
                gizel angry "Well, I suppose I can stop badgering you with the misbehavior of that insignificant insect."
                # Disables Gizel 'resisted events for this specific girl
                $ NPC_gizel.flags[("no events", girl)] = True
...
label farm_take_out(_girl, check_room=True, context="normal"): # Context can be 'normal', 'check' or 'swap'
    ...
    if _girl in farm.girls:
        ...
        elif context == "swap" or (context == "normal" and renpy.call_screen("yes_no", __("Do you want to send [_girl.fullname] back to the brothel?"))):
            hide screen girl_stats
            hide screen girl_profile
            hide screen button_overlay
...
label first_contract():
    ...
    while True:
        ...
        elif renpy.call_screen("yes_no", __("{b}[result.title]{/b}\\nAre you sure you want to apply for this contract?")):
...
label run_contract_end():
    ...
    if con.result in ("success", "partial"):
        ...
        if con.result == "success" and con.special_bonus > 1.0:
            ...
            call dialogue(con.char, "contract buy offer", con.type)
            ...
            if girl.free:
                ...
                call dialogue(con.char, "contract buy offer refuse", con.type)
            else:
                menu:
                    extend ""
...
#                Official Meeting (rep), Magic conference (double AP), Orgy (pref increase)
        ...
        if chal == "rally":
            ...
            if r:
                ...
                play sound s_surprise
                show screen show_img("resources/characters/npc/encounters/impress1_5.webp") with dissolve
...
label auction_brothel(): # old_brothel_auction_price is et in advance_to_chapter
...
label tax_check(): # Happens on the morning of the 15th of every month starting from district rank 2. Taxgirl announces the tax amount
    ...
    if not NPC_taxgirl.active:
        ...
        if tx <= 0:
            ...
            # Sanity check (older versions)
            if hasattr(game, "en_brothels"):
                taxgirl "Here's how you rank among your competitors."
                call show_brothel_ranking() from _call_show_brothel_ranking
...
label show_brothel_ranking():

    python:

        #1. Store old ranking
        try:
            old_brothel_ranking = current_brothel_ranking
        except:
            old_brothel_ranking = sorted(game.en_brothels, key = lambda x: x.income, reverse = True)
            old_brothel_ranking.append(brothel)

        if brothel not in old_brothel_ranking: # Replaces old brothel in the list if needed
            for bro in old_brothel_ranking:
                if isinstance(bro, Brothel):
                    old_brothel_ranking[old_brothel_ranking.index(bro)] = brothel

        #2. Update ranking
        for bro in game.en_brothels:
            bro.randomize_income()

        current_brothel_ranking = sorted([brothel] + game.en_brothels, key = lambda x: x.get_income(), reverse = True)

        brothel.my_ranking = current_brothel_ranking.index(brothel) + 1


    # 3. Display ranking

    call screen brothel_ranking(old_brothel_ranking, current_brothel_ranking)

    # Store old income
    $ NPC_taxgirl.old_MC_income = income

    $ test_achievement("brothel ranking")
    ...
    label broken_menu:
        ...
        menu:
            ...
            "Send her to the asylum for 1 month (cost: {image=img_gold_24}[asylum_cost])":
                if renpy.call_screen("yes_no", __("Do you want to send her to the lunatic asylum (small chance of recovering some sanity) for one month at the cost of %i gold?") % asylum_cost):
            ...
            "Sell her to the highest bidder (auction starting price: {image=img_gold_24}[auction_min_price])" if not girl.free:
                if renpy.call_screen("yes_no", __("Are you sure you want to sell her (starting auction price: %i gold)?") % auction_min_price):
            ...
            "Let her go (free girl)" if girl.free:
                if renpy.call_screen("yes_no", __("Are you sure you want to let her go?")):
            ...
            "Keep her as a street whore (small daily income)":
                if renpy.call_screen("yes_no", __("Are you sure you want to turn her into a street whore? (she will live in the farm barn, generating a small daily income and no longer counting as one of your girls)")):
...
label asylum_no_room(girl):
    ...
    $ nun(__("Very well, we will keep her resting for %s more day%s.") % (duration, plural(duration)))
...
label acquire_girl(girl, price=0, context="generic", can_follow=True):
    ...
    if price and not MC.has_gold(price):
        if context in ("intro", "slavemarket"):
    ...
    if result:
        ...
        if context == "free":
        ...
        elif context in ("intro", "slavemarket"):
            $ MC.gold -= price
            play sound s_gold
            ...
            $ _bought_text = __("You just bought [girl.fullname]. I hope you will enjoy her.")
