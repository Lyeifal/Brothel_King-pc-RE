...
label security_introduction:
    ...
    # First incident effect is neutralized by Sill

    if NPC_sill.flags["first security refund"]:
        $ lost_gold = NPC_sill.flags["first security refund"]
        ...
        $ MC.change_gold(lost_gold)

    elif NPC_sill.flags["first security heal"]:
        $ healed = and_text([g.fullname for g in NPC_sill.flags["first security heal"]])
        sill "I have used the last of my recovery scrolls to heal [healed]. Please, don't let anyone get hurt again!"

        python:
            for girl in NPC_sill.flags["first security heal"]:
                girl.heal(10)
...
label help_farm_question():
    ...
    menu:
        ...
        "Recover a girl from Gizel" if NPC_gizel.flags["held_girls"]:
            $ girl = NPC_gizel.flags["held_girls"][-1]
...
#             gizel smirk "Of course, you're free not to set any condition, and keep a girl at the farm indefinitely. I have many {i}ways{/i} to keep the slaves entertained, don't you worry..."
        ...
        "Tell me about farm shows" if story_flags["farm shows"]:
                ...
                "Test brothel events" if debug_mode:
                    python:
                        ...
                        for key, pic_list in security_pics.items():
                            for pic in pic_list:
                                ev_list.append(Event(Picture(pic, "resources/events/" + pic), text = "", sound = s_cash))
...
label invoke_packstate_log():
    $ os.chdir(config.gamedir + "/") # Fix suggested by Dexell, not sure if it was Chris12's original intention
