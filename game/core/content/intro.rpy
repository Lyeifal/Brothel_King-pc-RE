...
label intro:
    ...
    $ text1 = Text(_("Once upon a time"), size=50, yalign=0.5, xpos=0.5, drop_shadow=(2,2), font="resources/fonts/MATURASC.ttf")
    $ text2 = Text(_("In a far away realm"), size=50, yalign=0.5, xpos=0.5, drop_shadow=(2,2), font="resources/fonts/MATURASC.ttf")
...
label day1:
    ...
    $ unlock_pic("bg slave market7", silent=True)
    ...
    if starting_chapter == 1:
        ...
        call advance_to_chapter(starting_chapter, silent=False, start=True) from _call_advance_to_chapter_4
...
label districts_first_time_slums:
    ...
    $ narrator(_("Choose a district."), interact = False)
    ...
    if not renpy.call_screen("yes_no", __("Do you really want to choose the Slums to open your new brothel?")):
...
label brothel_first_time:
    ...
    $ renpy.say(gio, __("Which one of the common rooms do you want me to repair?"), interact = False)
...
label main_first_time:
    ...
    $ brothel.name = renpy.input(__("How do you want to call your brothel?"), default = brothel.name, length = 40)

    if brothel.name.lower() in ("the rose garden", "玫瑰园"):
