...
# If every picture is 'gallery' set to False, it must be put in the 'unused' subdictionary.
    ...
    ## EN: Load UI element color mappings from JSON (BK Evolution), fallback to hardcoded.
    ## ZH: 从 JSON 加载 UI 元素颜色映射（BK Evolution），否则使用硬编码。
    _ui_col_json = DataLoader.load_ui_element_colors()
    if _ui_col_json:
        UI_elements_colors = [(prefix, color) for prefix, color in _ui_col_json.get("UI_elements_colors", [])]
    else:
...
#### GALLERY CREATION ####
    ...
    ## EN: Load EV gallery tab names from JSON (BK Evolution), fallback to hardcoded.
    ## ZH: 从 JSON 加载 EV 画廊标签页名称（BK Evolution），否则使用硬编码。
    _evg_json = DataLoader.load_ev_gallery_list()
    if _evg_json:
        ev_gallery_list = _evg_json.get("ev_gallery_list", [])
    else:
...
#    add "" xalign 0.5 yalign 0.5
    ...
    hbox spacing 10:
        ...
        vbox spacing 10:
            ...
            text _("Page %s/%s") % (str(page+1), str(max_page+1)) size int(config.screen_height*0.02)
