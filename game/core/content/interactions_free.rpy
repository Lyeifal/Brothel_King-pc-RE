...
init python:
    ## EN: Load free girl interaction love limits from JSON (BK Evolution), fallback to hardcoded.
    ## ZH: 从 JSON 加载自由女孩交互爱情上限（BK Evolution），否则使用硬编码。
    _fgl_json = DataLoader.load_free_girl_limits()
    if _fgl_json:
        free_girl_love_limit = {int(k): v for k, v in _fgl_json["love_limit_by_relationship_level"].items()}
    else:
        free_girl_love_limit = {0 : 25, 1 : 50, 2 : 50, 3 : 75, 4 : 100, 5 : 100}
...
label free_flirt_sex_tastes(girl):
    ...
    if dice(6) >= 4: #Describe positive acts and fixations
        ...
        if girl.personality_unlock[act] and dice(6) >= 4:
            ...
            $ renpy.say(girl.char, __("You know what fascinates me about %s?") % __(long_act_description[act]))
        ...
        if fix:
            ...
            if girl.personality_unlock[act] and dice(6) >= 4:
                ...
                $ text1 = __("%s makes me uncomfortable...") % __(long_act_description[act])
