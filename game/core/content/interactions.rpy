...
label slave_justify(girl, context):
    ...
    python:
        ...
        menu_list = [(__("What do you want to %s her for?") % context, None)]
...
label slave_interact_menu():
    ...
    if not topic:
        ...
        if not free or renpy.call_screen("yes_no", __("Do you really want to leave without training [girl.fullname]?")):
            pass
        else:
            jump slave_interact_menu
    elif topic == "previous":
        ...
        if topic.get_MP_cost(girl):
            $ MC.mana -= topic.get_MP_cost(girl)
...
label slave_train_free_form(girl): #!
    ...
    while not finish:
        ...
        python:
            _menu_items = [(__(a.capitalize()) + get_act_weakness_symbol(girl, a), a) for a in available_acts if a != act]
            if act != "naked":
                _menu_items += [(__("Cum inside"), "inside"), (__("Cum outside"), "outside")]
            _menu_items.append((__("Quit"), "quit"))
...
label slave_remove_fixation(girl):
    ...
    python:
        ...
        if len(neg_fix) == 1:
            ...
            renpy.say(you, __("Today, I want you to overcome your disgust for [fix.short_name]."))
...
#         girl.char "..."
        ...
        $ renpy.say("", __("Your rough training has proven too much for her. ") + event_color["bad"] % (__("She now hates %s for good.") % fix.name))
...
label slave_train_lecture(girl, act):
    ...
    python:
        _td_json = DataLoader.load_training_difficulty()
        if _td_json:
            diff = _td_json["training_difficulty"]
        else:
...
label slave_train_obedience(girl):
    ...
    python:
        _td_json = DataLoader.load_training_difficulty()
        if _td_json and "obedience_training_difficulty" in _td_json:
            _obd = _td_json["obedience_training_difficulty"]
            if girl.is_("very dom"):
                diff = _obd.get("very dom", 7)
            elif girl.is_("dom"):
                diff = _obd.get("dom", 6)
            elif girl.is_("very sub"):
                diff = _obd.get("very sub", 4)
            elif girl.is_("sub"):
                diff = _obd.get("sub", 5)
            else:
                diff = 5
...
label slave_train_constitution(girl):
    ...
    python:
        _td_json = DataLoader.load_training_difficulty()
        if _td_json and "constitution_training_difficulty" in _td_json:
            _ctd = _td_json["constitution_training_difficulty"]
            if girl.is_("very introvert"):
                diff = _ctd.get("very introvert", 7)
            elif girl.is_("introvert"):
                diff = _ctd.get("introvert", 6)
            elif girl.is_("very extravert"):
                diff = _ctd.get("very extravert", 4)
            elif girl.is_("extravert"):
                diff = _ctd.get("extravert", 5)
            else:
                diff = 5
        else:
            if girl.is_("very introvert"):
...
#    scene black
    ...
    if response == "accepted":
        $ text1 = __("Giving you a sheepish look, ")

    elif response == "resisted":
        $ text1 = __("Looking at you with mournful eyes, ")

    elif response == "magic":
        $ text1 = __("With a dazed stare, ")
...
label slave_advanced_training(girl, act, step):
    ...
    python:
        _tte_json = DataLoader.load_training_tag_exclusions()
        if _tte_json:
            _tte_data = _tte_json.get("training_tag_exclusions", {}).get("slave_advanced_training", {})
            for _tag in _tte_data.get("step_" + str(step), []):
                not_tags.append(_tag)
        else:
            if step < 3:
    ...
    if fix != "no fix":
        ...
        if pic:
            ...
            if step == 1:
                if pic.has_tag("town"):
                    $ use_location = ["town"]
                    $ text1 = __("You decide to take her out in the street, to spice things up.\n")
                elif pic.has_tag("beach"):
                    $ use_location = ["beach"]
                    $ text1 = __("Today is a good day to go to the beach, so you tell [girl.name] to follow you there.\n")
                elif pic.has_tag("nature"):
                    $ use_location = ["nature"]
                    $ text1 = __("You decide to take it to the park today, for a change.\n")
...
label slave_do(girl, act, context="generic"): # Receives 'pic' and 'reluctance' from the previous label
    ...
    if act == "naked":
        ...
        $ text1 = rand_choice((__("pinch her nipples"), __("fondle her breasts"), __("inspect her juicy body"), __("pat her butt"), __("caress her soft skin"), __("inspect every inch of her body"),
                  __("check out her generous curves"), __("give her a thorough physical exam"), __("just stand there, looking at her naked body")))
        ...
        $ inter.score = girl.get_stat(act) - reluctance
        $ text1 = rand_choice((__("rides your dick"), __("gets fucked in all positions"), __("gets fucked hard and deep"), __("gets a good pounding from your hard cock")))
        ...
        $ inter.score = girl.get_stat(act) - reluctance
        $ text1 = rand_choice((__("fuck her tight asshole"), __("lift her legs in the air and fuck her butt"), __("slap her ass as you give it a good fucking"), __("violate her ass")))
        ...
        $ inter.score = girl.get_stat(act) - reluctance
        $ text1 = rand_choice((__("use your favorite toys on her"), __("rape her with some of your new toys"), __("violate her holes with cruel and unusual tools"), __("tie her to a bizarre contraption and fuck her brains out")))
        ...
        $ text1 = rand_choice((__("licking the love juice from Sill's cunt"), __("fucking Sill with a black rubber strap-on"), __("rubbing her clit hard against Sill's pussy"), __("kissing Sill passionately while she plays with herself")))
        ...
        $ text1 = rand_choice((__("fuck her hard while she sucks the other guy off"), __("fuck her while the other guy does her ass"), __("use her holes to satisfy yourself and the random guy"), __("make her deepthroat your dick while the other guy rams her")))
...
label slave_hypnotize_driver(girl):

    if MC.hypnotize_driver == "mana":
        $ MC.hypnotize_driver = "gold"
    else:
        $ MC.hypnotize_driver = "mana"
...
label slave_magic(girl, mode="train"):
    ...
    if not hasattr(MC, "hypnotize_driver"):
        call slave_hypnotize_driver(girl) from _call_slave_hypnotize_driver

    $ text1 = rand_choice(MC.filter_say((__("ar: By the light of Arios{nw}"), __("sh: In the shadows of Shalia{nw}"), __("By the strength of the 5 elements{nw}"), __("ev: By the darkness of the 7 hells{nw}"), __("gd: By the purity of my soul"), __("ne: By the voice of dragons{nw}"),
                   __("ng: By the swift sands of time{nw}"), __("By the beat of the magic groove{nw}"), __("By the radiant power of the dead stars{nw}"), __("By the dark side of the red moon{nw}"), __("By the depth of the endless sea{nw}"))))

    $ text1 += __(", I command thee, ")

    $ text1 += rand_choice((__("follow my voice into the labyrinth..."), __("bid farewell to the limits of the physical world..."),
                           __("fall into a deep, deep slumber..."), __("do not fight the power of the ethereal dream..."), __("embark on the seas of oblivion..."),
                           __("forget who you are and where you lay..."), __("hear my stories and forget yourself...")))
    ...
    python:
        _td_json = DataLoader.load_training_difficulty()
        if _td_json and "hypnosis_training_difficulty" in _td_json:
            _htd = _td_json["hypnosis_training_difficulty"]
            diff = _htd.get(act, 2)
            _hpm = _td_json.get("hypnosis_personality_modifiers", {})
            if girl.is_("very dom"):
                diff += _hpm.get("very dom", 1)
            elif girl.is_("very sub"):
                diff += _hpm.get("very sub", -2)
            elif girl.is_("sub"):
                diff += _hpm.get("sub", -1)
        else:
            if act == "obedience":
...
label slave_reward_gift(girl):
    ...
    python:
        ...
        inter.result = long_menu(__("Choose a present"), gift_list)
...
label slave_rape(girl, act): # If girl refused and was forced
    ...
    if girl.personality_unlock[act]:
        ...
        if pos_reaction and neg_reaction:
            $ reaction = __("ambivalent feelings")
        elif neg_reaction:
            $ reaction = __("a disgust")
        else:
            $ reaction = ""
...
label slave_story1(girl):
    ...
    if girl.flags["story"] == 4:
        ...
        $ activity = rand_choice((__("fetching water"), __("running an errand"), __("going to school"), __("cleaning outside"), __("playing outside with my sister")))
...
label slave_story3(girl):
    ...
    if girl.flags["story"] == 4:
        ...
        if girl.is_("dom"):
            $ text1 = __("It isn't like her.")
        else:
            $ text1 = __("You wonder what it is this time.")
...
label slave_story7(girl):
    ...
    if girl.flags["story"] == 4:
        ...
        girl.char "I have given up all my hopes and dreams. But you know what I truly regret?"
        if girl.is_(["lewd", "sub"]): # This is an 'and' clause
            girl.char "The more I think about it, the more it makes sense... I'm a bitch and a terrible person. I need to be punished..."
            $ act = "punishment"
        ...
        elif girl.is_(["idealist", "modest"]): # This is an 'and' clause
            girl.char "I wish I could give my [girl.story_guardian] a real burial... Set our souls at peace."
            $ act = "burial"
...
label slave_story_help(girl, act):
    ...
    menu:
        ...
        "Summon a slimy tentacle to her room for the night (1 day)" if act == "demon" and MC.get_spirit() >= 5:
            ...
            if not pic:
                $ pic = Picture(path="resources/events/" + rand_choice(security_pics["monster rape"]))
        ...
        "Let her speak her mind freely (replace her negative trait with the 'godless' trait)" if act == "godless":
            ...
            python:
                ...
                ### JMan's suggestion ###
                if len(girl.traits) > 2:
                    if game.has_active_mod("traitking"):
                        _fixable_names = [tr.name for tr in neg_traits_fixable]
                        old = [trait for trait in girl.traits if trait.name in _fixable_names]
                        if not old:
                            _special_names = [tr.name for tr in traitking_pos_special]
                            _pos_names = [tr.name for tr in pos_traits]
                            old = [trait for trait in girl.traits if trait.name in _pos_names and not trait.name in _special_names]
                            if not old:
                                _neg_names = [tr.name for tr in neg_traits]
                                old = [trait for trait in girl.traits if trait.name in _neg_names]
                    else:
                        _neg_names = [tr.name for tr in neg_traits]
                        old = [trait for trait in girl.traits if trait.name in _neg_names]
                        if not old:
                            _ignore_names = ["Virgin"]
                            _pos_names = [tr.name for tr in pos_traits]
                            old = [trait for trait in girl.traits if trait.name in _pos_names and not trait.name in _ignore_names]

                    if old:
                        old = old[0]

                ### End of JMan's suggestion ###

                # old = [trait for trait in girl.traits if trait in neg_traits]
                # if old:
                #     old = old[0]
                # else:
                #     old = girl.traits[-1]     
        ...
        "Offer to increase her allowance (replace her negative trait with the 'expensive' trait)" if act == "saving":
            ...
            if not girl.has_trait('Expensive'):
                $ old = girl.traits[2].name
                $ girl.remove_trait(girl.traits[2])
        ...
        "Organize a public event (1500 gold upfront, unknown gains)" if act == "horse":
            ...
            show screen show_event(Picture(pic, "resources/events/" + pic), x=config.screen_width, y=int(config.screen_height*0.8))
        ...
        "Punish her like the bad girl she is" if act == "punishment":
            ...
            if act == "service":
                ...
                if pic.has_tag("oral"):
                    $ text1 = __("You will suck my cock patiently, until I decide to blow my load down your throat.")

                elif pic.has_tags(("handjob", "titjob")):
                    $ text1 = __("You will service me with your tits and hands. Get ready!")

                elif pic.has_tag("mast"):
                    $ text1 = __("You will masturbate for me, until I bring myself off over your slutty body.")

                else:
                    $ text1 = __("You will use your body to pleasure me.")

            elif act == "sex":
                $ text1 = __("I will fuck your dirty pussy now. Prepare to receive your Master's dick.")
                $ vir = girl.pop_virginity(origin="MC")

            elif act == "anal":
                $ text1 = __("I will fuck your ass like the filthy slut you are. And then I'll fill it up with cum.")

            elif act == "fetish":
                $ text1 = __("Since you like kinky stuff, let's see how well you handle a little pleasure mixed with a lot of pain.")
