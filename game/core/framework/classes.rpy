...
#            renpy.say("", "Rolled " + str(your_score) + "against " + str(enemy_score))
        ...
        def add_effect(self, effect, apply_boost=False, spillover=False, expires = False):
            return self.add_effects([effect], apply_boost, spillover, expires)
        ...
        def wound(self, _mod, _duration):
            ...
            notify(MC.name + " is wounded for %i days." % _duration)
    ...
    class NPC(): # Attributes: name, pictures, inventory, girl inventory, character
        ...
        def restock(self, once_a_day=True, update_flag = True): # For shop NPCs
            ...
            if self == shop:
                shop_mix = [(qual, parse_dice_formula(shop_item_number["shop"][qual]) + self.stock_modifiers[qual] + shop_chapter_modifiers[game.chapter][qual]) for qual in all_qualities]

            elif self in city_merchants:
                shop_mix = [(qual, parse_dice_formula(shop_item_number["city"][qual]) + self.stock_modifiers[qual] + shop_chapter_modifiers[game.chapter][qual]) for qual in all_qualities]

            elif self in minion_merchants:
                shop_mix = [(typ, parse_dice_formula(shop_item_number["minion"][typ]) + self.stock_modifiers[typ] + shop_chapter_modifiers[game.chapter][typ]) for typ in ["minion", "item"]]
...
#            renpy.say("", self.label + "HAPPENS")
    ...
    class Trait(object):
        ...
        def from_dict(cls, d):
            ...
            if effects:
                effects = [Effect.from_dict(e) if isinstance(e, dict) else e for e in effects]
            return cls(
                name=d.get("name"),
                verb=d.get("verb", "be"),
                effects=effects,
        ...
        def get_description(self):
            ...
            if self.type in ("special", "personality"):
                ...
                if target == "naked":
                    ...
                    if self.chance < 1:
                        text1 += "%i%% chance of ignoring hurt damage" % (self.chance*100)
                    else:
                        text1 += "Cannot be hurt"
    ...
    class GirlInteractionTopic(object):
        ...
        def is_available(self, girl, mode=None, free=False): # The option will display inactive if False. Returns a tuple with bool and a tooltip description.
            ...
            if MC.interactions < 1 and self.AP_cost > 0 and not free:
                ...
                return False, "You do not have enough mana for this training (" + str(self.get_MP_cost(girl)) + "{image=img_MP})."
...
#                     name = req[4:]
    ...
    class EnemyBrothel(object):
        ...
        def generate_name(self):
            first = ("The", "House of the", "Den of the", "Garden of the")

            second = ("beautiful", "sleazy", "naughty", "horny", "hungry", "hasty", "mighty", "sweaty", "rowdy", "bawdy", "screwy", "loony", "bloody", "holy", "spicy", "grim", "old", "ol'", "lame", "hunchback", "blind", "rebel", "mysterious", "secret", "rogue", "lurking", "singing", "dancing", "laughing", "limping", "crying", "golden", "silver", "bronze", "copper", "iron", "stone", "ice", "fire", "velvet", "red", "green", "blue", "yellow", "purple", "violet", "white", "black", "light", "dark", "pink", "hot", "cold", "big", "long", "tiny", "fat", "ruby", "emerald", "opal", "Borgese", "Western", "Eastern", "Northern", "Southern", "Zanic")

            third = ("dragon", "griffin", "pegasus", "unicorn", "basilisk", "hydra", "hippogriff", "horse", "giant", "goliath", "kraken", "goblin", "orc", "gnoll", "hobgoblin", "troll", "mare", "bear", "boar", "cock", "rabbit", "snake", "serpent", "lizard", "dog", "cat", "swan", "falcon", "owl", "snail", "butterfly", "shark", "dolphin", "whale", "mermaid", "bard", "knight", "prince", "princess", "sailor", "priest", "pirate", "corsair", "gentleman", "man", "lady", "madam", "maiden", "belle", "star", "glove", "hat", "eye", "hand")

            names = ("Joe", "Mary", "Bill", "Sam", "Wendy", "Zora", "Zelda", "Sara", "Mila", "Estrella", "Lee", "Weng", "Dong", "Fang", "Chan", "Chen", "Mathilda", "Jon", "Dick")

            fourth = ("'s inn", "'s tavern", "'s club", "'s lair", "'s den", "'s World", "'s parlor", "'s place", "'s emporium" " Villa", " Palazzo", " of Sin", " of Pleasure", " of Redemption", " of Damnation", " of Lust", " of Westmarch", " of Karkyr")

            d = dice(10)

            if d <=2:
                name = rand_choice(first) + " " + rand_choice(second).capitalize() + " " + rand_choice(third).capitalize()
            elif d <= 3:
                name = rand_choice(second).capitalize() + " " + rand_choice(third).capitalize()
            elif d <= 4:
                name = rand_choice(second).capitalize() + " " + rand_choice(names).capitalize() + "'s"
            elif d <= 5:
                name = rand_choice(names).capitalize() + rand_choice(fourth)
            else:
                name = rand_choice(second).capitalize() + " " + rand_choice(third).capitalize() + rand_choice(fourth)
