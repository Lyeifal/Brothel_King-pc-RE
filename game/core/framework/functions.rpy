...
init -3 python:
    ...
    # Ensure formats are defined (moved from variables.rpy for load-order safety)
    if 'VIDEOFORMATS' not in globals():
        VIDEOFORMATS = (".webm", ".mkv", ".avi", ".mpg", ".mpeg")
    if 'IMGFORMATS' not in globals():
        IMGFORMATS = (".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp", ".avif")
...
#                         girl.add_log("hurt_days")
    ...
    def transact(obj, seller, buyer, price):
        ...
        if seller.type in ("MC", "girl"):
            notify("You have sold the %s to %s." % (obj.name, buyer.name))
...
#                     specials.append("flasher")
        ...
        if ignore_budget:
            ...
            if sum(tip_gains.values()) >= total_budget:
                budget_ttip += " (maxed)"
            change_log.add("Customer budget: %i" % int(initial_budget), ttip=budget_ttip, ttip_title="Customer budget")
