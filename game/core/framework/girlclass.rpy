...
#                log_file.write(self.path + " (" + str(len(GirlFilesDict.get_pics(self.path))) + " images)\n" + timingResult + "\n\n" + " ".join(traceback.format_stack()))
        ...
        def generate_traits(self, p_traits=None, n_trait = None): # Where p_traits is a list of gold or positive trait names, n_trait is a negative trait name
            ...
            new_traits = [] # A list of trait names that will be added to the girl
            ...
            while len(new_traits) < pos_traits_nb:
                ...
                if NGP_settings_dict["naturist frequency"].get():
                    for t in new_traits:
                        if t == "Naturist": # Cannot add Naturist twice
                            break
                    else:
...
#                renpy.say("", "Girl - Naked filter ON")
                if self.naked:
                    ...
                    not_tags.insert(0, "naked") # Places "naked" at the beginning of the not_tags list
            ...
            if soft: # Inserts soft filters at the beginning of the not_tags list
                not_tags = [ntag for ntag in (all_sex_acts + ["group", "bisexual", "cumshot"]) if ntag not in not_tags] + not_tags

            # Lesbian pics will be excluded unless it is explicitly requested or the context is bisexual/group
            ...
            if not allow_lesbian and "lesbian" not in (tags + and_tags + not_tags) and "bisexual" not in (tags + and_tags) and "group" not in (tags + and_tags):
                not_tags.insert(0, "lesbian") # Places "lesbian" at the beginning of the not_tags list

            # 'Virgin' girls will never have pictures displaying sex shown unless the act is explicitly 'sex' or 'group'

            if self.has_trait("Virgin"):
                if not "sex" in (tags + and_tags + not_tags) and not "group" in (tags + and_tags):
                    not_tags.insert(0, "sex") # Places "sex" at the beginning of the not_tags list
            ...
            if hide_farm:
                if not persistent.fuzzy_tagging_acts: # Disables machine and big for all non farm picture search
                    not_tags.insert(0, "machine") # Places "machine" at the beginning of the not_tags list
                    not_tags.append("big")
            ...
            return get_pic(self, tags=tags, alt_tags1=alt_tags1, alt_tags2=alt_tags2, alt_tags3=alt_tags3, and_tags=and_tags, not_tags=not_tags, strict=strict, and_priority=and_priority, attempts=attempts, always_stock=always_stock, horizontal=horizontal, vertical=vertical)
...
#            renpy.say(self.char, "My capacity is " + str(cust_cap) + " for " + job + " (" + self.fullname + "")
        ...
        def get_tip(self, act, result, customers, final_tip_change=0, first_customer=False, specials = []):
            ...
            # JMan's suggestion to enable Chris's job Mod to work
...
## LOG ACTIONS - Note: The various logs and stats are really messy and should be reworked from the grounds up
        ...
        def unlock_info(self, topic):
            if topic not in self.notebook_unlocks:
                self.notebook_unlocks.append(topic)
        ...
        def get_personality_description(self, show="personality"):
            ...
            if show == "personality":
                ...
                if "EI" in self.notebook_unlocks and "MI" in self.notebook_unlocks and "DS" in self.notebook_unlocks and "LM" in self.notebook_unlocks:
                    pers_des += self.personality.description + "\n\n"
