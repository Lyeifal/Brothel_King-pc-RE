...
## Allow Ren'Py to find resources under game/resources/ (e.g. gui/ moved to resources/gui/)
init -10 python:
    import os
    _resources_dir = os.path.join(renpy.config.gamedir, "resources")
    if _resources_dir not in config.searchpath:
        config.searchpath.append(_resources_dir)

define patch_version = "v260527"
...
define config.save_directory = "Bro King-2409"
...
define config.window_icon = "resources/gui/window_icon.webp"
...
init python:
    ...
    build.classify('game/girls/**.txt', "all")
