...
class EffectEditor(tk.Frame):
    ...
    def __init__(self, parent, effect=None):
        super().__init__(parent, bg=parent.cget("bg"))
        from bk_editor.core import EFFECT_TYPES, EFFECT_TARGETS, EFFECT_SCOPES

        self.type_combo = LabeledCombobox(self, "类型:", EFFECT_TYPES, default=effect.get("type", "boost") if effect else "boost")
        self.type_combo.pack(fill=tk.X, pady=2)

        self.target_combo = LabeledCombobox(self, "目标:", EFFECT_TARGETS, default=effect.get("target", "") if effect else "")
        ...
        self.scope_combo = LabeledCombobox(self, "范围:", ["(无)"] + EFFECT_SCOPES[1:], default=effect.get("scope", "") if effect else "")
