...
本文档说明 `tools/` 目录下的脚本及其使用方法。所有脚本已配置自动检测项目根目录，**从项目根目录或 tools 目录均可运行**：
...
# 方式二：直接从 tools 目录运行
cd C:\Users\akxls\Documents\Code\BK\Brothel_King-pc\tools
python <script>.py
```
...
```bash
python tools/export_translation_xlsx.py
```
...
```bash
python tools/import_translated_xlsx.py to_translated.xlsx
```
...
python tools/fix_all_variables.py

# 2. 修复 %s 占位符数量不匹配
python tools/fix_placeholders.py

# 3. 修复文本标签不匹配（如 {b}, {color=...}）
python tools/fix_all_tag_mismatches.py
...
| `fix_all_tag_mismatches.py` | 修复不匹配的 `{b}`/`{color}`/`{i}`/`{size}` 标签 |
...
| 脚本 | 说明 |
|------|------|
| `merge_translations.py` | 扫描源代码 `__()`/`_()` 调用，重新生成 `strings.rpy` |
...
| `batch_translate_dialogue.py` | 使用 Google Translate 批量翻译对话（带占位符保护） |
| `export_translation_docx.py` | 导出翻译到 docx 格式 |
| `remove_duplicates.py` | 删除 `strings.rpy` 中重复的 `old` 键 |
...
| `convert_docx.py` / `txt_to_docx.py` / `docx_to_txt.py` | docx/txt 格式转换 |
| `check_*.py` | 各类代码检查脚本 |
| `mark_*.py` | 源代码标记脚本（给字符串包裹 `__()`） |
...
解决：运行 `python tools/fix_placeholders.py`。

### 游戏启动时报错 `A translation for "..." already exists`

原因：剧情文件和 `strings.rpy` 中有重复的 `old` 键。

解决：运行 `python tools/merge_tl_strings.py` 或 `python tools/remove_duplicates.py`。

### 游戏启动时报错 `NameError: Name 'xxx' is not defined`

原因：`[variable]` 形式的变量名被 Google Translate 翻译成了中文（如 `[妓院广告]`）。

解决：运行 `python tools/fix_all_variables.py`。

### 游戏启动时报错 `/x closes a text tag that isn't open`

原因：`{b}`, `{color=...}` 等文本标签不匹配。

解决：运行 `python tools/fix_all_tag_mismatches.py`。

### 导入后很多对话仍是英文

1. 确认 `to_translate_remaining.xlsx` 中的原文列（A列）和翻译列（B列）正确
2. 重新运行 `python tools/find_untranslated.py` 查看最新统计
3. 如果源文件有新增文本，先运行 `python tools/merge_translations.py` 重新生成 `strings.rpy`
